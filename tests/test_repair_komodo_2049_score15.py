...
SCRIPT = REPO / "scripts" / "repair_komodo_2049_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2049_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2049")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 69)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 6.0,
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 2049 | "
            "DSMZ Medium: 2049 (mediadive.medium:2049)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2049",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 69)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2049_metabolite_table(repair_module, scorer_module) -> None:
    ...
    assert ingredients["ZnSO4 x 7 H2O"]["concentration"] == {
        "value": "22.00",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["Na2-EDTA"]["term"] == {
        "id": "CHEBI:64734",
        "label": "EDTA disodium salt (anhydrous)",
    }
    assert ingredients["(NH4)6 Mo7O24 x 4 H2O"]["term"] == {
        "id": "CHEBI:91249",
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2049}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2047"
