...
SCRIPT = REPO / "scripts" / "repair_togo_m1930_plate_count_agar_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1930_plate_count_agar")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1930")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Plate Count Agar (PCA)",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        "composition_type": "UNDEFINED",
        "physical_state": "SOLID_AGAR",
        "media_term": {
            "preferred_term": "TOGO Medium M1930",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Plate Count Agar (PCA)",
            },
        },
        "notes": "Source: NBRC",
        "ingredients": [
...
def test_repair_restores_nbrc_formula_and_leaves_review_ranking(
    repair_module,
    scorer_module,
) -> None:
...
def test_repair_record_rejects_wrong_id(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="expected id CultureMech:008509"):
        repair_module.repair_record(doc)


def test_repair_record_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M1931"

    with pytest.raises(ValueError, match="expected media term TOGO:M1930"):
        repair_module.repair_record(doc)
