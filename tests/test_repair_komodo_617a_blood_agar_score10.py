...
SCRIPT = REPO / "scripts" / "repair_komodo_617a_blood_agar_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_617a_blood_agar")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_617a")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_617a").CHILDREN))
def test_children_link_to_dsmz_617a_parent(repair_module, child) -> None:
    ...
    assert repair_module._ingredient_signature(repaired) == repair_module.PARENT_SIGNATURE
...
def test_strain_pointer_exits_review_ranking(repair_module, scorer_module) -> None:
    child = repair_module.CHILD_BY_PATH[Path("bacterial/for_dsm_7232.yaml")]
    repaired = repair_module.repair_child(
        child.path,
        _load_yaml(repair_module.NORMALIZED / child.path),
    )
...
def test_child_nacl_correction(repair_module) -> None:
    ...
    for row in doc["ingredients"]:
        if row["preferred_term"] == "NaCl":
            row["concentration"]["value"] = "30"

    repaired = repair_module.repair_child(child.path, doc)

    assert repair_module._ingredient_signature(repaired) == repair_module.PARENT_SIGNATURE


def test_parent_is_promoted_and_lists_617a_children(repair_module) -> None:
