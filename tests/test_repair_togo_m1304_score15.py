...
SCRIPT = REPO / "scripts" / "repair_togo_m1304_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1304_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1304")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1304",
            "term": {"id": repair_module.MEDIA_TERM, "label": "BSW3 Agar"},
        },
        "notes": "Source",
...
def test_repair_normalizes_components_and_ph(repair_module) -> None:
    ...
    assert repair_module._signature(repaired["ingredients"]) == (
        ...
        ("Glucose", "0.4", "G_PER_L"),
        ("Agar", "15", "G_PER_L"),
        ("0.1 x Artificial seawater", "1000", "ML_PER_L"),
    )

    assert ingredients["0.1 x Artificial seawater"]["concentration"] == {
        "value": "1000",
        "unit": "ML_PER_L",
    }
...
def test_repair_leaves_opaque_products_unmapped(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert "term" not in ingredients["Yeast extract (BD-Difco)"]
    assert "term" not in ingredients["Malt extract (BD-Difco)"]
    assert "term" not in ingredients["0.1 x Artificial seawater"]


def test_repair_adds_prep_references_and_event_once(repair_module) -> None:
    ...
    assert "mediadive.medium:7" in matching_events[0]["notes"]


def test_repair_drops_record_out_of_review_ranking(repair_module, scorer_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_record_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][1]["preferred_term"] = "Sea water"

    with pytest.raises(ValueError, match="ingredient signature drifted"):
        repair_module.repair_record(doc)


def test_repair_record_rejects_unexpected_kg_microbe_match(repair_module) -> None:
    doc = _doc(repair_module)
    doc["kg_microbe_match"] = "mediadive.medium:J1216"
