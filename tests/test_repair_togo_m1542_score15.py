...
SCRIPT = REPO / "scripts" / "repair_togo_m1542_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1542_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1542")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1542",
...
def test_repair_grounds_discrete_components(repair_module) -> None:
    ...
    assert ingredients["Agar (if needed)"]["concentration"] == {
        "value": "10.0",
        "unit": "G_PER_L",
    }
...
def test_repair_preserves_parent_link_without_inventing_steps(repair_module) -> None:
...
def test_target_record_matches_togo_m1542_repair_contract(
    repair_module,
) -> None:
