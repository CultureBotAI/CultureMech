...
SCRIPT = REPO / "scripts" / "repair_togo_m1936_lb_ampicillin_hygromycin_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1936_lb_ampicillin_hygromycin_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1936")
...
def _target_doc(repair_module) -> dict:
    return {
        ...
        "original_name": "LB + Ampicilin, Hygromycin medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1936",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "LB + Ampicilin, Hygromycin medium",
            },
        },
...
def test_repair_adds_flags_references_and_event_once(repair_module) -> None:
    ...
    assert once["references"] == [
        {"reference": repair_module.TOGO_M1936},
        {"reference": repair_module.NBRC_1203},
    ]
    matching_events = [
        event
