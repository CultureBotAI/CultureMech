...
SCRIPT = REPO / "scripts" / "repair_nbrc_316_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_nbrc_316_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_nbrc_316")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1529",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "TOGO Medium M1529",
            },
        },
        "notes": "Source: NBRC - NBRC_M316",
        ...
        "variant_modifications": [
            "MgSO4·7H2O decreased from 1 g/L to 0.5 g/L while the TOGO "
            "Methylobacterium medium formulation remains unchanged.",
        ],
    }
...
def test_repair_corrects_formula_ph_range_and_exits_review_ranking(
    repair_module,
    scorer_module,
) -> None:
    ...
    assert repaired["ph_range"] == {"min": 9.0, "max": 10.0}
...
def test_repair_moves_empty_solutions_to_grounded_formula_ingredients(
    repair_module,
) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
...
def test_repair_adds_only_source_stated_carbonate_preparation(
    repair_module,
) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "Na2CO3 from migrated empty" in matching_events[0]["notes"]
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["solutions"][0] = _solution("Na2CO3", "10", "G_PER_L")
