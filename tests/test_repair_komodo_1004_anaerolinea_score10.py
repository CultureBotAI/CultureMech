...
SCRIPT = REPO / "scripts" / "repair_komodo_1004_anaerolinea_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1004_anaerolinea")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1004")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_1004").CHILDREN))
def test_anaerolinea_strain_pointers_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_parent_preserves_dsmz_duplicate_and_lists_strain_children(
    repair_module,
) -> None:
