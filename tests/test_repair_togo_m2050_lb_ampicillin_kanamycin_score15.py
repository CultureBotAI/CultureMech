...
SCRIPT = REPO / "scripts" / "repair_togo_m2050_lb_ampicillin_kanamycin_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2050_lb_ampicillin_kanamycin_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2050")
...
def _target_doc(repair_module) -> dict:
    return {
        ...
        "original_name": "LB + Ampicillin, Kanamycin medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2050",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "LB + Ampicillin, Kanamycin medium",
            },
        },
...
def test_repair_expands_high_salt_lb_antibiotic_stocks(
    repair_module,
    scorer_module,
) -> None:
...
def test_repair_adds_flags_references_and_event_once(repair_module) -> None:
    ...
    assert once["references"] == [
        {"reference": repair_module.TOGO_M2050},
        {"reference": repair_module.NBRC_1350},
    ]
    matching_events = [
        event
