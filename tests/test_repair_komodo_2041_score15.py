...
SCRIPT = REPO / "scripts" / "repair_komodo_2041_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2041_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2041")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 315)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 7.0,
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 2041 | "
            "DSMZ Medium: 2041 (mediadive.medium:2041)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2041",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 315)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2041_metabolite_table(repair_module, scorer_module) -> None:
    ...
    assert ingredients["NiCl2 x 6 H2O"]["concentration"] == {
        "value": "0.02",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["CuSO4 x 5 H2O"]["term"] == {
        "id": "CHEBI:31440",
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2041}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2039"
