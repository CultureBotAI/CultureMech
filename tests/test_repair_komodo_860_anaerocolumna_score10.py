...
SCRIPT = REPO / "scripts" / "repair_komodo_860_anaerocolumna_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_860_anaerocolumna")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_860")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_860").CHILDREN))
def test_children_link_to_dsmz_860_parent(repair_module, child) -> None:
...
def test_parent_is_promoted_and_lists_860_children(repair_module) -> None:
