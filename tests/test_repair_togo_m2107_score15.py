...
SCRIPT = REPO / "scripts" / "repair_togo_m2107_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2107_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2107")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2107",
            "term": {
...
def test_repair_keeps_meat_extract_and_casitone_opaque(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
...
def test_target_record_matches_togo_m2107_repair_contract(
    repair_module,
) -> None:
