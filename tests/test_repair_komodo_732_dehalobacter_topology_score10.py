...
SCRIPT = REPO / "scripts" / "repair_komodo_732_dehalobacter_topology_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_732_dehalobacter_topology")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_732")
...
def test_strain_variants_exit_review_ranking(repair_module, scorer_module) -> None:
    repaired = [
        ...
        if child.relationship == "STRAIN_SPECIFIC_VARIANT"
    ]
...
def test_parent_promotes_medium_732_root(repair_module) -> None:
...
def test_old_hub_becomes_child(repair_module) -> None:
    child = _child_by_path(repair_module)["data/normalized_yaml/bacterial/for_dsm_13726.yaml"]
...
def test_mediadive_732a_remains_source_duplicate(repair_module) -> None:
    child = _child_by_path(repair_module)[
        "data/normalized_yaml/bacterial/desulfuromonas_medium_tce.yaml"
    ]
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / child.path),
        child,
...
def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    child = repair_module.CHILDREN[-1]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["ingredients"][0]["preferred_term"] = "NaH2PO4 x H2O"
