...
SCRIPT = REPO / "scripts" / "repair_dsmz_1168_yps_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_dsmz_1168_yps_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_dsmz_1168_yps")
...
def _doc(repair_module, target) -> dict:
    return {
        ...
        "name": "yps_medium",
        "original_name": "YPS medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        "composition_type": "UNDEFINED",
        "physical_state": "SOLID_AGAR",
        "media_term": {
            "preferred_term": "DSMZ Medium 1168",
            "term": {
...
def test_repair_adds_missing_water_and_maps_complex_inputs(repair_module) -> None:
    ...
    assert "term" not in ingredients["Sea Salt"]


def test_repair_preserves_optional_agar(repair_module) -> None:
    target = repair_module.TARGETS[0]
    repaired = repair_module.repair_record(_doc(repair_module, target), target)
    agar = _by_name(repaired["ingredients"])["Agar"]

    assert agar["term"] == {"id": "CHEBI:2509", "label": "agar"}
    assert agar["notes"] == (
        "DSMZ Medium 1168 says agar may be added at 15 g/L to solidify the medium."
    )
    assert repaired["preparation_steps"] == [
        {
            "step_number": 1,
            "action": "MIX",
            "description": "Add 15 g/L agar to solidify DSMZ Medium 1168 when needed.",
        }
    ]
...
def test_repair_adds_flags_references_and_history(repair_module) -> None:
    ...
    assert repaired["references"] == [{"reference": repair_module.DSMZ_1168}]
    assert repaired["curation_history"] == [
        {
            ...
            "source": repair_module.DSMZ_1168,
...
def test_repair_rejects_component_drift(repair_module) -> None:
    target = repair_module.TARGETS[0]
    doc = _doc(repair_module, target)
    doc["ingredients"][0]["concentration"]["value"] = "4.1"
