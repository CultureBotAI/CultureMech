...
SCRIPT = REPO / "scripts" / "repair_nbrc_273_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_nbrc_273_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_nbrc_273")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1490",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "TOGO Medium M1490",
            },
        },
        "notes": "Source: NBRC - NBRC_M273",
...
def test_repair_corrects_water_volume_and_keeps_meat_extract_unmapped(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ...
    assert "term" not in ingredients["Meat extract"]
    assert "without an ontology grounding" in ingredients["Meat extract"]["notes"]
...
def test_repair_adds_preparation_references_flags_and_event_once(
    ...
    assert "retained Meat extract" in matching_events[0]["notes"]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M1489"
