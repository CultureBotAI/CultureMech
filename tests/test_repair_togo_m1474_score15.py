...
SCRIPT = REPO / "scripts" / "repair_togo_m1474_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1474_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1474")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1474",
...
def test_repair_corrects_formula_units(repair_module) -> None:
    ...
    assert ingredients["Bacto Blood Agar Base (Difco)"] == {
...
def test_repair_models_post_autoclave_blood_addition(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "Corrected imported blood and water amounts" in matching_events[0]["notes"]
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"][0]["preferred_term"] = "Blood"

    with pytest.raises(ValueError, match="solution signature drifted"):
        repair_module.repair_record(doc)


def test_target_record_matches_togo_m1474_repair_contract(
    repair_module,
) -> None:
