...
SCRIPT = REPO / "scripts" / "repair_komodo_3075_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_3075_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_3075")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 663)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 6.5,
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 3075 | "
            "DSMZ Medium: 3075 (mediadive.medium:3075)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 3075",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 663)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_3075_metabolite_table(repair_module, scorer_module) -> None:
    ...
    assert ingredients["FeCl3 x 6 H2O"]["concentration"] == {
        "value": "1.35",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["CuCl2 x 2 H2O"]["term"] == {
        "id": "CHEBI:86318",
        "label": "copper(II) chloride dihydrate",
    }
    assert ingredients["FeCl3 x 6 H2O"]["term"] == {
        "id": "CHEBI:86254",
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_3075}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:3070"
