...
SCRIPT = REPO / "scripts" / "repair_nbrc_274_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_nbrc_274_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_nbrc_274")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1491",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "TOGO Medium M1491",
            },
        },
        "notes": "Source: NBRC - NBRC_M274",
        "applications": ["Microbial cultivation"],
        "curation_history": [],
        "parent_media": {
            "path": "data/normalized_yaml/bacterial/TOGO_M1540_Enriched_Cytophaga_Agar.yaml",
            "relationship": "CONCENTRATION_VARIANT",
            "id": "CultureMech:008088",
            "name": "enriched_cytophaga_agar",
            "notes": "Reviewed source-local Enriched Cytophaga Agar concentration variant.",
        },
        "variant_relationship": "CONCENTRATION_VARIANT",
        "variant_modifications": [
            "Bacto Tryptone decreases from 2.0 to 0.5 g/L.",
            "Beef extract decreases from 0.5 to 0.2 g/L.",
        ],
    }
...
def test_repair_preserves_parent_variant_linkage(repair_module) -> None:
    doc = _doc(repair_module)
    repaired = repair_module.repair_record(doc)

    assert repaired["parent_media"] == doc["parent_media"]
    assert repaired["variant_relationship"] == "CONCENTRATION_VARIANT"
    assert repaired["variant_modifications"] == doc["variant_modifications"]


def test_repair_corrects_water_volume_and_keeps_opaque_components_unmapped(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ...
    assert "without an ontology grounding" in ingredients["Bacto Tryptone (Difco)"]["notes"]


def test_repair_grounds_machine_resolvable_formula_components(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
...
def test_repair_adds_preparation_references_flags_and_event_once(
    ...
    assert "retained Bacto Tryptone" in matching_events[0]["notes"]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M1490"
