...
SCRIPT = REPO / "scripts" / "repair_togo_m549_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m549_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m549")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M549",
            "term": {
...
def _parent_doc(repair_module) -> dict:
    return {
        ...
        "name": "desulfotomaculum_pbe_medium",
        "original_name": "DESULFOTOMACULUM PBE MEDIUM",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        "composition_type": "UNDEFINED",
        "physical_state": "LIQUID",
        "media_term": {
            "preferred_term": "JCM Medium J547",
            "term": {
                "id": repair_module.EXPECTED_PARENT_MEDIA_TERM,
                "label": "DESULFOTOMACULUM PBE MEDIUM",
            },
        },
        "notes": "Source: JCM",
        "ingredients": [
...
def test_repair_corrects_base_formula_units_and_n2(repair_module) -> None:
    ...
    assert "N2 as the gas atmosphere" in ingredients["N2"]["notes"]


def test_repair_expands_all_stock_solutions(repair_module) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
    solutions = _by_name(repaired["solutions"])
    lactate = _by_name(solutions["5% Sodium lactate solution"]["composition"])
    iron = _by_name(solutions["4% Fe(NH4)2(SO4)2 x 6H2O solution"]["composition"])
    ...
    assert lactate["Sodium lactate"]["concentration"] == {
        "value": "5.0",
        "unit": "PERCENT_W_V",
    }
    assert iron["Fe(NH4)2(SO4)2 x 6H2O"]["term"] == {
        "id": "CHEBI:76181",
        "label": "ferrous ammonium sulfate hexahydrate",
    }


def test_repair_preserves_foodon_micro_and_chebi_groundings(repair_module) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
    ...
    assert ingredients["Na2SO4"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:32149",
...
def test_repair_adds_preparation_sterilization_references_flags_and_event_once(
    repair_module,
    scorer_module,
) -> None:
    ...
    assert scorer_module.score_record(twice) == (5, ["no pH and no temperature"])
    ...
    assert "filter-sterilized 1% sodium ascorbate" in matching_events[0]["notes"]
...
def test_repair_record_rejects_wrong_id(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="expected CultureMech:009942"):
        repair_module.repair_target(doc)


def test_repair_record_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M458"

    with pytest.raises(ValueError, match="expected media term TOGO:M549"):
        repair_module.repair_target(doc)
...
def test_repair_parent_rejects_ingredient_drift(repair_module) -> None:
    ...
    doc["ingredients"][0] = _ingredient("Peptone", "2.0", "G_PER_L")
