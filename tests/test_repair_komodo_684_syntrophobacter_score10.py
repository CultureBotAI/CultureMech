...
SCRIPT = REPO / "scripts" / "repair_komodo_684_syntrophobacter_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_684_syntrophobacter")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_684")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_684").CHILDREN))
def test_children_link_to_dsmz_684_parent(repair_module, child) -> None:
...
def test_parent_is_promoted_and_lists_684_children(repair_module) -> None:
