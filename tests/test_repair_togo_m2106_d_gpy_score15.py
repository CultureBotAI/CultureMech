...
SCRIPT = REPO / "scripts" / "repair_togo_m2106_d_gpy_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2106_d_gpy_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2106")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2106",
...
def test_repair_corrects_seawater_unit_and_exits_review_ranking(
    repair_module,
    scorer_module,
) -> None:
...
def test_repair_grounds_yeast_glucose_and_agar(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
...
def test_repair_keeps_polypeptone_and_seawater_opaque(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert "term" not in ingredients["Polypeptone"]
    assert "term" not in ingredients["Seawater (1.75 % salinity)"]
...
def test_repair_adds_references_flags_and_event_once(repair_module) -> None:
    ...
    assert "Corrected the imported seawater g/L artifact" in matching_events[0]["notes"]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M2107"
...
def test_repair_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][1] = _ingredient("Seawater", "1", "G_PER_L")
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "Polypeptone solution",
...
def test_target_record_matches_togo_m2106_repair_contract(
    repair_module,
) -> None:
