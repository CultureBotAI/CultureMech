...
SCRIPT = REPO / "scripts" / "repair_komodo_505_formivibrio_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_505_formivibrio")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_505")
...
@pytest.mark.parametrize("child_name", ["for_dsm_5885", "for_dsm_6139", "for_dsm_6150"])
...
def test_old_hub_becomes_child(repair_module) -> None:
    child = next(child for child in repair_module.CHILDREN if child.name == "for_dsm_5885")
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / child.path),
        child,
    )
