...
SCRIPT = REPO / "scripts" / "repair_komodo_532_score35.py"
...
def _minimal_doc() -> dict:
    return {
        "id": "CultureMech:005910",
        "name": "methanogenium_sp_medium",
        "original_name": "METHANOGENIUM SP. medium",
        "category": "archaea",
        "medium_type": "COMPLEX",
        "composition_type": "UNDEFINED",
        "physical_state": "LIQUID",
        "media_term": {
            "preferred_term": "KOMODO Medium 532",
            "term": {"id": "komodo.medium:532", "label": "METHANOGENIUM SP. medium"},
        },
        "notes": (
            "Source: KOMODO ModelSEED | ID: 532 | DSMZ Medium: 532 "
            "(mediadive.medium:532) | Aerobic: No"
        ),
        "ingredients": [],
        "curation_history": [],
...
def test_repair_record_adds_komodo_methanogenium_sp_components() -> None:
    repair = _load_script(SCRIPT, "repair_komodo_532_score35")
...
def test_repair_record_preserves_532_specific_concentrations() -> None:
    repair = _load_script(SCRIPT, "repair_komodo_532_score35_values")
...
def test_repair_record_preserves_null_amount_components_without_concentrations() -> None:
    repair = _load_script(SCRIPT, "repair_komodo_532_score35_nulls")
...
def test_repair_record_grounds_explicit_hydrates_and_volatile_acids() -> None:
    repair = _load_script(SCRIPT, "repair_komodo_532_score35_hydrates")
...
def test_repair_record_scores_as_complete_and_is_idempotent() -> None:
    repair = _load_script(SCRIPT, "repair_komodo_532_score35_idempotent")
    scorer = _load_script(SCORER, "score_review_need_for_komodo_532")
...
def test_repair_record_rejects_wrong_id() -> None:
    repair = _load_script(SCRIPT, "repair_komodo_532_score35_wrong_id")
    doc = _minimal_doc()
    doc["id"] = "CultureMech:wrong"

    with pytest.raises(ValueError, match="expected immutable id CultureMech:005910"):
        repair.repair_record(doc)


def test_repair_record_rejects_wrong_source() -> None:
    repair = _load_script(SCRIPT, "repair_komodo_532_score35_wrong_source")
    doc = _minimal_doc()
    doc["media_term"]["term"]["id"] = "mediadive.medium:532"

    with pytest.raises(ValueError, match="missing expected media term komodo.medium:532"):
        repair.repair_record(doc)
