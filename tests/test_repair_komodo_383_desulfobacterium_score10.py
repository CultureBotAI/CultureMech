...
SCRIPT = REPO / "scripts" / "repair_komodo_383_desulfobacterium_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_383_desulfobacterium")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_383")
...
@pytest.mark.parametrize(
    "child",
    tuple(_load_script(SCRIPT, "komodo_383").STRAIN_CHILDREN),
)
def test_strain_pointers_exit_review_ranking(
    ...
    assert score <= 5
    assert all("identifies a strain" not in reason for reason in reasons)
    assert scorer_module.score_parsed([(str(child.path), repaired)]) == []


@pytest.mark.parametrize(
    "source_duplicate",
    tuple(_load_script(SCRIPT, "komodo_383_duplicate").SOURCE_DUPLICATES),
)
def test_named_duplicates_link_to_komodo_383_parent(
    repair_module,
    source_duplicate,
) -> None:
    repaired = repair_module.repair_source_duplicate(
        source_duplicate.path,
        _load_yaml(repair_module.NORMALIZED / source_duplicate.path),
    )
...
def test_parent_is_promoted_and_lists_exact_signature_children(repair_module) -> None:
...
def test_plan_repairs_targets_current_records(repair_module) -> None:
    ...
    for source_duplicate in repair_module.SOURCE_DUPLICATES:
        path = repair_module.NORMALIZED / source_duplicate.path
        expected[path] = repair_module.repair_source_duplicate(
            source_duplicate.path,
            _load_yaml(path),
        )
    for child in repair_module.STRAIN_CHILDREN:
...
def test_repair_rejects_source_duplicate_ingredient_drift(repair_module) -> None:
    source_duplicate = repair_module.SOURCE_DUPLICATES[0]
    doc = _load_yaml(repair_module.NORMALIZED / source_duplicate.path)
    doc["ingredients"][0]["preferred_term"] = "Glucose"

    with pytest.raises(ValueError, match="ingredient signature drifted"):
        repair_module.repair_source_duplicate(source_duplicate.path, doc)
