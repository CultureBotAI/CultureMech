...
SCRIPT = REPO / "scripts" / "repair_komodo_1069_ph_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1069_ph_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1069")
...
def test_variants_exit_review_ranking(repair_module, scorer_module) -> None:
...
def test_parent_updates_curated_variant_children(repair_module) -> None:
    ...
    exact_duplicate_paths = {
        "data/normalized_yaml/bacterial/for_dsm_18713_and_18859.yaml",
        "data/normalized_yaml/bacterial/for_dsm_19947_and_19784.yaml",
        "data/normalized_yaml/bacterial/for_dsm_21808_and_21809.yaml",
    }
    for path in exact_duplicate_paths:
        assert by_path[path] == next(
            entry for entry in before["variant_children"] if entry["path"] == path
        )
