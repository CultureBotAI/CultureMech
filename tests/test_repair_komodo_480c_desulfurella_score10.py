...
SCRIPT = REPO / "scripts" / "repair_komodo_480c_desulfurella_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_480c_desulfurella")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_480c")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_480c").CHILDREN))
...
def test_parent_is_promoted_and_lists_strain_children(repair_module) -> None:
