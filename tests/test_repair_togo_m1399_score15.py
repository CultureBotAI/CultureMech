...
SCRIPT = REPO / "scripts" / "repair_togo_m1399_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1399_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1399")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1399",
...
def test_repair_expands_trace_and_selenite_stocks(repair_module) -> None:
    ...
    assert trace["Nitrilotriacetic acid"]["term"] == {
        "id": "CHEBI:44557",
        "label": "nitrilotriacetic acid",
    }
    assert trace["KOH"]["concentration"] == {
        "value": "variable",
        "unit": "VARIABLE",
    }
    assert solutions["0.1% Na2SeO3 solution"]["composition"] == [
        {
            "preferred_term": "Na2SeO3",
            "concentration": {"value": "0.1", "unit": "PERCENT_W_V"},
            "source": repair_module.SOURCE,
            "notes": "TOGO M1399 / JCM Medium 1302 lists 0.1 % w/v Na2SeO3.",
...
def test_repair_expands_post_autoclave_stocks(repair_module) -> None:
    ...
    assert solutions["Trace vitamins"]["preparation_notes"] == (
        "Filter-sterilize before aseptic and anaerobic addition."
    )
    assert vitamins["Vitamin B12"]["concentration"] == {
        "value": "0.1",
        "unit": "MG_PER_L",
    }
    assert solutions["1 M Sodium thiosulfate solution"]["composition"] == [
        {
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "corrected the imported stock additions from g/L artifacts" in (
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["solutions"][1]["preferred_term"] = "Trace minerals"

    with pytest.raises(ValueError, match="solution signature drifted"):
        repair_module.repair_record(doc)


def test_target_record_matches_togo_m1399_repair_contract(
    repair_module,
) -> None:
