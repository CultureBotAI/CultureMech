...
SCRIPT = REPO / "scripts" / "repair_komodo_jcm_score10_exact_terms.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_jcm_score10_exact_terms")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_jcm_score10")
...
@pytest.mark.parametrize("target", tuple(_load_script(SCRIPT, "komodo_jcm").TARGET_SIGNATURES))
...
def test_unresolved_local_unmapped_names_remain_unmapped(repair_module) -> None:
    target = Path("bacterial/KOMODO_1076_SP4_MEDIUM.yaml")
    repaired = repair_module.repair_record(
        target,
        _load_yaml(repair_module.NORMALIZED / target),
    )
    ...
    for preferred_term in ("PPLO broth", "CMRL 1066"):
