...
SCRIPT = REPO / "scripts" / "repair_komodo_3067_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_3067_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_3067")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 1263)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 7.0,
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 3067 | "
            "DSMZ Medium: 3067 (mediadive.medium:3067)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 3067",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 1263)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_3067_metabolite_table(repair_module, scorer_module) -> None:
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["NaSeO3"]["term"] == {
        "id": "CHEBI:48843",
        "label": "disodium selenite",
    }
    assert ingredients["Fe(NH4)2(SO4)2 x 6 H2O"]["term"] == {
        "id": "CHEBI:76181",
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_3067}]
