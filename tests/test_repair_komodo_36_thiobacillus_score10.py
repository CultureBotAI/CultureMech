...
SCRIPT = REPO / "scripts" / "repair_komodo_36_thiobacillus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_36_thiobacillus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_36")
...
def test_ph_variant_exits_review_ranking(repair_module, scorer_module) -> None:
    child = _child_by_path(repair_module)["data/normalized_yaml/bacterial/for_dsm_18181.yaml"]
...
def test_parent_promotes_medium_36_root(repair_module) -> None:
...
def test_mediadive_36_remains_source_duplicate(repair_module) -> None:
    child = _child_by_path(repair_module)["data/normalized_yaml/bacterial/thiobacillus_medium.yaml"]
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / child.path),
        child,
    )
...
def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    child = repair_module.CHILDREN[-1]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["ingredients"][0]["preferred_term"] = "K2HPO4"
