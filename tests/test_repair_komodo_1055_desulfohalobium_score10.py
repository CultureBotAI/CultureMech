...
SCRIPT = REPO / "scripts" / "repair_komodo_1055_desulfohalobium_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1055_desulfohalobium")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1055")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_1055").CHILDREN))
def test_desulfohalobium_strain_variants_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_parent_preserves_dsmz_source_duplicate_and_lists_strain_children(
    repair_module,
) -> None:
    ...
    assert repaired["variant_children"] == [
        repair_module.SOURCE_DUPLICATE_CHILD,
