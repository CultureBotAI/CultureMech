...
SCRIPT = REPO / "scripts" / "repair_komodo_557_tissierella_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_557_tissierella")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_557")
...
@pytest.mark.parametrize("child_name", ["for_dsm_6876", "for_dsm_6877"])
def test_strain_variants_exit_review_ranking(
    repair_module,
    scorer_module,
    child_name: str,
) -> None:
...
def test_komodo_base_is_source_duplicate(repair_module) -> None:
    child = next(child for child in repair_module.CHILDREN if child.name == "creatinine_nmh_medium")
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / child.path),
        child,
    )
