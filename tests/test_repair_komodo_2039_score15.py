...
SCRIPT = REPO / "scripts" / "repair_komodo_2039_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2039_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2039")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 141)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 7.0,
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 2039 | "
            "DSMZ Medium: 2039 (mediadive.medium:2039)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2039",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 141)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2039_metabolite_table(repair_module, scorer_module) -> None:
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["KAl(SO4)2 x 12 H2O"]["term"] == {
        "id": "CHEBI:86465",
        "label": "potassium aluminium sulfate dodecahydrate",
    }
    assert ingredients["Na2SeO3 x 5 H2O"]["term"] == {
        "id": "CHEBI:131361",
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2039}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2030"
