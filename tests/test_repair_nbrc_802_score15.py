...
SCRIPT = REPO / "scripts" / "repair_nbrc_802_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_nbrc_802_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_nbrc_802")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1603",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "TOGO Medium M1603",
            },
        },
        "notes": "Source: NBRC - NBRC_M802",
...
def test_repair_moves_hipolypepton_and_grounds_resolvable_rows(
    repair_module,
) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Hipolypepton*"]["concentration"] == {
        "value": "10",
        "unit": "G_PER_L",
    }
    assert "term" not in ingredients["Hipolypepton*"]
    assert "FUJIFILM Wako Pure Chemical Corporation" in ingredients["Hipolypepton*"]["notes"]
...
def test_repair_adds_only_source_stated_preparation_steps(repair_module) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
...
def test_repair_relinks_target_as_omitted_component_variant(
    repair_module,
) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))

    assert repaired["parent_media"] == repair_module.PARENT_MEDIA
    assert repaired["variant_relationship"] == "OMITTED_COMPONENT_VARIANT"
...
def test_repair_adds_references_flags_and_event_once(repair_module) -> None:
    ...
    assert "omitted-component variant" in matching_events[0]["notes"]
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["solutions"][0] = _solution("Hipolypepton", "10", "G_PER_L")
