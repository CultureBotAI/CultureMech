...
SCRIPT = REPO / "scripts" / "repair_togo_m172_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m172_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m172")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M172",
            "term": {
...
def test_repair_represents_simple_stock_solutes(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["solutions"][0] = _solution("Resazurin solution", "0.5", "G_PER_L")

    with pytest.raises(ValueError, match="solution signatures drifted"):
        repair_module.repair_record(doc)


def test_target_record_matches_togo_m172_repair_contract(
    repair_module,
) -> None:
