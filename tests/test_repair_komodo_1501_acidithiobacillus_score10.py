...
SCRIPT = REPO / "scripts" / "repair_komodo_1501_acidithiobacillus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1501_acidithiobacillus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1501")
...
def test_strain_pointer_exits_review_ranking(repair_module, scorer_module) -> None:
    ...
    assert repaired["variant_modifications"] == [repair_module.CHILD_NOTE]
...
def test_parent_is_promoted_and_lists_1501_child(repair_module) -> None:
...
def test_child_loses_old_duplicate_children(repair_module) -> None:
    doc = _load_yaml(repair_module.NORMALIZED / repair_module.CHILD)
    doc["variant_children"] = [repair_module._child_entry()]

    repaired = repair_module.repair_child(doc)
...
def test_repair_rejects_child_ph_drift(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="expected pH 0.8"):
        repair_module.repair_child(doc)
