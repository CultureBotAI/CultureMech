...
SCRIPT = REPO / "scripts" / "repair_komodo_509_spirochaeta_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_509_spirochaeta")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_509")
...
def test_strain_variant_exits_review_ranking(repair_module, scorer_module) -> None:
    child = next(child for child in repair_module.CHILDREN if child.name == "for_dsm_6578")
...
def test_komodo_base_is_ph_variant(repair_module) -> None:
    child = next(
        child for child in repair_module.CHILDREN if child.name == "spirochete_thermophile_medium"
    )
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / child.path),
        child,
...
def test_plan_repairs_targets_current_records(repair_module) -> None:
    ...
    assert all("509a" not in str(path) for path in expected)
