...
SCRIPT = REPO / "scripts" / "repair_komodo_1051_tepidanaerobacter_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1051_tepidanaerobacter_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1051")
...
def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    doc = _load_yaml(repair_module.NORMALIZED / repair_module.CHILD.path)
    doc["ingredients"][0]["preferred_term"] = "MgCl2 x 6 H2O"
