...
SCRIPT = REPO / "scripts" / "repair_komodo_575_score30.py"
...
def _doc(repair) -> dict:
    return {
        ...
        "name": "anaerovibrio_burkinabensis_medium",
        "original_name": "ANAEROVIBRIO BURKINABENSIS medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            "preferred_term": "KOMODO Medium 575",
            "term": {
                "id": repair.EXPECTED_MEDIA_TERM,
                "label": "ANAEROVIBRIO BURKINABENSIS medium",
            },
        },
        "notes": "Source: KOMODO ModelSEED | ID: 575 | DSMZ Medium: 575",
        "ingredients": [],
        "curation_history": [],
...
@pytest.fixture
def repair():
    return _load_script(SCRIPT, "repair_komodo_575_score30")


@pytest.fixture
def scorer():
    return _load_script(SCORER, "score_review_need_for_komodo_575")


def test_repair_record_expands_anaerovibrio_burkinabensis_medium(
    repair,
    scorer,
) -> None:
    ...
    assert once["ph_range"] == {"min": 6.8, "max": 7.2}
    assert len(once["ingredients"]) == 34


def test_repair_record_applies_575_substrate_changes(repair) -> None:
...
def test_repair_record_adds_references_and_flags(repair) -> None:
    ...
    assert repaired["references"] == [
        {"reference": repair.KOMODO_575_URL},
        {"reference": repair.DSMZ_575_URL},
...
def test_repair_record_rejects_wrong_id(repair) -> None:
    ...
    with pytest.raises(ValueError, match="expected immutable id CultureMech:006022"):
...
def test_repair_record_rejects_wrong_source(repair) -> None:
    ...
    with pytest.raises(ValueError, match="missing expected media term komodo.medium:575"):
        repair.repair_record(doc)
