...
SCRIPT = REPO / "scripts" / "repair_togo_m3116_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m3116_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m3116")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M3116",
            "term": {
...
def _parent_doc(repair_module) -> dict:
    return {
        ...
        "variant_children": [
            {
                "path": "data/normalized_yaml/bacterial/TOGO_M2339_Nutrient_Agar.yaml",
                "relationship": "SOURCE_DUPLICATE",
                "id": "CultureMech:008926",
                "name": "nutrient_agar",
            },
            {
                "path": "data/normalized_yaml/bacterial/TOGO_M2340_Nutrient_Agar.yaml",
...
def test_repair_corrects_distilled_water_unit_and_adds_ph_value(
    repair_module,
) -> None:
...
def test_repair_keeps_meat_extract_unmapped(repair_module) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
...
def test_repair_adds_ph_step_without_inventing_sterilization(
    repair_module,
) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
...
def test_target_record_matches_togo_m3116_repair_contract(
    repair_module,
) -> None:
