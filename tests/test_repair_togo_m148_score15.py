...
SCRIPT = REPO / "scripts" / "repair_togo_m148_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m148_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m148")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M148",
...
def test_repair_keeps_complex_products_ungrounded(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Beef brain powder"] == {
        "preferred_term": "Beef brain powder",
        "concentration": {"value": "0.3", "unit": "G_PER_L"},
        "source": repair_module.SOURCE,
        "notes": (
    ...
    assert ingredients["Fluid thioglycolate (BD-BBL)"] == {
        "preferred_term": "Fluid thioglycolate (BD-BBL)",
...
def test_repair_grounds_defined_components(repair_module) -> None:
    ...
    assert ingredients["Sodium thioglycolate"] == {
        "preferred_term": "Sodium thioglycolate",
        "concentration": {"value": "1.3", "unit": "G_PER_L"},
        "source": repair_module.SOURCE,
        "notes": "TOGO M148 / MediaDive J157 lists 1.3 g/L Sodium thioglycolate.",
        "term": {
            "id": "CHEBI:86481",
...
def test_target_record_matches_togo_m148_repair_contract(
    repair_module,
) -> None:
