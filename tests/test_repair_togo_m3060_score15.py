...
SCRIPT = REPO / "scripts" / "repair_togo_m3060_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m3060_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m3060")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M3060",
            "term": {
...
def _parent_doc(repair_module) -> dict:
    return {
        ...
        "ph_value": 8.5,
        "media_term": {
            "preferred_term": "DSMZ Medium 467",
            "term": {
        ...
        "variant_children": [
            {
                "path": "data/normalized_yaml/bacterial/KOMODO_467_OTTOW_medium.yaml",
                "relationship": "SOURCE_DUPLICATE",
                "id": "CultureMech:005580",
                "name": "ottow_medium",
            },
            {
                "path": "data/normalized_yaml/bacterial/TOGO_M2316_Ottow_Medium.yaml",
                "relationship": "SOURCE_DUPLICATE",
                "id": "CultureMech:008903",
                "name": "ottow_medium",
            },
        ],
    }
...
def test_repair_corrects_distilled_water_and_adds_ph_value(repair_module) -> None:
...
def test_repair_grounds_all_nbrc_ottow_components(repair_module) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
    ...
    assert ingredients["Gellan Gum (if needed)"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:85248",
        "label": "gellan gum",
    }
    ...
    assert ingredients["Meat extract"]["term"] == {
        "id": "FOODON:03315424",
        "label": "Meat Extract",
    }
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "grounded all components" in matching_events[0]["notes"]
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "Gellan gum stock",
            "concentration": {"value": "7.0", "unit": "G_PER_L"},
            "composition": [],
        }
    ]
...
def test_target_record_matches_togo_m3060_repair_contract(
    repair_module,
) -> None:
