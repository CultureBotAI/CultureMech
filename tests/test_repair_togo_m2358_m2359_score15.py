...
SCRIPT = REPO / "scripts" / "repair_togo_m2358_m2359_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2358_m2359_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2358_m2359")
...
def _doc(target) -> dict:
    return {
        ...
        "original_name": "Reactivation With Liquid Medium 1",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            ...
            "term": {
                ...
                "label": "Reactivation With Liquid Medium 1",
            },
        },
        "notes": "Source: TOGO",
...
def test_repair_corrects_distilled_water_and_manganese_units(
    ...
    expected = (
        (
            ...
            repair_module.M2358_FINAL_INGREDIENT_SIGNATURE,
            False,
        ),
        (
            repair_module.TARGET_M2359,
            repair_module.M2359_FINAL_INGREDIENT_SIGNATURE,
            True,
        ),
    )
...
def test_repair_grounds_disclosed_unambiguous_components(
    ...
    repaired = repair_module.repair_target(
        _doc(repair_module.TARGET_M2359),
        repair_module.TARGET_M2359,
    )
    ingredients = _by_name(repaired["ingredients"])
...
def test_repair_keeps_meat_extract_unmapped(
    ...
    repaired = repair_module.repair_target(
        _doc(repair_module.TARGET_M2358),
        repair_module.TARGET_M2358,
    )
...
def test_repair_adds_ph_steps_without_inventing_sterilization(
    ...
    m2358 = repair_module.repair_target(
        _doc(repair_module.TARGET_M2358),
        repair_module.TARGET_M2358,
    )
    m2359 = repair_module.repair_target(
        _doc(repair_module.TARGET_M2359),
        repair_module.TARGET_M2359,
    )

    assert m2358["preparation_steps"] == [
        {
            "step_number": 1,
            "action": "ADJUST_PH",
    ...
    assert m2359["preparation_steps"] == [
        {
            "step_number": 1,
            "action": "ADJUST_PH",
            "description": "Adjust pH to 7.0.",
    ...
    assert "sterilization" not in m2358
    assert "sterilization" not in m2359
    assert "ph_range" not in m2358
    assert "ph_range" not in m2359
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    target = repair_module.TARGET_M2359
    ...
    [matching_m2358_event] = [
        event
        for event in m2358["curation_history"]
        if (
            event.get("curator") == repair_module.CURATOR
            and event.get("action") == repair_module.ACTION
        )
    ]
    assert "manganese unit artifacts" not in matching_m2358_event["notes"]
...
def test_repair_parent_adds_both_togo_children_once(repair_module) -> None:
    ...
    assert twice["variant_children"] == [
        ...
        repair_module._variant_child(repair_module.TARGET_M2358),
        repair_module._variant_child(repair_module.TARGET_M2359),
    ]
    matching_events = [
        event
        for event in twice["curation_history"]
...
def test_repair_record_rejects_wrong_id(repair_module) -> None:
    target = repair_module.TARGET_M2358
    doc = _doc(target)
