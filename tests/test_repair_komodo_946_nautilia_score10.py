...
SCRIPT = REPO / "scripts" / "repair_komodo_946_nautilia_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_946_nautilia")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_946")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_946").CHILDREN))
def test_nautilia_strain_pointers_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
