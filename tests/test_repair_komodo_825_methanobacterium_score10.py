...
SCRIPT = REPO / "scripts" / "repair_komodo_825_methanobacterium_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_825_methanobacterium")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_825")
...
@pytest.mark.parametrize(
    "child",
    tuple(_load_script(SCRIPT, "komodo_825").STRAIN_CHILDREN),
)
...
def test_named_duplicate_links_to_komodo_825_parent(repair_module) -> None:
    repaired = repair_module.repair_source_duplicate(
        _load_yaml(repair_module.NORMALIZED / repair_module.SOURCE_DUPLICATE.path)
    )

    assert repaired["parent_media"] == repair_module._parent_ref(
        "SOURCE_DUPLICATE",
        repair_module._duplicate_notes(),
    )
    assert repaired["variant_relationship"] == "SOURCE_DUPLICATE"
    assert repaired["variant_modifications"] == [repair_module._duplicate_notes()]
    assert "ingredients_curated" in repaired["data_quality_flags"]


def test_parent_lists_exact_signature_children(repair_module) -> None:
...
def test_plan_repairs_targets_current_records(repair_module) -> None:
    ...
    source_duplicate_path = repair_module.NORMALIZED / repair_module.SOURCE_DUPLICATE.path
    expected = {
        parent_path: repair_module.repair_parent(_load_yaml(parent_path)),
        source_duplicate_path: repair_module.repair_source_duplicate(
            _load_yaml(source_duplicate_path)
        ),
    }
...
def test_repair_rejects_source_duplicate_ingredient_drift(repair_module) -> None:
    doc = _load_yaml(repair_module.NORMALIZED / repair_module.SOURCE_DUPLICATE.path)
