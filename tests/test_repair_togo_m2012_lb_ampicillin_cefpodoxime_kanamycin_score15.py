...
SCRIPT = REPO / "scripts" / "repair_togo_m2012_lb_ampicillin_cefpodoxime_kanamycin_score15.py"
...
def repair_module():
    return _load_script(
        SCRIPT,
        "repair_togo_m2012_lb_ampicillin_cefpodoxime_kanamycin_score15",
    )


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2012")
...
def _target_doc(repair_module) -> dict:
    return {
        ...
        "original_name": "LB + Ampicillin, Cefpodoxime, Kanamycin medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2012",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "LB + Ampicillin, Cefpodoxime, Kanamycin medium",
            },
        },
...
def test_repair_expands_lb_antibiotic_stocks(
    ...
    ampicillin = solutions["Ampicillin solution (50 mg/ml)"]["composition"][0]
    cefpodoxime = solutions["Cefpodoxime solution (10 mg/ml)"]["composition"][0]
    kanamycin = solutions["Kanamycin solution (50 mg/ml)"]["composition"][0]
    ...
    assert ampicillin["concentration"] == {"value": "50.0", "unit": "MG_PER_ML"}
    assert ampicillin["term"] == {"id": "CHEBI:28971", "label": "ampicillin"}
    assert cefpodoxime["concentration"] == {"value": "10.0", "unit": "MG_PER_ML"}
    assert cefpodoxime["term"] == {"id": "CHEBI:3504", "label": "Cefpodoxime"}
    assert kanamycin["concentration"] == {"value": "50.0", "unit": "MG_PER_ML"}
...
def test_repair_adds_flags_references_and_event_once(repair_module) -> None:
    ...
    assert once["references"] == [
        {"reference": repair_module.TOGO_M2012},
        {"reference": repair_module.NBRC_1300},
    ]
    matching_events = [
        event
