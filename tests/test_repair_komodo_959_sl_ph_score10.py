...
SCRIPT = REPO / "scripts" / "repair_komodo_959_sl_ph_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_959_sl_ph_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_959")
...
def test_komodo_ph_variant_exits_review_ranking(
    repair_module,
    scorer_module,
) -> None:
    komodo_child = repair_module.CHILDREN[0]
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / komodo_child.path),
        komodo_child,
    )
    ...
    assert scorer_module.score_parsed([(str(komodo_child.path), repaired)]) == []


def test_parent_lists_ph_variant_children(repair_module) -> None:
...
def test_official_dsmz_child_is_linked_as_ph_variant(repair_module) -> None:
    official_child = repair_module.CHILDREN[1]
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / official_child.path),
        official_child,
    )

    assert repaired["parent_media"]["relationship"] == "PH_VARIANT"
    assert repaired["variant_relationship"] == "PH_VARIANT"
    assert repaired["variant_modifications"] == [official_child.modification]
