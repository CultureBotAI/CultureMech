...
SCRIPT = REPO / "scripts" / "repair_togo_m2316_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2316_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2316")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2316",
            "term": {
...
def test_repair_grounds_all_ottow_components(repair_module) -> None:
    repaired = repair_module.repair_target(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
    ...
    assert ingredients["Tap water"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:15377",
        "label": "water",
    }
...
def test_repair_record_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][0] = _ingredient("Yeast extract", "3", "G_PER_L")
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "Casamino acids stock",
...
def test_target_record_matches_togo_m2316_repair_contract(
    repair_module,
) -> None:
