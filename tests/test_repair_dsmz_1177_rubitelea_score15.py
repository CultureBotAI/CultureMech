...
SCRIPT = REPO / "scripts" / "repair_dsmz_1177_rubitelea_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_dsmz_1177_rubitelea_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_dsmz_1177_rubitelea")


def _doc(repair_module, target) -> dict:
    return {
        "id": target.expected_id,
        "name": "rubitelea_medium",
        "original_name": "RUBITELEA medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        "composition_type": "UNDEFINED",
        "physical_state": "SOLID_AGAR",
        "media_term": {
            "preferred_term": "DSMZ Medium 1177",
            "term": {
                "id": target.expected_source_term,
                "label": "RUBITELEA medium",
            },
        },
        "notes": "DSMZ 1177",
        "ingredients": [
...
def test_repair_corrects_seawater_and_maps_complex_inputs(repair_module) -> None:
...
def test_repair_preserves_starch_and_optional_agar(repair_module) -> None:
    target = repair_module.TARGETS[0]
    repaired = repair_module.repair_record(_doc(repair_module, target), target)
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Starch"]["term"] == {"id": "CHEBI:28017", "label": "starch"}
    assert ingredients["Agar"]["term"] == {"id": "CHEBI:2509", "label": "agar"}
    assert repaired["preparation_steps"] == [
        {
            "step_number": 1,
            "action": "MIX",
            "description": "Add 15 g/L agar to solidify DSMZ Medium 1177 when needed.",
        }
    ]
...
def test_repair_adds_flags_references_and_history(repair_module) -> None:
    ...
    assert repaired["references"] == [{"reference": repair_module.DSMZ_1177}]
    assert repaired["curation_history"] == [
        {
            ...
            "source": repair_module.DSMZ_1177,
...
def test_repair_rejects_component_drift(repair_module) -> None:
    target = repair_module.TARGETS[0]
    doc = _doc(repair_module, target)
    doc["ingredients"][0]["concentration"]["value"] = "10.1"
