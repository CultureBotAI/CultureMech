...
SCRIPT = REPO / "scripts" / "repair_komodo_78_dsm_5566_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_78_dsm_5566")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_78")
...
def test_parent_reclassifies_only_dsm_5566(repair_module) -> None:
    ...
    assert by_path.keys() == before_by_path.keys()
    assert by_path[child_path] == repair_module._child_entry()
    for path, entry in by_path.items():
        if path != child_path:
            assert entry == before_by_path[path]


def test_child_parent_edge_is_reclassified(repair_module) -> None:
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / repair_module.CHILD)
    )

    assert repaired["parent_media"] == repair_module._parent_ref()
    assert repaired["variant_modifications"] == [repair_module._notes()]
