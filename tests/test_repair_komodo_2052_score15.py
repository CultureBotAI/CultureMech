...
SCRIPT = REPO / "scripts" / "repair_komodo_2052_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2052_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2052")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 882)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 1.8,
        "notes": (
            "pH buffer: H2SO4 | Source: KOMODO ModelSEED | ID: 2052 | "
            "DSMZ Medium: 2052 (mediadive.medium:2052)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2052",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 882)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2052_metabolite_table(repair_module, scorer_module) -> None:
    ...
    assert ingredients["Na2MoO4"]["concentration"] == {
        "value": "0.01",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["MnCl2 x 2 H2O"]["term"] == {
        "id": "CHEBI:131395",
        "label": "manganese(II) chloride dihydrate",
    }
    assert ingredients["Na2MoO4"]["term"] == {
        "id": "CHEBI:75215",
        "label": "sodium molybdate (anhydrous)",
    }


def test_repair_preserves_h2so4_as_process_context(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    ph_step = repaired["preparation_steps"][1]["description"]
    assert "pH 1.8" in ph_step
    assert "H2SO4" in ph_step
    assert ingredients["H2SO4"]["source"] == repair_module.SOURCE
    assert "H2SO4" in repaired["curation_history"][-1]["notes"]
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2052}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2051"
