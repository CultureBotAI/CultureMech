...
SCRIPT = REPO / "scripts" / "repair_komodo_1197_rhodobacter_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1197_rhodobacter")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1197")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_1197").CHILDREN))
def test_rhodobacter_strain_variants_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_parent_lists_both_strain_children(repair_module) -> None:
