...
SCRIPT = REPO / "scripts" / "repair_komodo_3095_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_3095_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_3095")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace elements solution (medium 1019)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 3095 | "
            "DSMZ Medium: 3095 (mediadive.medium:3095)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 3095",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace elements solution (medium 1019)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_3095_metabolite_table(repair_module, scorer_module) -> None:
    ...
    assert ingredients["CoCl2 x 6 H2O"]["concentration"] == {
        "value": "0.17",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["FeSO4 x 7 H2O"]["term"] == {
        "id": "CHEBI:75836",
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_3095}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2065"
