...
SCRIPT = REPO / "scripts" / "repair_komodo_960_pelotomaculum_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_960_pelotomaculum")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_960")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_960").CHILDREN))
def test_children_are_changed_to_strain_variants(repair_module, child) -> None:
...
@pytest.mark.parametrize(
    "child_path",
    (
        "bacterial/for_dsm_14795.yaml",
        "bacterial/for_dsm_14947.yaml",
        "bacterial/for_dsm_16121.yaml",
        "bacterial/for_dsm_16145.yaml",
        "bacterial/for_dsm_17771.yaml",
        "bacterial/for_dsm_18709.yaml",
    ),
)
def test_single_strain_pointers_exit_review_ranking(
    repair_module,
    scorer_module,
    child_path: str,
) -> None:
...
def test_parent_keeps_dsmz_960_parent_and_lists_strain_children(repair_module) -> None:
    repaired = repair_module.repair_parent(
        _load_yaml(repair_module.NORMALIZED / repair_module.PARENT)
    )

    assert repaired["parent_media"]["id"] == repair_module.DSMZ_PARENT_ID
