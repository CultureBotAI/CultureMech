...
SCRIPT = REPO / "scripts" / "repair_komodo_395_desulfurococcus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_395_desulfurococcus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_395")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_395").CHILDREN))
def test_desulfurococcus_variants_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_parent_preserves_dsmz_source_duplicate_and_lists_children(
    repair_module,
) -> None:
