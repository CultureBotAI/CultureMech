...
SCRIPT = REPO / "scripts" / "repair_jcm_1314_gam_pyruvate_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_jcm_1314_gam_pyruvate_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_jcm_1314")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "ph_value": 7.0,
        "media_term": {
            "preferred_term": "JCM Medium J1314",
            "term": {
...
def test_repair_adds_missing_water_and_vendor_attribute(
    ...
    assert ingredients["Sodium pyruvate"]["concentration"] == {
        "value": "10.0",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_sodium_pyruvate_and_water_only(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Sodium pyruvate"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:50144",
    ...
    assert ingredients["GAM broth (Nissui)"] == {
        "preferred_term": "GAM broth (Nissui)",
        "concentration": {"value": "59.0", "unit": "G_PER_L"},
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice["references"] == [{"reference": repair_module.JCM_1314}]
    ...
    assert matching_events[0]["source"] == repair_module.JCM_1314
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "mediadive.medium:J1244"
...
def test_repair_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][0] = _ingredient("GAM broth (Nissui)", "59", "G_PER_L")
...
def test_target_record_matches_jcm_1314_repair_contract(repair_module) -> None:
