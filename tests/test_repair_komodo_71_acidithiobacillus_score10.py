...
SCRIPT = REPO / "scripts" / "repair_komodo_71_acidithiobacillus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_71_acidithiobacillus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_71")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_71").CHILDREN))
def test_children_link_to_dsmz_71_parent(repair_module, child) -> None:
...
def test_strain_pointer_exits_review_ranking(repair_module, scorer_module) -> None:
    child = repair_module.CHILD_BY_PATH[Path("bacterial/for_dsm_14366.yaml")]
    repaired = repair_module.repair_child(
        child.path,
        _load_yaml(repair_module.NORMALIZED / child.path),
    )
...
def test_parent_is_promoted_and_lists_71_children(repair_module) -> None:
