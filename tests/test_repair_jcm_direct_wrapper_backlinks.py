...
SCRIPT = REPO / "scripts" / "repair_jcm_direct_wrapper_backlinks.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_jcm_direct_wrapper_backlinks")


@pytest.fixture(scope="module")
def validator_module():
    return _load_script(VALIDATOR, "validate_media_variant_links_for_jcm_direct")
...
def test_existing_desulfovibrio_children_are_preserved(repair_module) -> None:
    before = _load_yaml(repair_module.NORMALIZED / repair_module.DESULFOVIBRIO.path)
    repaired = repair_module.repair_parent(repair_module.DESULFOVIBRIO, before)
    ...
    assert set(after_by_id) == set(before_by_id) | {repair_module.CHILDREN[2].record_id}
