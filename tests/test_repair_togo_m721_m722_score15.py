...
def test_togo_m721_links_to_jcm_source_duplicate(
    ...
    assert repaired["variant_modifications"] == [
        repair_module.M721_VARIANT_MODIFICATION
    ]
...
def test_repair_removes_gas_pseudo_ingredients(repair_module) -> None:
    ...
    descriptions = "\n".join(
        step["description"] for step in repaired["preparation_steps"]
    )
...
def test_repair_adds_references_flags_and_event_once(repair_module) -> None:
    target = next(
        ...
        if target.path == repair_module.TOGO_M722_PATH
    )
...
def test_repair_rejects_ingredient_drift(repair_module) -> None:
    target = next(
        target
        for target in repair_module.TARGETS
        if target.path == repair_module.TOGO_M721_PATH
    )
    doc = _doc(target)
    doc["ingredients"].pop()
