...
SCRIPT = REPO / "scripts" / "repair_komodo_487_peptoclostridium_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_487_peptoclostridium")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_487")
...
def test_strain_variant_exits_review_ranking(repair_module, scorer_module) -> None:
    child = _child_by_path(repair_module)["data/normalized_yaml/bacterial/for_dsm_5388.yaml"]
...
def test_komodo_base_becomes_source_duplicate(repair_module) -> None:
    child = _child_by_path(repair_module)[
        "data/normalized_yaml/bacterial/clostridium_m1_medium.yaml"
    ]
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / child.path),
        child,
