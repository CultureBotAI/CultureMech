...
SCRIPT = REPO / "scripts" / "repair_komodo_88_sulfolobus_ph_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_88_sulfolobus_ph_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_88")
...
def test_sulfolobus_ph_variant_exits_review_ranking(
    repair_module,
    scorer_module,
) -> None:
...
def test_child_preserves_source_duplicate_children(repair_module) -> None:
    before = _load_yaml(repair_module.NORMALIZED / repair_module.CHILD)
    repaired = repair_module.repair_child(before)
...
def test_repair_rejects_parent_ph_drift(repair_module) -> None:
    doc = _load_yaml(repair_module.NORMALIZED / repair_module.PARENT)
    doc["ph_value"] = 3.0

    with pytest.raises(ValueError, match="pH 2.0"):
        repair_module.repair_parent(doc)
