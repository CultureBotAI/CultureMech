...
SCRIPT = REPO / "scripts" / "repair_togo_m2215_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2215_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2215")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2215",
...
def test_repair_corrects_liquid_and_milligram_units(
    repair_module,
) -> None:
    ...
    assert ingredients["Ampicillin"]["concentration"] == {
        "value": "120.0",
        "unit": "MG_PER_L",
    }
...
def test_repair_grounds_discrete_components(repair_module) -> None:
    ...
    assert ingredients["Ampicillin"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:28971",
        "label": "ampicillin",
    }
...
def test_repair_keeps_disclosed_complex_inputs_opaque(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
    solutions = _by_name(repaired["solutions"])
    ...
    isovitalex = solutions["IsoVitaleX solution (Becton Dickinson)"]
    assert isovitalex["composition"] == []
    assert "term" not in isovitalex
...
def test_repair_represents_stock_solutions(repair_module) -> None:
    ...
    assert solutions["25% (w/v) Glucose"]["concentration"] == {
        "value": "10.0",
        "unit": "ML_PER_L",
    }
    assert solutions["IsoVitaleX solution (Becton Dickinson)"]["concentration"] == {
        "value": "5.0",
        "unit": "ML_PER_L",
    }


def test_repair_does_not_invent_conditions(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "moved the disclosed glucose" in matching_events[0]["notes"]
...
def test_repair_record_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][0] = _ingredient("Phenol red", "4", "G_PER_L")
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["solutions"][0] = _solution("IsoVitaleX", "5", "G_PER_L")

    with pytest.raises(ValueError, match="solution signature drifted"):
        repair_module.repair_record(doc)


def test_target_record_matches_togo_m2215_repair_contract(
    repair_module,
) -> None:
