...
SCRIPT = REPO / "scripts" / "repair_komodo_898_anoxybacillus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_898_anoxybacillus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_898")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_898").CHILDREN))
def test_anoxybacillus_variants_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_parent_lists_strain_ph_and_source_duplicate_children(
    repair_module,
) -> None:
