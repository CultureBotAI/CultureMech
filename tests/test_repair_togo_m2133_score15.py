...
SCRIPT = REPO / "scripts" / "repair_togo_m2133_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2133_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2133")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2133",
            "term": {
...
def test_repair_grounds_source_stated_discrete_components(repair_module) -> None:
    ...
    assert ingredients["ACES buffer"]["term"] == {"id": "CHEBI:39061", "label": "ACES"}
    assert ingredients["CO2 gas"]["term"] == {
        "id": "CHEBI:16526",
        "label": "carbon dioxide",
    }
    assert ingredients["Ferric pyrophosphate"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:132767",
...
def test_repair_keeps_ambiguous_components_opaque(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Charcoal activated"] == {
        "preferred_term": "Charcoal activated",
    ...
    assert ingredients["\u03b1-Ketoglutarate"] == {
        "preferred_term": "\u03b1-Ketoglutarate",
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "BCYE alpha stock",
...
def test_target_record_matches_togo_m2133_repair_contract(
    repair_module,
) -> None:
