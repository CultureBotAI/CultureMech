...
SCRIPT = REPO / "scripts" / "repair_togo_m1839_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1839_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1839")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1839",
            "term": {
...
def test_repair_grounds_solidifying_agent(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Bacto Agar"]["term"] == {"id": "CHEBI:2509", "label": "agar"}
    assert ingredients["Bacto Agar"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:2509",
        "label": "agar",
    }


def test_repair_grounds_bacto_tryptone_to_mim(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))

    assert _by_name(repaired["ingredients"])["Bacto Tryptone (Difco)"] == {
        ...
        "term": {"id": "MICRO:0000182", "label": "Tryptone"},
    }
...
def test_target_record_matches_togo_m1839_repair_contract(
    repair_module,
) -> None:
