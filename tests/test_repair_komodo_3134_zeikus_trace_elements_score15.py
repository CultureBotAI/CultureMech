...
SCRIPT = REPO / "scripts" / "repair_komodo_3134_zeikus_trace_elements_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_3134_zeikus_trace_elements_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_3134")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Zeikus trace elements solution (medium 1343)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 6.5,
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 3134 | "
            "DSMZ Medium: 3134 (mediadive.medium:3134) | Aerobic: No | "
            "SubMedium: Yes"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 3134",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Zeikus trace elements solution (medium 1343)",
            },
        },
        "ingredients": [
        ...
        "curation_history": [],
        "data_quality_flags": ["extracted_from_notes", "incomplete_composition"],
    }
...
def test_repair_expands_komodo_3134_to_zeikus_solution(repair_module, scorer_module) -> None:
    ...
    assert ingredients["Nitrilotriacetic acid"]["concentration"] == {
        "value": "12.5",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["Na2SeO3"]["term"] == {
        "id": "CHEBI:48843",
        "label": "disodium selenite",
    }
    assert ingredients["H3BO3"]["term"] == {
        "id": "CHEBI:33118",
        "label": "boric acid",
    }


def test_repair_preserves_koh_as_process_context(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    ph_step = repaired["preparation_steps"][0]["description"]
    assert "pH 6.5" in ph_step
    assert "KOH" in ph_step
    assert ingredients["KOH"]["source"] == repair_module.SOURCE
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2041"
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "Zeikus trace elements solution",
