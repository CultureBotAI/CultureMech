...
SCRIPT = REPO / "scripts" / "repair_togo_m1544_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1544_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1544")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1544",
...
def test_repair_moves_seawater_solution_to_opaque_base(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))

    assert "solutions" not in repaired
    assert _by_name(repaired["ingredients"])["Seawater*"] == {
...
def test_repair_grounds_only_discrete_components(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_does_not_invent_steps(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "Moved the imported Seawater stock solution" in matching_events[0]["notes"]
...
def test_target_record_matches_togo_m1544_repair_contract(
    repair_module,
) -> None:
