...
SCRIPT = REPO / "scripts" / "repair_komodo_846_aminomonas_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_846_aminomonas")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_846")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_846").CHILDREN))
def test_children_link_to_dsmz_846_parent(repair_module, child) -> None:
...
def test_for_dsm_12260_exits_review_ranking(repair_module, scorer_module) -> None:
    child = repair_module.CHILD_BY_PATH[Path("bacterial/for_dsm_12260.yaml")]
    repaired = repair_module.repair_child(
        child.path,
        _load_yaml(repair_module.NORMALIZED / child.path),
    )
...
def test_parent_is_promoted_and_lists_846_children(repair_module) -> None:
...
def test_repair_rejects_ingredient_drift(repair_module) -> None:
    ...
    doc["ingredients"][0]["concentration"]["value"] = "0.3"
