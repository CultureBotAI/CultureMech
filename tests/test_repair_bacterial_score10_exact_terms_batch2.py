...
SCRIPT = REPO / "scripts" / "repair_bacterial_score10_exact_terms_batch2.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_bacterial_score10_exact_terms_batch2")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_bacterial_score10_batch2")
...
def _doc(repair_module, target: Path) -> dict:
    return {
        ...
        "media_term": {
            ...
            "term": {"id": "mediadive.medium:test", "label": target.stem},
        },
        "notes": "Source: DSMZ",
        "ph_value": 7.0,
        "ingredients": [
...
@pytest.mark.parametrize(
    "target", tuple(_load_script(SCRIPT, "bacterial_batch2").TARGET_SIGNATURES)
)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            ("Defibrinated Blood", "Tryptic Digest of beef heart"),
        ),
        (
