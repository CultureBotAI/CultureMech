...
SCRIPT = REPO / "scripts" / "repair_togo_m1961_cyc_medium_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1961_cyc_medium_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1961")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1961",
...
def test_repair_corrects_formula_units_and_ph(
    repair_module,
    scorer_module,
) -> None:
...
def test_repair_keeps_commercial_mixtures_opaque(repair_module) -> None:
    ...
    assert "term" not in ingredients["Casamino acids, vitamin assay (Difco)"]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M1962"
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "Czapek-Dox liquid medium",
            "concentration": {"value": "33.4", "unit": "G_PER_L"},
            "composition": [],
        }
    ]
...
def test_target_record_matches_togo_m1961_repair_contract(
    repair_module,
) -> None:
