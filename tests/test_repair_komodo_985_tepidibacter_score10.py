...
SCRIPT = REPO / "scripts" / "repair_komodo_985_tepidibacter_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_985_tepidibacter")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_985")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_985").CHILDREN))
def test_tepidibacter_children_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_parent_is_promoted_and_lists_strain_and_duplicate_children(
    repair_module,
) -> None:
