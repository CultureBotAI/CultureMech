...
SCRIPT = REPO / "scripts" / "repair_komodo_924_methanocalculus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_924_methanocalculus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_924")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_924").CHILDREN))
def test_children_link_to_dsmz_924_parent(repair_module, child) -> None:
...
def test_parent_is_promoted_and_lists_924_children(repair_module) -> None:
