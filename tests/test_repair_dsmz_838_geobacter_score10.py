...
SCRIPT = REPO / "scripts" / "repair_dsmz_838_geobacter_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_dsmz_838_geobacter")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_dsmz_838")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "dsmz_838").CHILDREN))
def test_children_link_to_dsmz_838_parent(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_strain_wrappers_use_strain_specific_relationship(repair_module) -> None:
