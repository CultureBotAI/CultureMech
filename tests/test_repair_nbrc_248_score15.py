...
SCRIPT = REPO / "scripts" / "repair_nbrc_248_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_nbrc_248_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_nbrc_248")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1469",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "TOGO Medium M1469",
            },
        },
        "notes": "Source: NBRC - NBRC_M248",
...
def test_repair_moves_carbonate_to_final_ingredients(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ...
    assert "sterilized separately" in ingredients["Na2CO3"]["notes"]
...
def test_repair_grounds_all_formula_components(repair_module) -> None:
    ...
    assert {ingredient["preferred_term"] for ingredient in repaired["ingredients"]} == set(
        repair_module.GROUNDINGS
    )
...
def test_repair_adds_preparation_references_flags_and_event_once(
    ...
    assert "moved Na2CO3 from the false empty solution" in matching_events[0]["notes"]
