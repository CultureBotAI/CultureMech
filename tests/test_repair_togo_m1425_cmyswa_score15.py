...
SCRIPT = REPO / "scripts" / "repair_togo_m1425_cmyswa_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1425_cmyswa_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1425")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1425",
...
def test_repair_corrects_seawater_unit_and_adds_ph_range(
    ...
    assert repaired["ph_range"] == {"min": 7.0, "max": 7.5}
...
def test_repair_grounds_yeast_extract_and_agar(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
    ...
    assert ingredients["Seawater (2% salinity)"] == {
        "preferred_term": "Seawater (2% salinity)",
...
def test_repair_preserves_kg_microbe_match(repair_module) -> None:
    ...
    assert repaired["kg_microbe_match"] == "mediadive.medium:12"
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M1426"
...
def test_target_record_matches_togo_m1425_repair_contract(repair_module) -> None:
