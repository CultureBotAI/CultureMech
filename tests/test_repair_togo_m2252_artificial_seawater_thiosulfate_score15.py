...
SCRIPT = REPO / "scripts" / "repair_togo_m2252_artificial_seawater_thiosulfate_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2252_artificial_seawater")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2252")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2252",
            "term": {
...
def test_repair_corrects_artificial_seawater_and_o2_rows(
    repair_module,
) -> None:
...
def test_repair_adds_temperature_and_static_o2_gradient(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))

    assert repaired["temperature_value"] == 25.0
    assert repaired["aeration"] == "static O2 gradient"
...
def test_repair_keeps_artificial_seawater_unmapped(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_record_rejects_wrong_id(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="expected id CultureMech:008841"):
        repair_module.repair_record(doc)


def test_repair_record_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M2251"

    with pytest.raises(ValueError, match="expected media term TOGO:M2252"):
        repair_module.repair_record(doc)


def test_repair_record_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][0] = _ingredient("Distilled water", "1000.0", "ML_PER_L")

    with pytest.raises(ValueError, match="ingredient signature drifted"):
        repair_module.repair_record(doc)


def test_target_record_matches_togo_m2252_repair_contract(
    repair_module,
) -> None:
