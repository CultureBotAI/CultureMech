...
SCRIPT = REPO / "scripts" / "repair_komodo_843_helmutkoenigia_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_843_helmutkoenigia")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_843")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_843").CHILDREN))
def test_children_link_to_dsmz_843_parent(repair_module, child) -> None:
...
def test_strain_pointer_exits_review_ranking(repair_module, scorer_module) -> None:
    child = repair_module.CHILD_BY_PATH[Path("bacterial/for_dsm_13862.yaml")]
    repaired = repair_module.repair_child(
        child.path,
        _load_yaml(repair_module.NORMALIZED / child.path),
    )
...
def test_parent_is_promoted_and_lists_843_children(repair_module) -> None:
