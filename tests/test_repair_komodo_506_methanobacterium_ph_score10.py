...
SCRIPT = REPO / "scripts" / "repair_komodo_506_methanobacterium_ph_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_506_methanobacterium")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_506")
...
def test_ph_variant_exits_review_ranking(repair_module, scorer_module) -> None:
    ...
    assert repaired["parent_media"]["path"].startswith("data/normalized_yaml/archaea/")
...
def test_parent_preserves_dsmz_duplicate_and_lists_ph_child(repair_module) -> None:
