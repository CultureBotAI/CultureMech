...
SCRIPT = REPO / "scripts" / "repair_dsmz_664a_pseudothermotoga_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_dsmz_664a_pseudothermotoga")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_dsmz_664a")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "dsmz_664a").CHILDREN))
def test_children_link_to_dsmz_664a_parent(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_strain_wrapper_uses_strain_specific_relationship(repair_module) -> None:
    child = repair_module.CHILD_BY_PATH[Path("bacterial/for_dsm_14385.yaml")]
    repaired = repair_module.repair_child(
        child.path,
        _load_yaml(repair_module.NORMALIZED / child.path),
    )
