...
SCRIPT = REPO / "scripts" / "repair_komodo_2073_2075_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2073_2075_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2073_2075")
...
def test_repair_expands_komodo_2073_record(repair_module, scorer_module) -> None:
    repaired = repair_module.repair_record(
        repair_module.FERGUSON_TARGET,
        _doc(repair_module, repair_module.FERGUSON_TARGET),
    )
    ingredients = _by_name(repaired["ingredients"])

    assert repair_module._signature(repaired["ingredients"]) == (
        repair_module.FINAL_INGREDIENT_SIGNATURES[repair_module.FERGUSON_TARGET]
    )
    assert repaired["record_kind"] == "SOLUTION"
    assert repaired["ph_value"] == 3.0
    assert "Na2MoO4 x 2 H2O" in ingredients
    assert "NaMoO4 x 2 H2O" in ingredients["Na2MoO4 x 2 H2O"]["notes"]
    assert scorer_module.score_record(repaired) == (0, [])
    assert scorer_module.score_parsed([(str(repair_module.FERGUSON_TARGET), repaired)]) == []


def test_repair_expands_empty_komodo_2074_record(repair_module, scorer_module) -> None:
    repaired = repair_module.repair_record(
        repair_module.MEDIUM_1000_TARGET,
        _doc(repair_module, repair_module.MEDIUM_1000_TARGET),
    )
    ingredients = _by_name(repaired["ingredients"])

    assert repair_module._signature(repaired["ingredients"]) == (
        repair_module.FINAL_INGREDIENT_SIGNATURES[repair_module.MEDIUM_1000_TARGET]
    )
    assert "ph_value" not in repaired
    assert ingredients["MnSO4 x 2 H2O"]["term"] == {
        "id": "CHEBI:86356",
        "label": "manganese(II) sulfate dihydrate",
    }
    assert scorer_module.score_record(repaired) == (5, ["no pH and no temperature"])
    assert scorer_module.score_parsed([(str(repair_module.MEDIUM_1000_TARGET), repaired)]) == []


def test_repair_expands_komodo_2075_record(repair_module, scorer_module) -> None:
    repaired = repair_module.repair_record(
        repair_module.MEDIUM_1003_TARGET,
        _doc(repair_module, repair_module.MEDIUM_1003_TARGET),
    )
    ingredients = _by_name(repaired["ingredients"])

    assert repair_module._signature(repaired["ingredients"]) == (
        repair_module.FINAL_INGREDIENT_SIGNATURES[repair_module.MEDIUM_1003_TARGET]
    )
    assert repaired["ph_value"] == 3.0
    assert ingredients["CuCl"]["term"] == {"id": "CHEBI:53472", "label": "copper(I) chloride"}
    assert "Se-acid" in ingredients["Selenic acid"]["notes"]
    assert "HBO3" in ingredients["H3BO3"]["notes"]
    assert "Ni2SO4 x 6 H2O" in ingredients["NiSO4 x 6 H2O"]["notes"]
    assert scorer_module.score_record(repaired) == (0, [])
    assert scorer_module.score_parsed([(str(repair_module.MEDIUM_1003_TARGET), repaired)]) == []
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    twice = repair_module.repair_record(repair_module.FERGUSON_TARGET, once)

    assert twice == once
    assert once["references"] == [
        {"reference": repair_module.REFERENCES[repair_module.FERGUSON_TARGET]}
    ]
...
def test_repair_rejects_wrong_id(repair_module) -> None:
    doc = _doc(repair_module, repair_module.FERGUSON_TARGET)
    doc["id"] = "CultureMech:wrong"

    with pytest.raises(ValueError, match="CultureMech:004350"):
        repair_module.repair_record(repair_module.FERGUSON_TARGET, doc)
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="komodo.medium:2073"):
        repair_module.repair_record(repair_module.FERGUSON_TARGET, doc)
