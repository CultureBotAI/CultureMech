...
SCRIPT = REPO / "scripts" / "repair_togo_m1419_cmswa_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1419_cmswa_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1419")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1419",
...
def test_repair_grounds_agar_only(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M1420"
...
def test_target_record_matches_togo_m1419_repair_contract(repair_module) -> None:
