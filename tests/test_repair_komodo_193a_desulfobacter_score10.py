...
SCRIPT = REPO / "scripts" / "repair_komodo_193a_desulfobacter_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_193a_desulfobacter")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_193a")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_193a").CHILDREN))
def test_children_link_to_komodo_193a_parent(repair_module, child) -> None:
...
@pytest.mark.parametrize(
    "child_path",
    (
        "bacterial/for_dsm_14956.yaml",
        "bacterial/for_dsm_15102.yaml",
        "bacterial/for_dsm_15249.yaml",
        "bacterial/for_dsm_16697.yaml",
        "bacterial/for_dsm_18843.yaml",
    ),
)
...
def test_parent_keeps_dsmz_193a_parent_and_lists_children(repair_module) -> None:
