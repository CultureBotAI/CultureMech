...
SCRIPT = REPO / "scripts" / "repair_komodo_120_methanosarcina_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_120_methanosarcina")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_120")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_120").CHILDREN))
def test_children_link_to_komodo_120_parent(repair_module, child) -> None:
...
@pytest.mark.parametrize(
    "child_path",
    (
        "bacterial/for_dsm_21571.yaml",
        "bacterial/for_dsm_9195.yaml",
    ),
)
...
def test_parent_lists_real_child_paths(repair_module) -> None:
