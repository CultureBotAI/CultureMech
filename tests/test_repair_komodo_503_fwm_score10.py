...
SCRIPT = REPO / "scripts" / "repair_komodo_503_fwm_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_503_fwm")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_503")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_503").CHILDREN))
def test_children_link_to_komodo_503_parent(repair_module, child) -> None:
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_503_score").CHILDREN))
...
def test_parent_lists_komodo_503_children(repair_module) -> None:
...
def test_503_14_is_h2so4_supplemented_variant(repair_module, scorer_module) -> None:
    child = repair_module.CHILD_BY_PATH[Path("bacterial/for_dsm_16082.yaml")]
    repaired = repair_module.repair_child(
        child.path,
        _load_yaml(repair_module.NORMALIZED / child.path),
    )
...
def test_repair_rejects_solution_component_drift(repair_module) -> None:
    doc = _load_yaml(repair_module.NORMALIZED / repair_module.PARENT)
    doc["solutions"][0]["composition"][0]["concentration"]["value"] = "2.0"

    with pytest.raises(ValueError, match="solution component signature drifted"):
        repair_module.repair_parent(doc)


def test_repair_rejects_solution_volume_evidence_drift(repair_module) -> None:
    child = repair_module.CHILDREN[0]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["solutions"][0]["concentration_candidates"][0]["value"] = "2"

    with pytest.raises(ValueError, match="solution volume evidence drifted"):
        repair_module.repair_child(child.path, doc)
