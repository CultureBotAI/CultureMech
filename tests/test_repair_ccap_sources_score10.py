...
SCRIPT = REPO / "scripts" / "repair_ccap_sources_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_ccap_sources_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_ccap_sources")


def _doc() -> dict:
    pdf_url = "https://www.ccap.ac.uk/wp-content/uploads/MR_Test.pdf"
    return {
        "id": "CultureMech:test",
        "name": "test_medium",
        "category": "algae",
...
def test_repair_adds_structured_ccap_source(repair_module, scorer_module) -> None:
...
def test_plan_repairs_finds_current_ccap_cohort(repair_module) -> None:
...
def test_repair_rejects_mismatched_pdf(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="does not match ccap-import"):
        repair_module.repair_record(Path("algae/test_medium.yaml"), doc)


def test_repair_rejects_missing_ccap_import(repair_module) -> None:
    doc = _doc()
    doc["curation_history"] = []

    with pytest.raises(ValueError, match="expected one ccap-import"):
        repair_module.repair_record(Path("algae/test_medium.yaml"), doc)
