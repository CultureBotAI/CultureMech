...
SCRIPT = REPO / "scripts" / "repair_togo_m1738_marine_methanol_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1738_marine_methanol_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1738")
...
def _target_doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Marine Broth with 0.5% Methanol",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1738",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Marine Broth with 0.5% Methanol",
            },
        },
...
def test_repair_restores_nbrc_formula_and_leaves_queue(
    repair_module,
    scorer_module,
) -> None:
    ...
    assert ingredients["Bacto Marine Broth 2216 (Difco)"]["concentration"] == {
        "value": "37.4",
        "unit": "G_PER_L",
    }
    assert "term" not in ingredients["Bacto Marine Broth 2216 (Difco)"]
    assert ingredients["Methanol"]["concentration"] == {
        "value": "5",
        "unit": "ML_PER_L",
    }
    assert ingredients["Methanol"]["term"] == {
        "id": "CHEBI:17790",
        "label": "methanol",
    }
    ...
    assert score < 15
    assert all("only" not in reason for reason in reasons)
...
def test_repair_adds_flags_references_and_event_once(repair_module) -> None:
    ...
    assert once["references"] == [
        {"reference": repair_module.TOGO_M1738},
        {"reference": repair_module.NBRC_947},
    ]
    matching_events = [
        event
