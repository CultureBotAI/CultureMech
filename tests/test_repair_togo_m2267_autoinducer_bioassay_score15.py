...
SCRIPT = REPO / "scripts" / "repair_togo_m2267_autoinducer_bioassay_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2267_autoinducer_bioassay")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2267")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2267",
            "term": {
...
def test_repair_expands_base_and_stock_wrappers(repair_module) -> None:
    ...
    assert solutions["Potassium phosphate buffer"]["concentration"] == {
        "value": "10.0",
        "unit": "ML_PER_L",
    }


def test_repair_preserves_phosphate_ambiguity_and_grounds_stocks(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    solutions = _by_name(repaired["solutions"])
    naoh = _by_name(solutions["3 N NaOH"]["composition"])
    arginine = _by_name(solutions["L-Arginine solution"]["composition"])

    phosphate = solutions["Potassium phosphate buffer"]
    assert "term" not in phosphate
    assert "mediaingredientmech_chebi_term" not in phosphate
    assert "ratio is not disclosed" in phosphate["notes"]
    assert naoh["NaOH"]["concentration"] == {"value": "3.0", "unit": "MOLAR"}
    assert naoh["NaOH"]["term"] == {
        "id": "CHEBI:32145",
        "label": "sodium hydroxide",
    }
    assert arginine["L-Arginine"]["term"] == {
        "id": "CHEBI:16467",
        "label": "L-arginine",
    }
    assert solutions["Glycerol"]["term"] == {
        "id": "CHEBI:17754",
        "label": "glycerol",
    }
...
def test_repair_record_rejects_wrong_id(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="expected id CultureMech:008854"):
        repair_module.repair_record(doc)


def test_repair_record_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "TOGO:M2266"

    with pytest.raises(ValueError, match="expected media term TOGO:M2267"):
        repair_module.repair_record(doc)


def test_repair_record_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][0] = _ingredient("Distilled water", "970", "ML_PER_L")
...
def test_target_record_matches_togo_m2267_repair_contract(
    repair_module,
) -> None:
