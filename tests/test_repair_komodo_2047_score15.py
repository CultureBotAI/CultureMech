...
SCRIPT = REPO / "scripts" / "repair_komodo_2047_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2047_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2047")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 574)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 6.0,
        "notes": (
            "pH buffer: KOH | Source: KOMODO ModelSEED | ID: 2047 | "
            "DSMZ Medium: 2047 (mediadive.medium:2047)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2047",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 574)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2047_metabolite_table(repair_module, scorer_module) -> None:
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2047}]
