...
SCRIPT = REPO / "scripts" / "repair_togo_m2099_lb_kanamycin_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2099_lb_kanamycin_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2099")
...
def _target_doc(repair_module) -> dict:
    return {
        ...
        "original_name": "LB + 50 ug/ml Kanamycin medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2099",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "LB + 50 ug/ml Kanamycin medium",
            },
        },
        ...
        "parent_media": {
            ...
            "id": repair_module.STALE_PARENT_ID,
...
def test_repair_expands_lb_and_kanamycin_stock(
    repair_module,
    scorer_module,
) -> None:
    ...
    kanamycin = solutions["Kanamycin sulfate solution (50 mg/ml)"]["composition"][0]
    ...
    assert "term" not in ingredients["Bacto Tyrptone (Difco)"]
    assert kanamycin["concentration"] == {"value": "50.0", "unit": "MG_PER_ML"}
    assert kanamycin["term"] == {
        "id": "CHEBI:6109",
...
def test_repair_updates_variant_relationships_and_is_idempotent(repair_module) -> None:
    ...
    stale_parent = repair_module.repair_stale_parent(
        {
            "id": repair_module.STALE_PARENT_ID,
            "variant_children": [
                repair_module.VARIANT_CHILD,
...
def test_repair_adds_flags_references_and_event_once(repair_module) -> None:
    ...
    assert once["references"] == [
        {"reference": repair_module.TOGO_M2099},
        {"reference": repair_module.NBRC_1415},
    ]
    matching_events = [
        event
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    doc = _target_doc(repair_module)
    doc["solutions"][0]["preferred_term"] = "Kanamycin solution"
