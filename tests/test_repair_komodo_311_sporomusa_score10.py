...
SCRIPT = REPO / "scripts" / "repair_komodo_311_sporomusa_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_311_sporomusa")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_311")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_311").CHILDREN))
def test_children_link_to_komodo_311_parent(repair_module, child) -> None:
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_311_score").CHILDREN))
...
def test_parent_lists_komodo_311_children(repair_module) -> None:
...
def test_repair_rejects_komodo_311a_child(repair_module) -> None:
    path = Path("bacterial/for_dsm_11379_dsm_11380_dsm_11381_and_dsm_11382.yaml")
