...
SCRIPT = REPO / "scripts" / "repair_togo_m217_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m217_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m217")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M217",
...
def test_repair_corrects_seawater_unit_and_adds_ph_range(
    ...
    assert _by_name(repaired["ingredients"])["Seawater"]["concentration"] == {
        "value": "1.0",
        "unit": "L",
    }
...
def test_repair_adds_jcm_ph_and_default_autoclave(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_record_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][0] = _ingredient("Sea water", "1", "G_PER_L")
...
def test_target_record_matches_togo_m217_repair_contract(
    repair_module,
) -> None:
