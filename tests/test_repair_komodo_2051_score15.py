...
SCRIPT = REPO / "scripts" / "repair_komodo_2051_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2051_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2051")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 796)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 1.0,
        "notes": (
            "pH buffer: HCl | Source: KOMODO ModelSEED | ID: 2051 | "
            "DSMZ Medium: 2051 (mediadive.medium:2051)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2051",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 796)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2051_metabolite_table(repair_module, scorer_module) -> None:
...
def test_repair_grounds_all_disclosed_components(repair_module) -> None:
    ...
    assert ingredients["Na2WO4 x 2 H2O"]["term"] == {
        "id": "CHEBI:63939",
        "label": "sodium tungstate dihydrate",
    }
    assert ingredients["(NH4)2Ni(SO4)2 x 6 H2O"]["term"] == {
        "id": "CHEBI:86149",
        "label": "ammonium nickel sulfate hexahydrate",
    }
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2051}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2049"
