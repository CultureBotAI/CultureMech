...
SCRIPT = REPO / "scripts" / "repair_komodo_820_16960_score30.py"
...
def _doc(repair) -> dict:
    return {
        ...
        "name": "medium_820_modified_for_dsm_16960",
        "original_name": "MEDIUM 820 MODIFIED FOR DSM 16960",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "ph_value": 7.8,
        "media_term": {
            "preferred_term": "KOMODO Medium 820_16960",
            "term": {
                "id": repair.EXPECTED_MEDIA_TERM,
                "label": "MEDIUM 820 MODIFIED FOR DSM 16960",
            },
        },
        "notes": "Source: KOMODO ModelSEED | ID: 820_16960 | DSMZ Medium: 820",
        "ingredients": [],
        "curation_history": [],
...
@pytest.fixture
def repair():
    return _load_script(SCRIPT, "repair_komodo_820_16960_score30")


@pytest.fixture
def scorer():
    return _load_script(SCORER, "score_review_need_for_komodo_820_16960")


def test_repair_record_expands_820_variant_for_dsm_16960(
    repair,
    scorer,
) -> None:
    ...
    assert "ph_range" not in once
    assert len(once["ingredients"]) == 18


def test_repair_record_uses_dsmz_820_components(repair) -> None:
    repaired = repair.repair_record(_doc(repair))

    assert _ingredient(repaired, "Na2S2O3 x 5 H2O")["concentration"] == {
        "value": "1.000000",
        "unit": "G_PER_L",
    }
...
def test_repair_record_adds_references_and_flags(repair) -> None:
    ...
    assert repaired["references"] == [
        {"reference": repair.KOMODO_820_16960_URL},
...
def test_repair_record_rejects_wrong_id(repair) -> None:
    ...
    with pytest.raises(ValueError, match="expected immutable id CultureMech:006546"):
        repair.repair_record(doc)


def test_repair_record_rejects_wrong_source(repair) -> None:
    doc = _doc(repair)
    doc["media_term"]["term"]["id"] = "komodo.medium:820"

    with pytest.raises(
        ValueError,
        match="missing expected media term komodo.medium:820_16960",
    ):
        repair.repair_record(doc)
