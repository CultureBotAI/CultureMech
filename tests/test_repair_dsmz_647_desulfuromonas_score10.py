...
SCRIPT = REPO / "scripts" / "repair_dsmz_647_desulfuromonas_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_dsmz_647_desulfuromonas")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_dsmz_647")
...
@pytest.mark.parametrize(
    "child",
    tuple(_load_script(SCRIPT, "dsmz_647_desulfuromonas").CHILDREN),
)
def test_children_link_to_dsmz_647_parent(
    repair_module,
    scorer_module,
    child,
) -> None:
...
def test_parent_is_promoted_and_lists_exact_children(repair_module) -> None:
...
def test_dsm_8988_uses_strain_specific_relationship(repair_module) -> None:
    repaired = repair_module.plan_repairs()
    strain_child = repair_module.CHILD_BY_PATH[Path("bacterial/for_dsm_8988.yaml")]
    path = repair_module.NORMALIZED / strain_child.path

    assert repaired[path]["parent_media"]["relationship"] == "STRAIN_SPECIFIC_VARIANT"
    assert repaired[path]["variant_relationship"] == "STRAIN_SPECIFIC_VARIANT"
