...
SCRIPT = REPO / "scripts" / "repair_komodo_133_carbon_monoxide_topology_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_133_carbon_monoxide_topology")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_133")
...
def test_strain_variants_exit_review_ranking(repair_module, scorer_module) -> None:
    strain_paths = {
        "bacterial/for_dsm_1083.yaml",
        "bacterial/for_dsm_1085.yaml",
        "bacterial/for_dsm_13294.yaml",
    }
    ...
    repaired = [
        ...
        if str(child.path) in strain_paths
    ]
...
def test_parent_promotes_medium_133_root(repair_module) -> None:
...
def test_old_hub_becomes_child(repair_module) -> None:
    child = _child_by_path(repair_module)["data/normalized_yaml/bacterial/for_dsm_1083.yaml"]
...
def test_non_dsm_rows_remain_source_duplicates(repair_module) -> None:
    source_duplicate_paths = {
        "data/normalized_yaml/bacterial/carbon_monoxide_oxidizer_medium.yaml",
        "data/normalized_yaml/bacterial/for_chemoautotrophic_growth.yaml",
        "data/normalized_yaml/bacterial/for_chemoorganotrophic_growth.yaml",
    }
    children = _child_by_path(repair_module)

    for path in source_duplicate_paths:
        child = children[path]
        repaired = repair_module.repair_child(
            _load_yaml(repair_module.NORMALIZED / child.path),
            child,
