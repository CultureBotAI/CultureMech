...
SCRIPT = REPO / "scripts" / "repair_bacterial_score10_exact_terms_batch6.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_bacterial_score10_exact_terms_batch6")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_bacterial_score10_batch6")
...
@pytest.mark.parametrize("target", tuple(_load_script(SCRIPT, "bacterial_batch6").TARGETS))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            Path("bacterial/nutriment_agar.yaml"),
            ("Meat extract",),
        ),
        (
            Path("bacterial/nutriment_agar_with_150_g_l_nacl.yaml"),
            ("Meat extract",),
        ),
        (
