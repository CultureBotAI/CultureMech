...
SCRIPT = REPO / "scripts" / "repair_komodo_1058_thiohalophilus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1058_thiohalophilus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1058")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_1058").CHILDREN))
def test_children_link_to_komodo_1058_parent(repair_module, child) -> None:
...
@pytest.mark.parametrize(
    "child_path",
    (
        "bacterial/for_dsm_15074.yaml",
        "bacterial/for_dsm_15699.yaml",
        "bacterial/for_dsm_21152.yaml",
    ),
)
...
def test_parent_is_promoted_and_lists_1058_children(repair_module) -> None:
...
def test_repair_rejects_composition_drift(repair_module) -> None:
