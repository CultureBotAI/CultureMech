...
SCRIPT = REPO / "scripts" / "repair_komodo_769_clostridium_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_769_clostridium")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_769")
...
def test_dsm_5262_exits_review_ranking(repair_module, scorer_module) -> None:
    ...
    assert repaired["variant_modifications"] == [repair_module.CHILD.notes]
...
def test_parent_preserves_dsmz_parent_and_lists_dsm_5262(repair_module) -> None:
    repaired = repair_module.repair_parent(
        _load_yaml(repair_module.NORMALIZED / repair_module.PARENT)
    )

    assert repaired["parent_media"]["id"] == "CultureMech:001902"
...
def test_plan_repairs_targets_current_records(repair_module) -> None:
    ...
    child_path = repair_module.NORMALIZED / repair_module.CHILD.path
...
def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    ...
    doc["ingredients"][0]["concentration"]["value"] = "12"
