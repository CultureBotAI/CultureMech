...
SCRIPT = REPO / "scripts" / "repair_togo_m1593_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1593_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1593")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1593",
...
def test_repair_keeps_commercial_mixtures_opaque(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert "term" not in ingredients["Czapek-Dox Liquid Medium, Modified (OXOID)"]
    assert "term" not in ingredients["Bacto Vitamin Assay Casamino Acids (Difco)"]


def test_repair_adds_only_source_stated_ph_adjustment(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ...
    doc["solutions"] = [
        {
            ...
            "concentration": {"value": "16", "unit": "G_PER_L"},
            "composition": [],
        }
    ]
...
def test_target_record_matches_togo_m1593_repair_contract(
    repair_module,
) -> None:
