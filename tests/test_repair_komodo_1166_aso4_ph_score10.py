...
SCRIPT = REPO / "scripts" / "repair_komodo_1166_aso4_ph_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1166_aso4_ph_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1166")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_1166").CHILDREN))
def test_aso4_children_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
    ...
    assert repaired["variant_modifications"] == [child.modification]
...
def test_parent_preserves_other_children(repair_module) -> None:
    ...
    for entry in before["variant_children"]:
        if entry["path"] not in {
            f"data/normalized_yaml/{child.path}" for child in repair_module.CHILDREN
        }:
...
def test_repair_is_idempotent(repair_module) -> None:
    ...
    twice = {
        ...
        if path == repair_module.NORMALIZED / repair_module.PARENT
            ...
            doc,
            repair_module.CHILD_BY_PATH[path.relative_to(repair_module.NORMALIZED)],
        )
...
def test_plan_repairs_targets_current_records(repair_module) -> None:
    ...
    for child in repair_module.CHILDREN:
        ...
        expected[path] = repair_module.repair_child(_load_yaml(path), child)
...
def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    child = repair_module.CHILDREN[0]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["ingredients"][0]["preferred_term"] = "KCl"
...
def test_repair_rejects_child_ph_drift(repair_module) -> None:
    child = next(
        child
        for child in repair_module.CHILDREN
        if child.relationship == repair_module.PH_RELATIONSHIP
    )
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["ph_value"] = 10.0
