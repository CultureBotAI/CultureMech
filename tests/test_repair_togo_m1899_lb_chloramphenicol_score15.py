...
SCRIPT = REPO / "scripts" / "repair_togo_m1899_lb_chloramphenicol_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1899_lb_chloramphenicol_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1899")
...
def _target_doc(repair_module) -> dict:
    return {
        ...
        "original_name": "LB + Chloramphenicol medium",
        "category": "bacterial",
        "medium_type": "COMPLEX",
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1899",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "LB + Chloramphenicol medium",
            },
        },
...
def test_repair_expands_lb_chloramphenicol_stock(
    repair_module,
    scorer_module,
) -> None:
    ...
    chloramphenicol = solutions["Chloramphenicol solution (25 mg/ml)"]["composition"][0]
    ...
    assert chloramphenicol["concentration"] == {
        "value": "25.0",
        "unit": "MG_PER_ML",
    }
    assert chloramphenicol["term"] == {
        "id": "CHEBI:17698",
...
def test_repair_adds_flags_references_and_event_once(repair_module) -> None:
    ...
    assert once["references"] == [
        {"reference": repair_module.TOGO_M1899},
        {"reference": repair_module.NBRC_1158},
    ]
    matching_events = [
        event
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    doc = _target_doc(repair_module)
    doc["solutions"][0]["preferred_term"] = "Chloramphenicol"
