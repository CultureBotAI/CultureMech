...
SCRIPT = REPO / "scripts" / "repair_nbrc_272_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_nbrc_272_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_nbrc_272")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1489",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "TOGO Medium M1489",
            },
        },
        "notes": "Source: NBRC - NBRC_M272",
...
def test_repair_corrects_water_volume_and_keeps_casitone_unmapped(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ...
    assert "opaque complex component" in ingredients["Bacto Casitone (Difco)"]["notes"]


def test_repair_grounds_all_disclosed_formula_components(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
...
def test_repair_adds_preparation_references_flags_and_event_once(
    ...
    assert "retained Bacto Casitone" in matching_events[0]["notes"]
