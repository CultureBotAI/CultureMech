...
SCRIPT = REPO / "scripts" / "repair_komodo_2065_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2065_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2065")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution SL-4 (medium 457)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "notes": (
            "pH buffer: NaOH | Source: KOMODO ModelSEED | ID: 2065 | "
            "DSMZ Medium: 2065 (mediadive.medium:2065)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2065",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution SL-4 (medium 457)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2065_metabolite_table(repair_module, scorer_module) -> None:
    ...
    assert repaired["ph_range"] == {"min": 7.0, "max": 8.0}
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2065}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2058"
...
def test_repair_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "Trace element solution SL-4",
