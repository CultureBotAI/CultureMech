...
SCRIPT = REPO / "scripts" / "repair_komodo_778_thermoterrabacterium_ph_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_778_thermoterrabacterium_ph_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_778")
...
def test_ph_variants_exit_review_ranking(repair_module, scorer_module) -> None:
...
def test_parent_relabels_all_children(repair_module) -> None:
...
def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    child = repair_module.CHILDREN[0]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["ingredients"][0]["preferred_term"] = "Anthraquinone"
