...
SCRIPT = REPO / "scripts" / "repair_togo_m1696_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1696_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1696")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "name": "reinforced_clostridial_medium",
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1696",
            "term": {
...
def test_repair_keeps_commercial_base_opaque(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))

    assert _by_name(repaired["ingredients"])["CM0149 Broth (OXOID)"] == {
        "preferred_term": "CM0149 Broth (OXOID)",
...
def test_repair_adds_only_source_stated_autoclave_step(
    repair_module,
) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "added the NBRC autoclave step" in matching_events[0]["notes"]
...
def test_repair_record_rejects_solution_drift(repair_module) -> None:
    ...
    doc["solutions"] = [
        {
            "preferred_term": "RCM solution",
            "concentration": {"value": "38", "unit": "G_PER_L"},
            "composition": [],
        }
    ]
...
def test_target_record_matches_togo_m1696_repair_contract(
    repair_module,
) -> None:
