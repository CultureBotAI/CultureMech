...
SCRIPT = REPO / "scripts" / "repair_komodo_480_desulfurella_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_480_desulfurella")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_480")
...
def test_strain_pointer_exits_review_ranking(
    repair_module,
    scorer_module,
) -> None:
...
def test_parent_preserves_duplicate_parent_and_lists_strain_child(repair_module) -> None:
    repaired = repair_module.repair_parent(
        _load_yaml(repair_module.NORMALIZED / repair_module.PARENT)
    )

    assert repaired["parent_media"]["id"] == repair_module.SOURCE_DUPLICATE_PARENT_ID
    assert repaired["variant_relationship"] == "SOURCE_DUPLICATE"
    assert repaired["variant_children"] == [repair_module._child_entry()]


def test_source_duplicate_parent_drops_strain_pointer(repair_module) -> None:
    repaired = repair_module.repair_source_duplicate_parent(
        _load_yaml(repair_module.NORMALIZED / repair_module.SOURCE_DUPLICATE_PARENT)
    )

    children = repaired["variant_children"]
    assert len(children) == repair_module.EXPECTED_SOURCE_DUPLICATE_CHILD_COUNT
    assert all(child["id"] != repair_module.CHILD_ID for child in children)


def test_repair_is_idempotent(repair_module) -> None:
    once = repair_module.plan_repairs()
    twice = {
        path: repair_module.repair_source_duplicate_parent(doc)
        if path == repair_module.NORMALIZED / repair_module.SOURCE_DUPLICATE_PARENT
        else repair_module.repair_parent(doc)
...
def test_plan_repairs_targets_current_records(repair_module) -> None:
    assert repair_module.plan_repairs() == {
        ...
        / repair_module.SOURCE_DUPLICATE_PARENT: repair_module.repair_source_duplicate_parent(
            _load_yaml(repair_module.NORMALIZED / repair_module.SOURCE_DUPLICATE_PARENT)
        ),
