...
SCRIPT = REPO / "scripts" / "repair_togo_m184_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m184_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m184")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M184",
            "term": {
...
def test_repair_grounds_jcm_complex_components(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_keeps_jcm_component_amounts(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Bacto peptone (BD-Difco)"]["concentration"] == {
        "value": "5.0",
        "unit": "G_PER_L",
    }
    assert ingredients["Beef extract (BD-Difco)"]["concentration"] == {
        "value": "3.0",
        "unit": "G_PER_L",
    }
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "added the JCM pH adjustment" in matching_events[0]["notes"]
...
def test_target_record_matches_togo_m184_repair_contract(
    repair_module,
) -> None:
