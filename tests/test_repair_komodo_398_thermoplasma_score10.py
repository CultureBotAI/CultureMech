...
SCRIPT = REPO / "scripts" / "repair_komodo_398_thermoplasma_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_398_thermoplasma")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_398")
...
def test_strain_variant_exits_review_ranking(repair_module, scorer_module) -> None:
    child = _child_by_path(repair_module)["data/normalized_yaml/bacterial/for_dsm_4301.yaml"]
...
def test_old_hub_becomes_strain_child(repair_module) -> None:
    child = _child_by_path(repair_module)["data/normalized_yaml/bacterial/for_dsm_4301.yaml"]
    repaired = repair_module.repair_child(
        _load_yaml(repair_module.NORMALIZED / child.path),
        child,
...
def test_komodo_records_become_source_duplicate_children(repair_module) -> None:
    child_paths = (
        "data/normalized_yaml/archaea/KOMODO_398_THERMOPLASMA_VOLCANIUM_medium.yaml",
        "data/normalized_yaml/bacterial/aerobic_version.yaml",
    )

    for path in child_paths:
        child = _child_by_path(repair_module)[path]
        repaired = repair_module.repair_child(
            _load_yaml(repair_module.NORMALIZED / child.path),
            child,
        )
        ...
        assert repaired["parent_media"]["path"] == (
            "data/normalized_yaml/archaea/thermoplasma_volcanium_medium.yaml"
        )
