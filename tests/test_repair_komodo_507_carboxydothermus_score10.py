...
SCRIPT = REPO / "scripts" / "repair_komodo_507_carboxydothermus_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_507_carboxydothermus")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_507")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_507").CHILDREN))
def test_children_link_to_komodo_507_parent(repair_module, child) -> None:
...
@pytest.mark.parametrize(
    "child_path, relationship",
    (
        ("bacterial/for_dsm_12326.yaml", "STRAIN_SPECIFIC_VARIANT"),
        ("bacterial/for_dsm_18923.yaml", "STRAIN_SPECIFIC_VARIANT"),
        ("bacterial/for_dsm_21278.yaml", "STRAIN_SPECIFIC_VARIANT"),
        ("bacterial/for_dsm_21830.yaml", "STRAIN_SPECIFIC_VARIANT"),
        ("bacterial/for_dsm_23132.yaml", "PH_VARIANT"),
        ("bacterial/for_dsm_23698.yaml", "STRAIN_SPECIFIC_VARIANT"),
    ),
)
def test_score_pointers_exit_review_ranking(
    repair_module,
    scorer_module,
    child_path: str,
    relationship: str,
) -> None:
    child = repair_module.CHILD_BY_PATH[Path(child_path)]
    repaired = repair_module.repair_child(
        child.path,
        _load_yaml(repair_module.NORMALIZED / child.path),
    )

    assert repaired["parent_media"]["relationship"] == relationship
    assert scorer_module.score_record(repaired) == (0, [])
    assert scorer_module.score_parsed([(str(child.path), repaired)]) == []


def test_parent_lists_507_children(repair_module) -> None:
