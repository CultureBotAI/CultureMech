...
SCRIPT = REPO / "scripts" / "repair_komodo_2056_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_2056_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_2056")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "original_name": "Trace element solution (medium 925)",
        "category": "bacterial",
        "medium_type": "DEFINED",
        ...
        "ph_value": 3.0,
        "notes": (
            "pH buffer: HCl | Source: KOMODO ModelSEED | ID: 2056 | "
            "DSMZ Medium: 2056 (mediadive.medium:2056)"
        ),
        "media_term": {
            "preferred_term": "KOMODO Medium 2056",
            "term": {
                "id": repair_module.EXPECTED_MEDIA_TERM,
                "label": "Trace element solution (medium 925)",
            },
        },
        "ingredients": [
...
def test_repair_expands_komodo_2056_metabolite_table(repair_module, scorer_module) -> None:
...
def test_repair_adds_reference_flags_and_event_once(repair_module) -> None:
    ...
    assert twice == once
    assert once["references"] == [{"reference": repair_module.KOMODO_2056}]
...
def test_repair_rejects_wrong_media_term(repair_module) -> None:
    doc = _doc(repair_module)
    doc["media_term"]["term"]["id"] = "komodo.medium:2052"
...
def test_repair_rejects_ingredient_drift(repair_module) -> None:
    doc = _doc(repair_module)
    doc["ingredients"][0]["preferred_term"] = "H2SO4"
