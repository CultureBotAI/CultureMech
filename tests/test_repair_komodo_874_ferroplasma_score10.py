...
SCRIPT = REPO / "scripts" / "repair_komodo_874_ferroplasma_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_874_ferroplasma")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_874")
...
@pytest.mark.parametrize("child", tuple(_load_script(SCRIPT, "komodo_874").CHILDREN))
def test_ferroplasma_strain_variants_exit_review_ranking(
    repair_module,
    scorer_module,
    child,
) -> None:
