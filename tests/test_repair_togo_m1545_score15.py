...
SCRIPT = REPO / "scripts" / "repair_togo_m1545_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1545_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1545")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1545",
...
def test_repair_keeps_peptone_ungrounded(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))

    assert _by_name(repaired["ingredients"])["Peptone"] == {
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "added the NBRC pH value" in matching_events[0]["notes"]
...
def test_target_record_matches_togo_m1545_repair_contract(
    repair_module,
) -> None:
