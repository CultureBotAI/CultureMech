from __future__ import annotations

import importlib.util
import sys
from pathlib import Path

import pytest
import yaml

REPO = Path(__file__).resolve().parent.parent
SCRIPT = REPO / "scripts" / "repair_komodo_390_pyrobaculum_score10.py"
SCORER = REPO / "scripts" / "score_review_need.py"


def _load_script(path: Path, name: str):
    spec = importlib.util.spec_from_file_location(name, path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    assert spec.loader is not None
    spec.loader.exec_module(module)
    return module


@pytest.fixture(scope="module")
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_390_pyrobaculum")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_390")


def _load_yaml(path: Path) -> dict:
    return yaml.safe_load(path.read_text(encoding="utf-8"))


def _child_by_path(repair_module):
    return {f"data/normalized_yaml/{child.path}": child for child in repair_module.CHILDREN}


def test_strain_variants_exit_review_ranking(repair_module, scorer_module) -> None:
    repaired = [
        (
            str(child.path),
            repair_module.repair_child(
                _load_yaml(repair_module.NORMALIZED / child.path),
                child,
            ),
        )
        for child in repair_module.CHILDREN
    ]

    assert all(
        doc["parent_media"]["relationship"] == "STRAIN_SPECIFIC_VARIANT"
        and doc["variant_relationship"] == "STRAIN_SPECIFIC_VARIANT"
        and "ingredients_curated" in doc["data_quality_flags"]
        and scorer_module.score_record(doc) == (0, [])
        for _, doc in repaired
    )
    assert scorer_module.score_parsed(repaired) == []


def test_dsmz_parent_points_to_archaeal_komodo_record(repair_module) -> None:
    before = _load_yaml(repair_module.NORMALIZED / repair_module.DSMZ_PARENT)
    repaired = repair_module.repair_dsmz_parent(before)

    assert repaired["variant_children"] == [repair_module._komodo_source_child_entry()]


def test_komodo_parent_points_to_archaeal_dsmz_parent(repair_module) -> None:
    before = _load_yaml(repair_module.NORMALIZED / repair_module.KOMODO_PARENT)
    repaired = repair_module.repair_komodo_parent(before)
    by_path = {entry["path"]: entry for entry in repaired["variant_children"]}

    assert repaired["parent_media"] == repair_module._dsmz_parent_ref()
    assert set(by_path) == set(_child_by_path(repair_module))
    for path, child in _child_by_path(repair_module).items():
        assert by_path[path] == repair_module._strain_child_entry(child)


def test_repair_is_idempotent(repair_module) -> None:
    once = repair_module.plan_repairs()
    children = _child_by_path(repair_module)
    twice = {}
    for path, doc in once.items():
        rel_path = path.relative_to(repair_module.NORMALIZED)
        if rel_path == repair_module.DSMZ_PARENT:
            twice[path] = repair_module.repair_dsmz_parent(doc)
        elif rel_path == repair_module.KOMODO_PARENT:
            twice[path] = repair_module.repair_komodo_parent(doc)
        else:
            twice[path] = repair_module.repair_child(
                doc,
                children[f"data/normalized_yaml/{rel_path}"],
            )

    assert twice == once
    for path in once:
        assert repair_module.dump_record(twice[path]) == repair_module.dump_record(once[path])


def test_plan_repairs_targets_current_records(repair_module) -> None:
    expected = {
        repair_module.NORMALIZED
        / repair_module.DSMZ_PARENT: repair_module.repair_dsmz_parent(
            _load_yaml(repair_module.NORMALIZED / repair_module.DSMZ_PARENT)
        ),
        repair_module.NORMALIZED
        / repair_module.KOMODO_PARENT: repair_module.repair_komodo_parent(
            _load_yaml(repair_module.NORMALIZED / repair_module.KOMODO_PARENT)
        ),
    }
    for child in repair_module.CHILDREN:
        expected[repair_module.NORMALIZED / child.path] = repair_module.repair_child(
            _load_yaml(repair_module.NORMALIZED / child.path),
            child,
        )

    assert repair_module.plan_repairs() == expected


def test_repair_rejects_wrong_child_source(repair_module) -> None:
    child = repair_module.CHILDREN[-1]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["media_term"]["term"]["id"] = "komodo.medium:wrong"

    with pytest.raises(ValueError, match=child.source_term):
        repair_module.repair_child(doc, child)


def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    child = repair_module.CHILDREN[-1]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["ingredients"][0]["preferred_term"] = "KH2PO4"

    with pytest.raises(ValueError, match="ingredient signature drifted"):
        repair_module.repair_child(doc, child)
