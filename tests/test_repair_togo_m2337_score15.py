...
SCRIPT = REPO / "scripts" / "repair_togo_m2337_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m2337_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m2337")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M2337",
...
def test_repair_corrects_formula_units_and_ph(repair_module) -> None:
    ...
    assert ingredients["Cellobiose"]["concentration"] == {
        "value": "5.0",
        "unit": "G_PER_L",
    }
...
def test_repair_grounds_defined_components(repair_module) -> None:
    ...
    assert ingredients["Cellobiose"]["mediaingredientmech_chebi_term"] == {
        "id": "CHEBI:17057",
        "label": "cellobiose",
    }
...
def test_repair_keeps_casitone_ungrounded(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])

    assert ingredients["Casitone"] == {
...
def test_repair_adds_ph_and_cellobiose_steps(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "converted cellobiose from 0.5% w/v to 5 g/L" in matching_events[0]["notes"]
...
def test_target_record_matches_togo_m2337_repair_contract(
    repair_module,
) -> None:
