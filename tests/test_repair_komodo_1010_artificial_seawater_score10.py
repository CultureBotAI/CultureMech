...
SCRIPT = REPO / "scripts" / "repair_komodo_1010_artificial_seawater_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_1010_artificial_seawater_score10")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_1010")
...
def test_parent_preserves_desulfopila_source_duplicate(repair_module) -> None:
    before = _load_yaml(repair_module.NORMALIZED / repair_module.PARENT)
    repaired = repair_module.repair_parent(before)
    expected_path = f"data/normalized_yaml/{repair_module.CHILD.path}"
    by_path = {entry["path"]: entry for entry in repaired["variant_children"]}

    assert by_path[expected_path] == repair_module._child_entry(repair_module.CHILD)
...
def test_plan_repairs_targets_current_records(repair_module) -> None:
    assert repair_module.plan_repairs() == {
        ...
        repair_module.NORMALIZED / repair_module.CHILD.path: repair_module.repair_child(
            _load_yaml(repair_module.NORMALIZED / repair_module.CHILD.path)
        ),
    }
...
def test_repair_rejects_child_ph_range_drift(repair_module) -> None:
    doc = _load_yaml(repair_module.NORMALIZED / repair_module.CHILD.path)
    doc["ph_range"]["max"] = 7.7
