...
SCRIPT = REPO / "scripts" / "repair_komodo_945_marinitoga_strain_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_945_marinitoga_strain")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_945_strain")
...
def test_parent_adds_strain_child_without_removing_ph_child(repair_module) -> None:
    ...
    assert repair_module._child_entry() in children
    assert any(
        isinstance(child, dict) and child.get("id") == "CultureMech:006879" for child in children
    )
