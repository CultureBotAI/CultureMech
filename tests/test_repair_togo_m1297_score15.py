...
SCRIPT = REPO / "scripts" / "repair_togo_m1297_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1297_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1297")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1297",
...
def test_repair_corrects_base_formula_and_conditions(repair_module) -> None:
    ...
    assert ingredients["MES"]["term"] == {
        "id": "CHEBI:39005",
        "label": "2-(N-morpholino)ethanesulfonic acid",
    }
...
def test_repair_adds_preparation_metadata(repair_module) -> None:
    ...
    assert repaired["preparation_steps"][1]["description"] == "Adjust to pH 6.0."
...
def test_repair_record_rejects_wrong_id(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="expected id CultureMech:007830"):
...
def test_repair_record_rejects_wrong_media_term(repair_module) -> None:
    ...
    with pytest.raises(ValueError, match="expected media term TOGO:M1297"):
        repair_module.repair_record(doc)
...
def test_target_record_matches_togo_m1297_repair_contract(
    repair_module,
) -> None:
