...
SCRIPT = REPO / "scripts" / "repair_komodo_339_wilkins_topology_score10.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_komodo_339_wilkins_topology")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_komodo_339")
...
def test_strain_variants_exit_review_ranking(repair_module, scorer_module) -> None:
    ranked_paths = {
        "bacterial/for_dsm_14428.yaml",
        "bacterial/for_dsm_23669.yaml",
    }
    repaired = [
        (
            str(child.path),
        ...
        if str(child.path) in ranked_paths
    ]
...
def test_parent_promotes_medium_339_root(repair_module) -> None:
...
def test_mediadive_339_remains_source_duplicate(repair_module) -> None:
    child = _child_by_path(repair_module)[
...
def test_repair_rejects_child_ingredient_drift(repair_module) -> None:
    child = repair_module.CHILDREN[-1]
    doc = _load_yaml(repair_module.NORMALIZED / child.path)
    doc["ingredients"][0]["preferred_term"] = "Sodium resazurin"
