...
SCRIPT = REPO / "scripts" / "repair_togo_m1540_score15.py"
...
def repair_module():
    return _load_script(SCRIPT, "repair_togo_m1540_score15")


@pytest.fixture(scope="module")
def scorer_module():
    return _load_script(SCORER, "score_review_need_for_togo_m1540")
...
def _doc(repair_module) -> dict:
    return {
        ...
        "media_term": {
            "preferred_term": "TOGO Medium M1540",
        ...
        "variant_children": [
            {
                "path": "data/normalized_yaml/bacterial/flavobacterium_columnare_agar.yaml",
                "relationship": "CONCENTRATION_VARIANT",
                "id": "CultureMech:008090",
                "name": "flavobacterium_columnare_agar",
            }
        ],
    }
...
def test_repair_corrects_formula_units_and_ph(repair_module) -> None:
...
def test_repair_keeps_peptone_products_ungrounded(repair_module) -> None:
    repaired = repair_module.repair_record(_doc(repair_module))
    ingredients = _by_name(repaired["ingredients"])
...
def test_repair_preserves_variant_children_without_inventing_steps(
    repair_module,
) -> None:
...
def test_repair_adds_references_and_event_once(repair_module) -> None:
    ...
    assert "added the NBRC pH range" in matching_events[0]["notes"]
...
def test_target_record_matches_togo_m1540_repair_contract(
    repair_module,
) -> None:
