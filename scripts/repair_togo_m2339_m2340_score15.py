#!/usr/bin/env python3
"""Repair TOGO M2339/M2340 Nutrient Agar records."""
...
CURATOR = "repair_togo_m2339_m2340_score15.py"
ACTION = "RESOLVED_TOGO_M2339_M2340_SCORE15"
LINK_ACTION = "LINKED_TOGO_M2339_M2340_DSMZ_MEDIUM_1"
TIMESTAMP = "2026-09-11T00:00:00-07:00"

TOGO_M2339 = "https://togomedium.org/medium/M2339"
TOGO_M2340 = "https://togomedium.org/medium/M2340"
...
M2339_IMPORTED_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
...
M2340_IMPORTED_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
    ("Distilled water", "1000", "G_PER_L"),
    ("MnSO4", "10", "G_PER_L"),
    ("Agar, if necessary", "15", "G_PER_L"),
    ("Meat extract", "3", "G_PER_L"),
...
M2339_FINAL_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
    ("Peptone", "5.0", "G_PER_L"),
...
M2340_FINAL_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
    ("Peptone", "5.0", "G_PER_L"),
    ("Meat extract", "3.0", "G_PER_L"),
...
class Target:
    ...
    def reference_urls(self) -> tuple[str, ...]:
        return (self.togo_url, DSMZ_1)


TARGET_M2339 = Target(
    path=Path("bacterial/TOGO_M2339_Nutrient_Agar.yaml"),
    expected_id="CultureMech:008926",
    expected_media_term="TOGO:M2339",
    togo_url=TOGO_M2339,
    source="TOGO M2339 / DSMZ Medium 1",
    imported_signature=M2339_IMPORTED_INGREDIENT_SIGNATURE,
    final_signature=M2339_FINAL_INGREDIENT_SIGNATURE,
    include_sporulation_manganese=False,
    relationship="SOURCE_DUPLICATE",
    parent_notes=(
        "TOGO M2339 imports DSMZ Medium 1 and explicitly retains its " "distilled-water row."
    ),
    child_notes=(
        "TOGO M2339 imports DSMZ Medium 1 and exactly matches its formula, "
        "with the source distilled-water row retained."
    ),
    variant_modification=(
        "Same ingredient and concentration signature as DSMZ Medium 1; TOGO "
        "M2339 explicitly retains the source distilled-water row."
    ),
)

TARGET_M2340 = Target(
    path=Path("bacterial/TOGO_M2340_Nutrient_Agar.yaml"),
    expected_id="CultureMech:008928",
    expected_media_term="TOGO:M2340",
    togo_url=TOGO_M2340,
    source="TOGO M2340 / DSMZ Medium 1",
    imported_signature=M2340_IMPORTED_INGREDIENT_SIGNATURE,
    final_signature=M2340_FINAL_INGREDIENT_SIGNATURE,
    include_sporulation_manganese=True,
    relationship="SUPPLEMENTED_VARIANT",
    parent_notes=(
        "TOGO M2340 imports DSMZ Medium 1 with the 10 mg/L MnSO4 x H2O "
        "Bacillus sporulation supplement."
    ),
    child_notes=(
        "TOGO M2340 imports DSMZ Medium 1 with the source-recommended "
...
TARGETS = (TARGET_M2339, TARGET_M2340)
...
def _component(
    ...
    *,
    ...
    row: dict[str, Any] = {
        ...
        "source": source,
        "notes": notes or f"DSMZ Medium 1 lists {value} {UNIT_LABELS[unit]} {preferred_term}.",
    }
    ...
    if target.include_sporulation_manganese:
        rows.append(
            _component(
                ...
                source=target.source,
                notes=(
                    "DSMZ Medium 1 recommends adding 10 mg/L MnSO4 x H2O "
...
def _notes(target: Target) -> str:
    supplement = (
        " TOGO M2340 represents the source-recommended Bacillus sporulation "
        "addition as 10 mg/L MnSO4 x H2O."
        if target.include_sporulation_manganese
        else ""
    )
    return (
        f"{target.source} defines Nutrient Agar using DSMZ Medium 1. DSMZ "
        "Medium 1 lists, per liter, 5 g Peptone, 3 g Meat extract, "
        "15 g Agar if necessary, and 1000 ml Distilled water. Adjust pH "
        f"to 7.0.{supplement}"
    )
...
def _event_notes(target: Target) -> str:
    ...
    return (
        ...
        "unmapped, and linked the record to DSMZ Medium 1."
    )
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _append_curation_event(
        repaired,
        action=LINK_ACTION,
        source="; ".join((TOGO_M2339, TOGO_M2340, DSMZ_1)),
        notes=(
            "Linked TOGO M2339 as a source duplicate and TOGO M2340 as a "
            "10 mg/L MnSO4 x H2O supplemented variant of DSMZ Medium 1."
        ),
    )
    return repaired
