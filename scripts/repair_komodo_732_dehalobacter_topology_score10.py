#!/usr/bin/env python3
"""Repair KOMODO 732 topology rooted at a strain-specific duplicate."""
...
PARENT = Path("bacterial/dehalobacter_restrictus_medium.yaml")
PARENT_ID = "CultureMech:006377"
PARENT_NAME = "dehalobacter_restrictus_medium"
PARENT_SOURCE_TERM = "komodo.medium:732"
PARENT_PH = 7.2

INGREDIENT_SIGNATURE = (
    ("K2HPO4", "0.747126", "G_PER_L"),
    ("NaH2PO4 x H2O", "0.195402", "G_PER_L"),
    ("Peptone", "0.114943", "G_PER_L"),
    ("Na-acetate", "0.528736", "G_PER_L"),
    ("Sodium resazurin", "0.000574713", "G_PER_L"),
    ("(NH4)2CO3", "2.7", "G_PER_L"),
    ("NaHCO3", "37.3", "G_PER_L"),
    ("CaCl2 x 2 H2O", "11", "G_PER_L"),
    ("MgCl2 x 6 H2O", "10", "G_PER_L"),
    ("Na2S x 9 H2O", "30", "G_PER_L"),
    ("Hexadecane", "13.5", "G_PER_L"),
    ("Tetrachloroethene", "1.5", "G_PER_L"),
    ("NaOH", "0.5", "G_PER_L"),
...
CURATOR = "repair_komodo_732_dehalobacter_topology_score10.py"
ACTION = "RESOLVED_KOMODO_732_DEHALOBACTER_TOPOLOGY"
...
class Child:
    ...
    def notes(self) -> str:
        if self.relationship == "SOURCE_DUPLICATE":
            return (
                f"{self.source_label} exactly matches the KOMODO Medium 732 "
                "DEHALOBACTER RESTRICTUS MEDIUM base."
            )
        return (
            f"{self.source_label} applies DEHALOBACTER RESTRICTUS MEDIUM "
...
CHILDREN = (
    Child(
        path=Path("bacterial/desulfuromonas_medium_tce.yaml"),
        record_id="CultureMech:001868",
        name="desulfuromonas_medium_tce",
        source_term="mediadive.medium:732a",
        relationship="SOURCE_DUPLICATE",
        context="MediaDive Medium 732a",
    ),
    Child(
        path=Path("bacterial/for_dsm_13726.yaml"),
        record_id="CultureMech:006375",
        name="for_dsm_13726",
        source_term="komodo.medium:732.1",
        relationship="STRAIN_SPECIFIC_VARIANT",
        context="DSM 13726",
    ),
    Child(
        path=Path("bacterial/for_dsm_15941.yaml"),
        record_id="CultureMech:006376",
        name="for_dsm_15941",
        source_term="komodo.medium:732.2",
        relationship="STRAIN_SPECIFIC_VARIANT",
        context="DSM 15941",
    ),
)
...
def _parent_ref(child: Child) -> dict[str, str]:
    return {
        ...
        "name": PARENT_NAME,
        "notes": (
            f"KOMODO Medium 732 is the pH {PARENT_PH:g} " "DEHALOBACTER RESTRICTUS MEDIUM base."
        ),
    }


def _require_dehalobacter_record(
    doc: dict[str, Any],
    relative_path: Path,
    record_id: str,
    source_term: str,
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_dehalobacter_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Promoted KOMODO Medium 732 to the Dehalobacter family parent",
            "source": "KOMODO Medium 732-732.2 and MediaDive Medium 732a",
            "notes": (
                "Moved the family root from strain-specific KOMODO Medium 732.1 "
                "to base KOMODO Medium 732 and reclassified DSM-specific submedia "
...
def repair_child(doc: dict[str, Any], child: Child) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked under DEHALOBACTER RESTRICTUS MEDIUM",
