#!/usr/bin/env python3
"""Repair a KOMODO Chrysiogenes strain variant mislinked as a duplicate."""
...
PARENT = Path("bacterial/KOMODO_818_CHRYSIOGENES_MEDIUM.yaml")
PARENT_ID = "CultureMech:006543"
PARENT_NAME = "chrysiogenes_medium"
PARENT_SOURCE_TERM = "komodo.medium:818"
PH_RANGE = {"min": 7.4, "max": 7.8}
PH_LABEL = "7.4-7.8"

INGREDIENT_SIGNATURE = (
    ("NaCl", "1.19641", "G_PER_L"),
    ("KCl", "0.299103", "G_PER_L"),
    ("NH4Cl", "0.299103", "G_PER_L"),
    ("KH2PO4", "0.199402", "G_PER_L"),
    ("Na2SO4", "0.299103", "G_PER_L"),
    ("MgCl2 x 6 H2O", "0.398804", "G_PER_L"),
    ("CaCl2 x 2 H2O", "0.149551", "G_PER_L"),
    ("Sodium resazurin", "0.000498504", "G_PER_L"),
    ("NaHCO3", "0.598205", "G_PER_L"),
    ("KNO3", "0.498504", "G_PER_L"),
    ("Na-acetate", "0.498504", "G_PER_L"),
...
CURATOR = "repair_komodo_818_chrysiogenes_score10.py"
ACTION = "RESOLVED_KOMODO_818_CHRYSIOGENES_STRAIN_VARIANT"
...
class Child:
    ...
    def modification(self) -> str:
        return (
            f"{self.source_label} preserves CHRYSIOGENES MEDIUM components "
...
CHILD = Child(
    path=Path("bacterial/for_dsm_16346.yaml"),
    record_id="CultureMech:006542",
    name="for_dsm_16346",
    source_term="komodo.medium:818.1",
    dsm="DSM 16346",
)
...
def _parent_ref(child: Child) -> dict[str, str]:
    return {
        ...
        "id": PARENT_ID,
        "name": PARENT_NAME,
        "notes": f"KOMODO Medium 818 is the pH {PH_LABEL} CHRYSIOGENES MEDIUM base.",
    }
...
def _require_chrysiogenes_record(
    doc: dict[str, Any],
    relative_path: Path,
    record_id: str,
    source_term: str,
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_chrysiogenes_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked a CHRYSIOGENES MEDIUM strain-specific child",
            "source": CHILD.source_label,
            "notes": (
                "Changed DSM 16346 from SOURCE_DUPLICATE to a "
                "STRAIN_SPECIFIC_VARIANT child of KOMODO Medium 818."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any], child: Child = CHILD) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a CHRYSIOGENES MEDIUM strain-specific variant",
            "source": child.source_label,
