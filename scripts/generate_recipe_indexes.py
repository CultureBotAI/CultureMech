...
def extract_recipe_metadata(recipe: dict, filepath: Path) -> dict:
    ...
    Returns:
        Metadata dictionary
    """
    # Extract source. A medium names its upstream in `media_term.term.id`; a
    # solution record names it in a top-level `term.id` (#452).
    source = "unknown"
    source_id = None
    media_term = recipe.get("media_term", {})
    term = media_term.get("term", {}) if isinstance(media_term, dict) else {}
    if not (isinstance(term, dict) and term.get("id")):
        term = recipe.get("term", {})
    if isinstance(term, dict) and term.get("id"):
        ...
        elif term_id.startswith("MediaIngredientMech:"):
            source = "MediaIngredientMech"

    # Get CultureMech ID if present
    culturemech_id = recipe.get("id")

    # Count reagents. A solution record keeps them in `composition`; its
    # `ingredients` is at most the documented "See source for composition"
    # placeholder, which is not a reagent (#452, #444).
    composition = recipe.get("composition") or []
    ingredient_count = len(composition) if composition else len(recipe.get("ingredients") or [])
    ...
    metadata = {
        ...
        "name": recipe.get("name") or recipe.get("preferred_term"),
    ...
    if is_merged:
        ...
        metadata["merged_sources"] = list({mf.split("_")[0] for mf in merged_from if "_" in mf})
...
def generate_category_indexes(recipe_dir: Path, output_dir: Path) -> dict[str, dict]:
    ...
    for category_dir in sorted(recipe_dir.iterdir()):
        ...
        for recipe_path in recipes:
            ...
            key = metadata.get("id") or metadata.get("source_id") or recipe_path.stem
...
def generate_master_index(category_indexes: dict[str, dict], output_path: Path) -> dict:
...
def generate_source_indexes(master_index: dict, output_dir: Path) -> dict[str, dict]:
...
def generate_statistics(master_index: dict, output_path: Path) -> dict:
    ...
    # Count merged
    merged_count = sum(1 for m in recipes.values() if m.get("merged_from_count", 0) > 1)

    # Count with hierarchy
    hierarchy_count = sum(1 for m in recipes.values() if m.get("has_hierarchy_enrichment", False))

    # Ingredient statistics
    ingredient_counts = [m["ingredient_count"] for m in recipes.values() if "ingredient_count" in m]
    ...
    stats = {
        ...
        "ingredient_statistics": {
            ...
            "avg": (
                round(sum(ingredient_counts) / len(ingredient_counts), 1)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate index files for recipe collections")
    ...
    parser.add_argument(
        "--output-dir", type=Path, help="Output directory for indexes (default: same as recipe-dir)"
    )
    ...
    parser.add_argument(
        "--stats-only", action="store_true", help="Generate only statistics (skip indexes)"
    )
    ...
    print("=" * 70)
    print(
        f"Total recipes indexed: {master_index.get('total_recipes', stats.get('total_recipes', 0)):,}"
    )
    print()
    print("Files generated:")
    print("  • recipe_index.json (master index)")
    print("  • recipe_statistics.json")

    if not args.master_only:
        print(f"  • {len(master_index.get('categories', {}))} category indexes")
        print(
            f"  • {len({m.get('source') for m in master_index['recipes'].values()})} source indexes"
        )
