#!/usr/bin/env python3
"""Repair KOMODO 895 Anaerobranca exact and strain child links."""
...
PARENT = Path("bacterial/anaerobranca_gottschalkii_medium.yaml")
PARENT_ID = "CultureMech:002058"
PARENT_NAME = "anaerobranca_gottschalkii_medium"
PARENT_SOURCE_TERM = "mediadive.medium:895"
SOURCE_DUPLICATE_RELATIONSHIP = "SOURCE_DUPLICATE"
STRAIN_RELATIONSHIP = "STRAIN_SPECIFIC_VARIANT"

SIGNATURE = (
    ("(NH4)2SO4", "0.987167", "G_PER_L"),
    ("NH4Cl", "0.394867", "G_PER_L"),
    ("Na2S2O3 x 5 H2O", "0.0987167", "G_PER_L"),
    ("K2HPO4", "0.493583", "G_PER_L"),
    ("MgSO4 x 7 H2O", "3.0987167", "G_PER_L"),
    ("CaCl2 x 2 H2O", "0.1493583", "G_PER_L"),
    ("NaCl", "10.87167", "G_PER_L"),
    ("FeSO4 x 7 H2O", "0.10197433", "G_PER_L"),
    ("Yeast extract", "0.246792", "G_PER_L"),
    ("Tryptone", "0.246792", "G_PER_L"),
    ("Sodium resazurin", "0.000493583", "G_PER_L"),
    ("L-Cysteine HCl x H2O", "0.493583", "G_PER_L"),
    ("Na2CO3", "2.9615", "G_PER_L"),
    ("Starch", "1.97433", "G_PER_L"),
...
CHILDREN = (
    Child(
        Path("bacterial/KOMODO_895_ANAEROBRANCA_GOTTSCHALKII_medium.yaml"),
        "CultureMech:006750",
        "komodo.medium:895",
        "KOMODO Medium 895",
        SOURCE_DUPLICATE_RELATIONSHIP,
        "KOMODO Medium 895 has the same 35-ingredient signature as DSMZ Medium 895.",
    ),
    Child(
        Path("bacterial/for_dsm_14826.yaml"),
        "CultureMech:006748",
        "komodo.medium:895.1",
        "KOMODO Medium 895.1",
        STRAIN_RELATIONSHIP,
        "KOMODO Medium 895.1 applies DSMZ Medium 895 to DSM 14826.",
    ),
    Child(
        Path("bacterial/for_dsm_14828.yaml"),
        "CultureMech:006749",
        "komodo.medium:895.2",
        "KOMODO Medium 895.2",
        STRAIN_RELATIONSHIP,
        "KOMODO Medium 895.2 applies DSMZ Medium 895 to DSM 14828.",
    ),
)
CHILD_BY_PATH = {child.path: child for child in CHILDREN}
EXPECTED_CHILD_COUNT = 3

CURATOR = "repair_komodo_895_anaerobranca_score10.py"
ACTION = "RESOLVED_KOMODO_895_ANAEROBRANCA_TOPOLOGY"
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Re-rooted KOMODO Medium 895 Anaerobranca exact topology",
            "source": "DSMZ Medium 895; KOMODO Medium 895, 895.1, 895.2",
            "notes": (
                "Moved exact KOMODO Medium 895 plus its DSM-specific wrappers "
                "under DSMZ Medium 895."
            ),
        },
    )
    return repaired
...
def repair_child(relative_path: Path, doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": f"Linked under DSMZ Medium 895 as {child.relationship}",
