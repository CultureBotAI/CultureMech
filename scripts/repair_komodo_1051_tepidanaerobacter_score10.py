#!/usr/bin/env python3
"""Repair a KOMODO Tepidanaerobacter strain variant mislinked as a duplicate."""
...
PARENT = Path("bacterial/KOMODO_1051_TEPIDANAEROBACTER_medium.yaml")
PARENT_ID = "CultureMech:003626"
PARENT_NAME = "tepidanaerobacter_medium"
PARENT_SOURCE_TERM = "komodo.medium:1051"
...
INGREDIENT_SIGNATURE = (
    ...
    ("Na2CO3", "1.5", "G_PER_L"),
    ("Yeast extract", "2.3", "G_PER_L"),
    ("D-Glucose", "2.2", "G_PER_L"),
...
CURATOR = "repair_komodo_1051_tepidanaerobacter_score10.py"
ACTION = "RESOLVED_KOMODO_1051_TEPIDANAEROBACTER_STRAIN_VARIANT"
...
class Child:
    ...
    def modification(self) -> str:
        return (
            f"{self.source_label} preserves TEPIDANAEROBACTER MEDIUM components "
    ...
    def notes(self) -> str:
        return (
            f"{self.source_label} applies TEPIDANAEROBACTER MEDIUM at pH {PH_LABEL} "
            f"for {self.dsm}."
        )


CHILD = Child(
    path=Path("bacterial/for_dsm_21860.yaml"),
    record_id="CultureMech:003625",
    name="for_dsm_21860",
    source_term="komodo.medium:1051.1",
    dsm="DSM 21860",
)
...
def _parent_ref(child: Child) -> dict[str, str]:
    return {
        ...
        "id": PARENT_ID,
        "name": PARENT_NAME,
        "notes": (f"KOMODO Medium 1051 is the pH {PH_LABEL} " "TEPIDANAEROBACTER MEDIUM base."),
    }
...
def _require_tepidanaerobacter_record(
    doc: dict[str, Any],
    relative_path: Path,
    record_id: str,
    source_term: str,
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_tepidanaerobacter_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked a TEPIDANAEROBACTER MEDIUM strain-specific child",
            "source": CHILD.source_label,
            "notes": (
                "Changed DSM 21860 from SOURCE_DUPLICATE to a "
                "STRAIN_SPECIFIC_VARIANT child of KOMODO Medium 1051."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any], child: Child = CHILD) -> dict[str, Any]:
    _require_tepidanaerobacter_record(
        doc,
        child.path,
        child.record_id,
        child.source_term,
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a TEPIDANAEROBACTER MEDIUM strain-specific variant",
            "source": child.source_label,
