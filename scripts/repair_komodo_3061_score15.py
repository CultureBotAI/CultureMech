#!/usr/bin/env python3
"""Repair KOMODO 3061 Trace element solution (630)."""
...
TARGET = Path("bacterial/trace_element_solution_630.yaml")
YAML_LOADER = getattr(yaml, "CSafeLoader", yaml.SafeLoader)

CURATOR = "repair_komodo_3061_score15.py"
ACTION = "RESOLVED_KOMODO_3061_SCORE15"
TIMESTAMP = "2026-09-12T00:00:00-07:00"

EXPECTED_ID = "CultureMech:004845"
EXPECTED_MEDIA_TERM = "komodo.medium:3061"
KOMODO_3061 = (
    "https://komodo.modelseed.org/servlet/KomodoTomcatServerSideUtilitiesModelSeed?MediaInfo=3061"
)
SOURCE = "KOMODO Medium 3061"
...
FINAL_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
    ...
    ("MnCl2 x 4 H2O", "0.50", "G_PER_L"),
    ("CuCl2 x 2 H2O", "0.05", "G_PER_L"),
    ("FeCl2 x 4 H2O", "1.00", "G_PER_L"),
    ("Nitrilotriacetic acid", "12.80", "G_PER_L"),
    ("H2SO4", "variable", "VARIABLE"),
    ("H3BO3", "0.02", "G_PER_L"),
    ("Na2MoO4 x 2 H2O", "0.05", "G_PER_L"),
    ("CoCl2 x 4 H2O", "0.30", "G_PER_L"),
)

GROUNDINGS: dict[str, tuple[str, str]] = {
    "CoCl2 x 4 H2O": ("CHEBI:35696", "cobalt dichloride"),
...
NOTES = (
    "KOMODO 3061 lists Trace element solution (630) as a defined submedium at "
    "pH 6.8. Its metabolite table contains 12.80 g/L nitrilotriacetic acid, "
    "1.00 g/L FeCl2 x 4 H2O, 0.50 g/L MnCl2 x 4 H2O, 0.30 g/L CoCl2 x 4 H2O, "
    "0.05 g/L CuCl2 x 2 H2O, 0.05 g/L Na2MoO4 x 2 H2O, 0.02 g/L "
    "NiCl2 x 6 H2O, 0.02 g/L H3BO3, variable H2SO4, and distilled water."
)
...
PREPARATION_STEPS: tuple[dict[str, Any], ...] = (
    {
        ...
        "action": "MIX",
        "description": (
            "Prepare the 1 L Trace element solution (630) stock from distilled "
            "water, nitrilotriacetic acid, the chloride salts, H3BO3, Na2MoO4 x "
            "2 H2O, and H2SO4 listed by KOMODO 3061."
        ),
    },
    {
        "step_number": 2,
        "action": "ADJUST_PH",
        "description": (
            "Adjust the Trace element solution stock to pH 6.8 with KOH, NaOH, or H2SO4 as needed."
        ),
    },
)
...
def _component(preferred_term: str, value: str, unit: str) -> dict[str, Any]:
    ...
    if preferred_term == "Distilled water":
        row["notes"] = (
            ...
            "here as the solvent for 1 L of Trace element solution (630)."
        )
    elif preferred_term == "H2SO4":
        row["notes"] = (
            f"{SOURCE} lists H2SO4 without a gram amount and also identifies "
            "H2SO4 as one of the pH adjusters for pH 6.8."
        )
...
def _ensure_references(doc: dict[str, Any]) -> None:
    ...
    if not any(row.get("reference") == KOMODO_3061 for row in references if isinstance(row, dict)):
        references.append({"reference": KOMODO_3061})
...
def _append_event(doc: dict[str, Any]) -> None:
    event = {
        ...
        "notes": (
            ...
            "3061 metabolite table; preserved H2SO4 as a variable listed "
            "component; kept KOH and NaOH as pH adjusters only; and grounded all "
            "disclosed components through MediaIngredientMech."
        ),
    }
