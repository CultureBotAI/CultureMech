#!/usr/bin/env python3
"""Repair empty KOMODO 199 DESULFOVIBRIO SAPOVORANS MEDIUM."""
...
TARGET = Path("bacterial/desulfovibrio_sapovorans_medium.yaml")
EXPECTED_ID = "CultureMech:004260"
EXPECTED_MEDIA_TERM = "komodo.medium:199"
YAML_LOADER = getattr(yaml, "CSafeLoader", yaml.SafeLoader)

KOMODO_199_URL = (
    "https://komodo.modelseed.org/servlet/" "KomodoTomcatServerSideUtilitiesModelSeed?MediaInfo=199"
)
DSMZ_199_URL = (
    "https://web.archive.org/web/20121030082653id_/"
    "http://www.dsmz.de/microorganisms/medium/pdf/DSMZ_Medium199.pdf"
)

SOURCE_199 = "Archived DSMZ Medium 199"

CURATOR = "repair_komodo_199_score30.py"
ACTION = "RESOLVED_KOMODO_199_SCORE30"
TIMESTAMP = "2026-09-10T00:00:00-07:00"
NOTES = (
    "Archived DSMZ Medium 199 defines DESULFOVIBRIO SAPOVORANS MEDIUM as "
    "DSMZ Medium 193 with NaCl lowered to 1.0 g/L, MgCl2 x 6 H2O lowered "
    "to 0.4 g/L, a 90% N2 + 10% CO2 gas mixture, and the DSMZ 193 acetate "
    "substrate replaced by sodium butyrate, sodium caproate, and sodium "
    "octanoate; this record expands archived DSMZ Media 199, 193, 320, and "
    "141 into final per-liter components."
)


def _iter_komodo_199_components() -> tuple[Component, ...]:
        ...
        if component.preferred_term == "NaCl":
            components.append(
                Component(
                    ...
                    SOURCE_199,
                    f"{SOURCE_199} lowers the DSMZ Medium 193 NaCl to 1.0 g/L.",
                )
            )
            continue
        if component.preferred_term == "MgCl2 x 6 H2O":
            components.append(
                Component(
                    ...
                    SOURCE_199,
                    (f"{SOURCE_199} lowers the DSMZ Medium 193 " "MgCl2 x 6 H2O to 0.4 g/L."),
                )
            )
            continue
            ...
            components.extend(
                (
                    Component(
                        ...
                        SOURCE_199,
                        (
                            f"{SOURCE_199} replaces the acetate substrate "
                            "with 0.70 g/L sodium butyrate."
                        ),
                    ),
                    Component(
                        "Sodium caproate",
                    ...
                    Component(
                        ...
                        (
                            f"{SOURCE_199} replaces the acetate substrate "
                            "with 0.15 g/L sodium octanoate."
                        ),
                    ),
                )
            )
            continue
        if component.preferred_term == "CO2":
            components.append(
                Component(
                    "CO2",
                    "variable",
                    ...
                    SOURCE_199,
                    f"{SOURCE_199} uses a gas mixture of 90% N2 and 10% CO2.",
                )
            )
            continue
        if component.preferred_term == "N2":
            components.append(
                Component(
                    "N2",
                    "variable",
    ...
    components.append(
        Component(
            ...
            (
                ...
                f"additions; {SOURCE_199} replaces the DSMZ Medium 193 "
                "acetate substrate and gas mixture."
            ),
        )
    )
...
COMPONENTS = _iter_komodo_199_components()
...
def _ensure_references(doc: dict[str, Any]) -> None:
    doc["references"] = [
        {"reference": KOMODO_199_URL},
        {"reference": DSMZ_199_URL},
