#!/usr/bin/env python3
"""Repair KOMODO 1264 Spirochaeta exact and strain child links."""
...
PARENT = Path("bacterial/spirochaeta_sp_medium.yaml")
PARENT_ID = "CultureMech:000723"
PARENT_NAME = "spirochaeta_sp_medium"
PARENT_SOURCE_TERM = "mediadive.medium:1264"
...
SIGNATURE = (
    ...
    ("MgCl2 x 2 H2O", "3", "G_PER_L"),
    ("Starch", "5", "G_PER_L"),
    ("Yeast extract", "1", "G_PER_L"),
    ("Peptone", "5", "G_PER_L"),
    ("Phosphate buffer", "1", "G_PER_L"),
    ("Resazurin", "0.5", "G_PER_L"),
    ("Na2S", "0.003", "G_PER_L"),
    ("Vitamine B12", "50", "G_PER_L"),
    ("EDTA", "0.5", "G_PER_L"),
...
CHILDREN = (
    Child(
        Path("bacterial/KOMODO_1264_SPIROCHAETA_SP._medium.yaml"),
        "CultureMech:004021",
        "komodo.medium:1264",
        "KOMODO Medium 1264",
        SOURCE_DUPLICATE_RELATIONSHIP,
        "KOMODO Medium 1264 has the same 20-ingredient signature as DSMZ Medium 1264.",
    ),
    Child(
        Path("bacterial/for_dsm_14994.yaml"),
        "CultureMech:004020",
        "komodo.medium:1264.1",
        "KOMODO Medium 1264.1",
        STRAIN_RELATIONSHIP,
        "KOMODO Medium 1264.1 applies DSMZ Medium 1264 to DSM 14994.",
    ),
)
CHILD_BY_PATH = {child.path: child for child in CHILDREN}
EXPECTED_CHILD_COUNT = 2

CURATOR = "repair_komodo_1264_spirochaeta_score10.py"
ACTION = "RESOLVED_KOMODO_1264_SPIROCHAETA_TOPOLOGY"
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Re-rooted KOMODO Medium 1264 Spirochaeta exact topology",
            "source": "DSMZ Medium 1264; KOMODO Medium 1264 and 1264.1",
            "notes": (
                "Moved exact KOMODO Medium 1264 plus its DSM 14994 wrapper "
                "under DSMZ Medium 1264."
            ),
        },
    )
    return repaired
...
def repair_child(relative_path: Path, doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": f"Linked under DSMZ Medium 1264 as {child.relationship}",
