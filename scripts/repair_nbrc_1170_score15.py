#!/usr/bin/env python3
"""Repair the recovered NBRC Medium 1169 Methanohalobium record."""
...
CURATOR = "repair_nbrc_1170_score15.py"
ACTION = "RESOLVED_NBRC_1170_SCORE15"
TIMESTAMP = "2026-09-11T00:00:00-07:00"

TARGET_PATH = "bacterial/NBRC_1170.yaml"
TARGET_ID = "CultureMech:007461"
REQUIRED_ACTION = "Recovered composition from source HTML"

SOURCE = "NBRC Medium 1169"
NBRC_URL = "https://www.nite.go.jp/nbrc/catalogue/NBRCMediumDetailServlet?NO=1169"
TITLE = "Methanohalobium medium"
...
IMPORTED_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
    ...
    ("KH2PO4", "0.33", "G_PER_L"),
    ("KCl", "0.33", "G_PER_L"),
    ("CaCl2·2H2O", "0.33", "G_PER_L"),
    ("MgCl2·6H2O", "0.33", "G_PER_L"),
    ("MgSO4·7H2O", "4", "G_PER_L"),
    ("NaCl", "250", "G_PER_L"),
    ("Coenzyme M", "0.142", "G_PER_L"),
    ("Bacto Yeast Extract (Difco)", "0.5", "G_PER_L"),
    ("Vitamin solution*", "10", "ML_PER_L"),
    ("Trace elements solution**", "1", "ML_PER_L"),
    ("Trimethylamine-HCl", "5", "G_PER_L"),
...
PREPARATION_STEPS: tuple[dict[str, Any], ...] = (
    {
        ...
        "action": "DISSOLVE",
        "description": (
            "Mix all ingredients except vitamin solution, trimethylamine-HCl, "
            "NaHCO3, and Na2S·9H2O."
        ),
    },
    {
        "step_number": 2,
    ...
    },
    {
        "step_number": 3,
        "action": "AUTOCLAVE",
        "description": "Separately autoclave Na2S·9H2O as a 5% solution under N2.",
    },
    {
        "step_number": 4,
        "action": "FILTER_STERILIZE",
        "description": (
            "Filter-sterilize the vitamin solution, 50% trimethylamine-HCl "
            "solution, 5% NaHCO3 solution, and 5% Na2CO3 solution."
        ),
    },
    {
        "step_number": 5,
        "action": "MIX",
        "description": (
            "Prior to inoculation, add the filter-sterile vitamin, "
            "trimethylamine-HCl, and NaHCO3 solutions and the Na2S·9H2O solution."
        ),
    },
    {
        "step_number": 6,
        "action": "ADJUST_PH",
        "description": (
            "Adjust pH of the medium to 7.4 with filter-sterile 5% Na2CO3 "
            "solution, approximately 0.5 g/L Na2CO3 final concentration."
        ),
    },
    {
        "step_number": 7,
...
NOTES = (
    "Source: NBRC Medium 1169 | Link: "
    "https://www.nite.go.jp/nbrc/catalogue/NBRCMediumDetailServlet?NO=1169\n\n"
    "NBRC lists Methanohalobium medium with mineral salts, Coenzyme M, Bacto "
    "Yeast Extract, vitamin and trace-element stocks, trimethylamine-HCl, "
    "NaHCO3, Na2S, Na2CO3 adjustment to pH 7.4, and anaerobic N2/CO2 "
    "preparation."
)
...
def _component(preferred_term: str, value: str, unit: str) -> dict[str, Any]:
    row: dict[str, Any] = {
        ...
        "source": SOURCE,
        "notes": (
            "NBRC Medium 1169 uses NaOH to adjust the trace elements solution to "
            "pH 6.5 before mineral addition."
            if preferred_term == "NaOH"
            else (
                "NBRC Medium 1169 adjusts the final medium to pH 7.4 with "
                "filter-sterile 5% Na2CO3, approximately 0.5 g/L final Na2CO3."
            )
        ),
    }
...
def _ensure_target(doc: dict[str, Any]) -> None:
    ...
    if doc.get("name") not in {"1170", TITLE}:
...
def _ensure_main_na2co3(doc: dict[str, Any]) -> None:
    ingredients = doc.get("ingredients")
    if not isinstance(ingredients, list):
        raise ValueError("ingredients is not a list")
    if not any(
        isinstance(row, dict) and row.get("preferred_term") == "Na2CO3" for row in ingredients
    ):
        ingredients.append(_component("Na2CO3", "0.5", "G_PER_L"))
...
def _ensure_event(doc: dict[str, Any]) -> None:
    event = {
        ...
        "curator": CURATOR,
        "action": ACTION,
        "changes": "Grounded recovered NBRC Medium 1169 Methanohalobium formula",
        "source": NBRC_URL,
        "notes": (
            "Added the NBRC Medium 1169 source term and official title, "
            "grounded the recovered Methanohalobium medium formula and stocks, "
            "recorded pH 7.4, and converted anaerobic preparation instructions "
...
def repair_document(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _put_after(
        ...
        "media_term",
        {
            ...
            "term": _term("nbrc.medium:1169", SOURCE),
        },
        "physical_state",
    )
    _put_after(repaired, "ph_value", 7.4, "media_term")
    _put_after(repaired, "notes", NOTES, "description")

    repaired["preparation_steps"] = copy.deepcopy(list(PREPARATION_STEPS))
    repaired["sterilization"] = {"method": "AUTOCLAVE"}

    _ensure_main_na2co3(repaired)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"{action} {changed_count} NBRC 1170 records")
    return 0
