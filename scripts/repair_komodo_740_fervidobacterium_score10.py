#!/usr/bin/env python3
"""Repair KOMODO 740 Fervidobacterium exact and strain child links."""
...
PARENT = Path("bacterial/fervidobacterium_tf_a_medium.yaml")
PARENT_ID = "CultureMech:001876"
PARENT_NAME = "fervidobacterium_tf_a_medium"
PARENT_SOURCE_TERM = "mediadive.medium:740"
SOURCE_DUPLICATE_RELATIONSHIP = "SOURCE_DUPLICATE"
STRAIN_RELATIONSHIP = "STRAIN_SPECIFIC_VARIANT"

SIGNATURE = (
    ("NH4Cl", "0.49456", "G_PER_L"),
    ("MgSO4 x 7 H2O", "3.158259", "G_PER_L"),
    ("K2HPO4", "1.58259", "G_PER_L"),
    ("Na2HPO4 x H2O", "0.98912", "G_PER_L"),
    ...
    ("D-Glucose", "2.96736", "G_PER_L"),
    ("L-Cysteine HCl x H2O", "0.296736", "G_PER_L"),
    ("Na2S x 9 H2O", "0.296736", "G_PER_L"),
...
CHILDREN = (
    Child(
        Path("bacterial/tf_a_medium.yaml"),
        "CultureMech:006391",
        "komodo.medium:740",
        "KOMODO Medium 740",
        SOURCE_DUPLICATE_RELATIONSHIP,
        "KOMODO Medium 740 has the same 34-ingredient signature as DSMZ Medium 740.",
    ),
    Child(
        Path("bacterial/for_dsm_17883.yaml"),
        "CultureMech:006390",
        "komodo.medium:740.1",
        "KOMODO Medium 740.1",
        STRAIN_RELATIONSHIP,
        "KOMODO Medium 740.1 applies DSMZ Medium 740 to DSM 17883.",
    ),
)
CHILD_BY_PATH = {child.path: child for child in CHILDREN}
EXPECTED_CHILD_COUNT = 2

CURATOR = "repair_komodo_740_fervidobacterium_score10.py"
ACTION = "RESOLVED_KOMODO_740_FERVIDOBACTERIUM_TOPOLOGY"
...
def repair_child(relative_path: Path, doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": f"Linked under DSMZ Medium 740 as {child.relationship}",
