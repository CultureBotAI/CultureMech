#!/usr/bin/env python3
"""Repair KOMODO 480c Desulfurella II strain child links."""
...
PARENT = Path("bacterial/desulfurella_ii_medium.yaml")
PARENT_ID = "CultureMech:005601"
PARENT_NAME = "desulfurella_ii_medium"
PARENT_SOURCE_TERM = "komodo.medium:480c"
...
CHILDREN = (
    Child(
        Path("bacterial/dsm_10409.yaml"),
        "CultureMech:005599",
        "komodo.medium:480c.1",
        "KOMODO Medium 480c.1",
        "DSM 10409",
    ),
    Child(
        Path("bacterial/dsm_10410.yaml"),
        "CultureMech:005600",
        "komodo.medium:480c.2",
        "KOMODO Medium 480c.2",
        "DSM 10410",
    ),
)
...
CURATOR = "repair_komodo_480c_desulfurella_score10.py"
ACTION = "RESOLVED_KOMODO_480C_DESULFURELLA_TOPOLOGY"
...
def _child_notes(child: Child) -> str:
    return f"{child.source_label} applies DESULFURELLA II medium to {child.target}."
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Resolved KOMODO Medium 480c strain wrappers",
            "source": "KOMODO Medium 480c.1 and 480c.2",
            "notes": (
                "Moved the exact-signature DSM-specific KOMODO Medium 480c "
                "wrappers under the KOMODO Medium 480c Desulfurella II base."
            ),
        },
    )
    return repaired
...
def repair_child(relative_path: Path, doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a KOMODO Medium 480c strain-specific variant",
