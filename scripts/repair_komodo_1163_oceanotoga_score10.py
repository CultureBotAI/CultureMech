#!/usr/bin/env python3
"""Repair KOMODO 1163 Oceanotoga strain child links."""
...
PARENT = Path("bacterial/oceanotoga_medium.yaml")
PARENT_ID = "CultureMech:000604"
PARENT_NAME = "oceanotoga_medium"
PARENT_SOURCE_TERM = "mediadive.medium:1163a"
RELATIONSHIP = "STRAIN_SPECIFIC_VARIANT"

INGREDIENT_SIGNATURE = (
    ("NaCl", "30.6736", "G_PER_L"),
    ("HEPES", "4.64886", "G_PER_L"),
    ("KCl", "0.326409", "G_PER_L"),
    ("MgCl2 x 2 H2O", "0.890208", "G_PER_L"),
    ("MgSO4 x 7 H2O", "4.38477", "G_PER_L"),
    ("NH4Cl", "0.24728", "G_PER_L"),
    ("CaCl2 x 2 H2O", "0.238477", "G_PER_L"),
    ("KH2PO4", "0.445104", "G_PER_L"),
    ("Yeast extract", "4.9456", "G_PER_L"),
    ("Sodium resazurin", "0.00049456", "G_PER_L"),
    ("L-Cysteine HCl x H2O", "0.49456", "G_PER_L"),
    ("D-Glucose", "4.9456", "G_PER_L"),
...
CHILDREN = (
    Child(
        Path("bacterial/for_dsm_15011_and_dsm_24906.yaml"),
        "CultureMech:003874",
        "komodo.medium:1163.1",
        "KOMODO Medium 1163.1",
        "DSM 15011 and DSM 24906",
    ),
    Child(
        Path("bacterial/for_dsm_24739.yaml"),
        "CultureMech:003875",
        "komodo.medium:1163.2",
        "KOMODO Medium 1163.2",
        "DSM 24739",
    ),
)
...
CURATOR = "repair_komodo_1163_oceanotoga_score10.py"
ACTION = "RESOLVED_KOMODO_1163_OCEANOTOGA_TOPOLOGY"
...
def _child_notes(child: Child) -> str:
    return f"{child.source_label} applies OCEANOTOGA MEDIUM to {child.target}."
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Promoted DSMZ Medium 1163a as the Oceanotoga parent",
            "source": "KOMODO Medium 1163.1 and 1163.2; DSMZ Medium 1163a",
            "notes": (
                "Moved the KOMODO Medium 1163.1 and 1163.2 strain wrappers "
                "under the exact-signature DSMZ Medium 1163a Oceanotoga base."
            ),
        },
    )
    return repaired
...
def repair_child(relative_path: Path, doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked under DSMZ Medium 1163a",
