#!/usr/bin/env python3
"""Repair KOMODO 3067 Trace element solution (medium 1263)."""
...
TARGET = Path("bacterial/trace_element_solution_medium_1263.yaml")
YAML_LOADER = getattr(yaml, "CSafeLoader", yaml.SafeLoader)

CURATOR = "repair_komodo_3067_score15.py"
ACTION = "RESOLVED_KOMODO_3067_SCORE15"
TIMESTAMP = "2026-09-12T00:00:00-07:00"

EXPECTED_ID = "CultureMech:004851"
EXPECTED_MEDIA_TERM = "komodo.medium:3067"
KOMODO_3067 = (
    "https://komodo.modelseed.org/servlet/KomodoTomcatServerSideUtilitiesModelSeed?MediaInfo=3067"
)
SOURCE = "KOMODO Medium 3067"
...
FINAL_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
    ...
    ("MnSO4 x H2O", "0.10", "G_PER_L"),
    ("ZnSO4 x 7 H2O", "0.10", "G_PER_L"),
    ("KOH", "variable", "VARIABLE"),
    ("Distilled water", "1.0", "L"),
    ("CuSO4 x 5 H2O", "0.01", "G_PER_L"),
    ("Fe(NH4)2(SO4)2 x 6 H2O", "0.20", "G_PER_L"),
    ("Nitrilotriacetic acid", "1.50", "G_PER_L"),
    ("Na2MoO4 x H2O", "0.10", "G_PER_L"),
    ("Na2WO4 x 2 H2O", "1.00", "G_PER_L"),
)
...
GROUNDINGS: dict[str, tuple[str, str]] = {
    ...
    "Na2MoO4 x H2O": ("CHEBI:75215", "sodium molybdate (anhydrous)"),
    "Na2WO4 x 2 H2O": ("CHEBI:63939", "sodium tungstate dihydrate"),
    "NaSeO3": ("CHEBI:48843", "disodium selenite"),
...
NOTES = (
    "KOMODO 3067 lists Trace element solution (medium 1263) as a defined "
    "submedium at pH 7.0. Its metabolite table contains 1.50 g/L "
    "nitrilotriacetic acid, 1.00 g/L Na2WO4 x 2 H2O, 0.20 g/L NaSeO3, "
    "0.20 g/L Fe(NH4)2(SO4)2 x 6 H2O, 0.10 g/L MnSO4 x H2O, 0.10 g/L "
    "ZnSO4 x 7 H2O, 0.10 g/L Na2MoO4 x H2O, 0.02 g/L NiCl2 x 6 H2O, "
    "0.01 g/L CuSO4 x 5 H2O, variable KOH, and distilled water."
)
...
PREPARATION_STEPS: tuple[dict[str, Any], ...] = (
    {
        ...
        "action": "MIX",
        "description": (
            "Prepare the 1 L Trace element solution stock from distilled water, "
            "nitrilotriacetic acid, NaSeO3, the nickel, manganese, zinc, copper, "
            "iron, molybdate, and tungstate salts listed by KOMODO 3067, and KOH."
        ),
    },
    {
        "step_number": 2,
        "action": "ADJUST_PH",
        "description": "Adjust the Trace element solution stock to pH 7.0 with KOH as needed.",
    },
)
...
def _component(preferred_term: str, value: str, unit: str) -> dict[str, Any]:
    ...
    if preferred_term == "Distilled water":
        row["notes"] = (
            f"{SOURCE} lists distilled water without a gram amount; normalized here "
            "as the solvent for 1 L of Trace element solution (medium 1263)."
        )
    ...
    elif preferred_term == "NaSeO3":
        row["notes"] = (
            f"{SOURCE} lists 0.20 g/L NaSeO3, with SEED identifiers for sodium "
            "and selenite; normalized here to the disodium selenite CHEBI term."
        )
...
def _ensure_references(doc: dict[str, Any]) -> None:
    ...
    if not any(row.get("reference") == KOMODO_3067 for row in references if isinstance(row, dict)):
        references.append({"reference": KOMODO_3067})
...
def _append_event(doc: dict[str, Any]) -> None:
    event = {
        ...
        "notes": (
            ...
            "3067 metabolite table; preserved KOH as a variable listed "
