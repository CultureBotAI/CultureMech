#!/usr/bin/env python3
"""Repair KOMODO 2051 Trace element solution (medium 796)."""
...
TARGET = Path("bacterial/trace_element_solution_medium_796.yaml")
YAML_LOADER = getattr(yaml, "CSafeLoader", yaml.SafeLoader)

CURATOR = "repair_komodo_2051_score15.py"
ACTION = "RESOLVED_KOMODO_2051_SCORE15"
TIMESTAMP = "2026-09-12T00:00:00-07:00"

EXPECTED_ID = "CultureMech:004324"
EXPECTED_MEDIA_TERM = "komodo.medium:2051"
KOMODO_2051 = (
    "https://komodo.modelseed.org/servlet/KomodoTomcatServerSideUtilitiesModelSeed?MediaInfo=2051"
)
SOURCE = "KOMODO Medium 2051"
...
GROUNDINGS: dict[str, tuple[str, str]] = {
    "(NH4)2Ni(SO4)2 x 6 H2O": ("CHEBI:86149", "ammonium nickel sulfate hexahydrate"),
...
NOTES = (
    "KOMODO 2051 lists Trace element solution (medium 796) as a defined "
    "submedium at pH 1.0. Its metabolite table contains 10.00 g/L NaCl, "
    "5.80 g/L MnCl2 x 4 H2O, 2.00 g/L (NH4)2Ni(SO4)2 x 6 H2O, "
    "1.00 g/L CoCl2 x 6 H2O, 1.00 g/L CaCl2 x 2 H2O, "
    "1.00 g/L FeSO4 x 7 H2O, 1.00 g/L ZnSO4 x 7 H2O, "
    "0.10 g/L CuSO4 x 5 H2O, 0.10 g/L Na2SeO3 x 5 H2O, "
    "0.10 g/L H3BO3, 0.10 g/L Na2MoO4 x 2 H2O, "
    "0.10 g/L KAl(SO4)2 x 12 H2O, 0.10 g/L Na2WO4 x 2 H2O, "
...
PREPARATION_STEPS: tuple[dict[str, Any], ...] = (
    {
        ...
        "action": "MIX",
        "description": (
            "Prepare the 1 L Trace element solution stock from H2O, HCl, "
            "NaCl, the cobalt, ammonium nickel sulfate, calcium, manganese, "
            "copper, iron, selenite, borate, molybdate, alum, tungstate, and "
            "zinc salts listed by KOMODO 2051."
        ),
    },
    {
        "step_number": 2,
        "action": "ADJUST_PH",
        "description": "Adjust the Trace element solution stock to pH 1.0 with HCl as needed.",
    },
)
...
def _component(preferred_term: str, value: str, unit: str) -> dict[str, Any]:
    ...
    if preferred_term == "H2O":
        row["notes"] = (
            ...
            "the solvent for 1 L of Trace element solution (medium 796)."
        )
    elif preferred_term == "HCl":
        row["notes"] = (
            f"{SOURCE} lists HCl without a gram amount and also identifies "
            "HCl as the pH adjuster for pH 1.0."
        )
...
def _ensure_references(doc: dict[str, Any]) -> None:
    ...
    if not any(row.get("reference") == KOMODO_2051 for row in references if isinstance(row, dict)):
        references.append({"reference": KOMODO_2051})
...
def _append_event(doc: dict[str, Any]) -> None:
    event = {
        ...
        "source": KOMODO_2051,
        "notes": (
            "Replaced the HCl-only pH-buffer import with the complete KOMODO "
            "2051 metabolite table; preserved HCl as a variable listed "
...
def repair_record(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _put_after(repaired, "ph_value", 1.0, "physical_state")
