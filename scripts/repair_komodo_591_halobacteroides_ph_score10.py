#!/usr/bin/env python3
"""Repair a KOMODO Halobacteroides pH variant mislinked as a duplicate."""
...
PARENT = Path("bacterial/KOMODO_591_HALOBACTEROIDES_HALOINCOLA_medium.yaml")
PARENT_ID = "CultureMech:006073"
PARENT_NAME = "halobacteroides_haloincola_medium"
PARENT_SOURCE_TERM = "komodo.medium:591"
PARENT_PH = 7.5
...
INGREDIENT_SIGNATURE = (
    ...
    ("Calcium D-(+)-pantothenate", "0.005", "G_PER_L"),
    ("Vitamin B12", "0.0001", "G_PER_L"),
    ("p-Aminobenzoic acid", "0.005", "G_PER_L"),
    ("(DL)-alpha-Lipoic acid", "0.005", "G_PER_L"),
)

CURATOR = "repair_komodo_591_halobacteroides_ph_score10.py"
ACTION = "RESOLVED_KOMODO_591_HALOBACTEROIDES_PH_VARIANT"
...
class Child:
    ...
    def modification(self) -> str:
        return (
            f"{self.source_label} preserves HALOBACTEROIDES/HALOINCOLA medium "
    ...
    def notes(self) -> str:
        return (
            f"{self.source_label} applies HALOBACTEROIDES/HALOINCOLA medium at "
...
CHILDREN = (
    Child(
        path=Path("bacterial/for_dsm_12596.yaml"),
        record_id="CultureMech:006072",
        name="for_dsm_12596",
        source_term="komodo.medium:591.2",
        ph_value=6.6,
        dsm="DSM 12596",
    ),
)
...
def _parent_ref(child: Child) -> dict[str, str]:
    return {
        ...
        "name": PARENT_NAME,
        "notes": (
            f"KOMODO Medium 591 is the pH {PARENT_PH:g} " "HALOBACTEROIDES/HALOINCOLA medium base."
        ),
    }
...
def _require_halobacteroides_record(
    doc: dict[str, Any],
    relative_path: Path,
    record_id: str,
    source_term: str,
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_halobacteroides_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM, PARENT_PH)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked a HALOBACTEROIDES/HALOINCOLA medium pH variant child",
            "source": "KOMODO Medium 591.2",
            "notes": (
                "Changed DSM 12596 from SOURCE_DUPLICATE to PH_VARIANT " "under KOMODO Medium 591."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any], child: Child) -> dict[str, Any]:
    _require_halobacteroides_record(
        doc,
        child.path,
        child.record_id,
        child.source_term,
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a HALOBACTEROIDES/HALOINCOLA medium pH variant",
