#!/usr/bin/env python3
"""Repair KOMODO Natranaerobius pH variants mislinked as duplicates."""
...
PARENT = Path("bacterial/KOMODO_1095_NATRANAEROBIUS_medium.yaml")
PARENT_ID = "CultureMech:003747"
PARENT_NAME = "natranaerobius_medium"
PARENT_SOURCE_TERM = "komodo.medium:1095"
PARENT_PH = 8.5

INGREDIENT_SIGNATURE = (
    ("NaCl", "99.8004", "G_PER_L"),
    ("KH2PO4", "0.199601", "G_PER_L"),
    ("MgCl2", "0.0998004", "G_PER_L"),
    ("NH4Cl", "0.499002", "G_PER_L"),
    ("Yeast extract", "9.98004", "G_PER_L"),
    ("Tryptone", "9.98004", "G_PER_L"),
    ("Na2CO3", "67.8643", "G_PER_L"),
    ("NaHCO3", "37.9242", "G_PER_L"),
    ("L-Cysteine HCl x H2O", "0.698603", "G_PER_L"),
    ("Sucrose", "4.99002", "G_PER_L"),
    ("HCl", "2.5", "G_PER_L"),
...
CURATOR = "repair_komodo_1095_natranaerobius_ph_score10.py"
ACTION = "RESOLVED_KOMODO_1095_NATRANAEROBIUS_PH_VARIANTS"
...
class Child:
    ...
    def modification(self) -> str:
        return (
            f"{self.source_label} preserves NATRANAEROBIUS medium components "
    ...
    def notes(self) -> str:
        return (
            f"{self.source_label} applies NATRANAEROBIUS medium at "
...
CHILDREN = (
    Child(
        path=Path("bacterial/for_dsm_18760.yaml"),
        record_id="CultureMech:003745",
        name="for_dsm_18760",
        source_term="komodo.medium:1095.1",
        ph_value=9.5,
        dsm="DSM 18760",
    ),
    Child(
        path=Path("bacterial/for_dsm_18770.yaml"),
        record_id="CultureMech:003746",
        name="for_dsm_18770",
        source_term="komodo.medium:1095.2",
        ph_value=9.7,
        dsm="DSM 18770",
    ),
)
...
def _require_natranaerobius_record(
    doc: dict[str, Any],
    relative_path: Path,
    record_id: str,
    source_term: str,
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_natranaerobius_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM, PARENT_PH)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked NATRANAEROBIUS medium pH variant children",
            "source": "KOMODO Medium 1095.1 and KOMODO Medium 1095.2",
            "notes": (
                "Changed DSM 18760 and DSM 18770 from SOURCE_DUPLICATE to "
                "PH_VARIANT under KOMODO Medium 1095."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any], child: Child) -> dict[str, Any]:
    _require_natranaerobius_record(
        doc,
        child.path,
        child.record_id,
        child.source_term,
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a NATRANAEROBIUS medium pH variant",
