#!/usr/bin/env python3
"""Repair a KOMODO SYLC strain variant mislinked as a duplicate."""
...
PARENT = Path("bacterial/sylc_medium.yaml")
PARENT_ID = "CultureMech:003598"
PARENT_NAME = "sylc_medium"
PARENT_SOURCE_TERM = "komodo.medium:1040"
PH_RANGE = {"min": 7.0, "max": 7.2}
PH_LABEL = "7.0-7.2"

INGREDIENT_SIGNATURE = (
    ("Sea Salt", "34.9301", "G_PER_L"),
...
CURATOR = "repair_komodo_1040_sylc_score10.py"
ACTION = "RESOLVED_KOMODO_1040_SYLC_STRAIN_VARIANT"
...
class Child:
    ...
    def modification(self) -> str:
        return (
            f"{self.source_label} preserves SYLC medium components and pH {PH_LABEL} "
            f"but applies the recipe for {self.dsm}."
        )

    @property
    def notes(self) -> str:
        return f"{self.source_label} applies SYLC medium at pH {PH_LABEL} for {self.dsm}."


CHILD = Child(
    path=Path("bacterial/for_dsm_17682.yaml"),
    record_id="CultureMech:003597",
    name="for_dsm_17682",
    source_term="komodo.medium:1040.1",
    dsm="DSM 17682",
)
...
def _parent_ref(child: Child) -> dict[str, str]:
    return {
        ...
        "id": PARENT_ID,
        "name": PARENT_NAME,
        "notes": f"KOMODO Medium 1040 is the pH {PH_LABEL} SYLC medium base.",
    }
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_sylc_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked a SYLC strain-specific child",
            "source": CHILD.source_label,
            "notes": (
                "Changed DSM 17682 from SOURCE_DUPLICATE to a "
                "STRAIN_SPECIFIC_VARIANT child of KOMODO Medium 1040."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any], child: Child = CHILD) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a SYLC strain-specific variant",
            "source": child.source_label,
