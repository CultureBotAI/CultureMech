#!/usr/bin/env python3
"""Repair KOMODO 480.1 Desulfurella strain child link."""
...
PARENT = Path("bacterial/KOMODO_480_DESULFURELLA_medium.yaml")
PARENT_ID = "CultureMech:005596"
PARENT_NAME = "desulfurella_medium"
PARENT_SOURCE_TERM = "komodo.medium:480"

SOURCE_DUPLICATE_PARENT = Path("bacterial/KOMODO_480b_TTD-MEDIUM.yaml")
SOURCE_DUPLICATE_PARENT_ID = "CultureMech:005598"
SOURCE_DUPLICATE_PARENT_SOURCE_TERM = "komodo.medium:480b"
EXPECTED_SOURCE_DUPLICATE_CHILD_COUNT = 4

CHILD = Path("bacterial/for_dsm_11321.yaml")
CHILD_ID = "CultureMech:005595"
CHILD_SOURCE_TERM = "komodo.medium:480.1"
RELATIONSHIP = "STRAIN_SPECIFIC_VARIANT"
CHILD_NOTES = "KOMODO Medium 480.1 applies KOMODO Medium 480 to DSM 11321."
...
INGREDIENT_SIGNATURE = (
    ...
    ("MOPS buffer", "3.89222", "G_PER_L"),
    ("Yeast extract", "0.0998004", "G_PER_L"),
    ("Sodium resazurin", "0.000499002", "G_PER_L"),
    ("Sulfur", "9.98004", "G_PER_L"),
    ("Na-DL-lactate", "2.49501", "G_PER_L"),
    ("Na2CO3", "0.998004", "G_PER_L"),
...
CURATOR = "repair_komodo_480_desulfurella_score10.py"
ACTION = "RESOLVED_KOMODO_480_DESULFURELLA_TOPOLOGY"
...
def _child_refers_to_strain_pointer(row: dict[str, Any]) -> bool:
    return (
        row.get("id") == CHILD_ID
        or row.get("path") == f"data/normalized_yaml/{CHILD}"
        or row.get("name") == CHILD.stem
    )
...
def _parent_ref() -> dict[str, str]:
    return {
        ...
        "id": PARENT_ID,
        "name": PARENT_NAME,
        "notes": "KOMODO Medium 480 is the DESULFURELLA base for KOMODO Medium 480.1.",
    }
...
def _without_strain_pointer_child(doc: dict[str, Any]) -> list[dict[str, Any]]:
    _require_record(
        doc,
        SOURCE_DUPLICATE_PARENT,
        SOURCE_DUPLICATE_PARENT_ID,
        SOURCE_DUPLICATE_PARENT_SOURCE_TERM,
    )

    children = doc.get("variant_children") or []
    if not isinstance(children, list):
        raise ValueError(f"{SOURCE_DUPLICATE_PARENT}: variant_children is not a list")
    ...
    for row in children:
        if not isinstance(row, dict):
            raise ValueError(
                f"{SOURCE_DUPLICATE_PARENT}: variant_children contains a non-mapping row"
            )
        if _child_refers_to_strain_pointer(row):
            removed += 1
            continue
        filtered.append(row)

    if removed > 1:
        raise ValueError(f"{SOURCE_DUPLICATE_PARENT}: duplicate {CHILD} child entries")
    if len(filtered) != EXPECTED_SOURCE_DUPLICATE_CHILD_COUNT:
        raise ValueError(
            f"{SOURCE_DUPLICATE_PARENT}: expected "
            f"{EXPECTED_SOURCE_DUPLICATE_CHILD_COUNT} remaining child entries"
        )
    return filtered


def repair_source_duplicate_parent(doc: dict[str, Any]) -> dict[str, Any]:
    children = _without_strain_pointer_child(doc)

    repaired = copy.deepcopy(doc)
    repaired["variant_children"] = copy.deepcopy(children)
    _upsert_event(
        repaired,
        {
            "timestamp": TIMESTAMP,
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Moved KOMODO Medium 480.1 out of the 480b duplicate set",
            "source": "KOMODO Medium 480, 480.1, and 480b",
            "notes": (
                "KOMODO Medium 480.1 points to DSMZ Medium 480, so its "
                "strain-specific wrapper belongs under KOMODO Medium 480 "
                "instead of the KOMODO Medium 480b exact-duplicate parent."
            ),
        },
    )
    return repaired
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Reclassified KOMODO Medium 480.1 as a strain variant",
            "source": "KOMODO Medium 480 and 480.1",
            "notes": (
                "Linked KOMODO Medium 480.1 under the KOMODO Medium 480 "
                "DESULFURELLA base while preserving the 480b duplicate parent "
                "relationship."
            ),
        },
    )
...
def repair_child(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked under KOMODO Medium 480",
            "source": "KOMODO Medium 480.1",
            "notes": CHILD_NOTES,
        },
    )
    return repaired
