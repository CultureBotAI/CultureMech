#!/usr/bin/env python3
"""Repair TOGO M2013 LB + Nalidixic acid, Ciprofloxacin medium."""
...
TARGET = Path("bacterial/lb_nalidixic_acid_ciprofloxacin_medium.yaml")
LB_PARENT = Path("bacterial/lb_medium.yaml")
YAML_LOADER = getattr(yaml, "CSafeLoader", yaml.SafeLoader)

CURATOR = "repair_togo_m2013_lb_nalidixic_ciprofloxacin_score15.py"
ACTION = "RESOLVED_TOGO_M2013_LB_NALIDIXIC_CIPROFLOXACIN_SCORE15"
TIMESTAMP = "2026-09-12T00:00:00-07:00"

EXPECTED_ID = "CultureMech:008600"
EXPECTED_MEDIA_TERM = "TOGO:M2013"
LB_PARENT_ID = "CultureMech:008037"
TOGO_M2013 = "https://togomedium.org/medium/M2013"
NBRC_1301 = "https://www.nite.go.jp/nbrc/catalogue/NBRCMediumDetailServlet?NO=1301"
SOURCE = "TOGO M2013 / NBRC Medium 1301"
...
IMPORTED_SOLUTION_SIGNATURE: tuple[SolutionSignature, ...] = (
    ("Ciprofloxacin solution (5 mg/ml)*", "1", "G_PER_L", ()),
    ("Nalidixic acid solution (30 mg/ml)*", "1", "G_PER_L", ()),
)

NALIDIXIC_SIGNATURE: tuple[Component, ...] = (("Nalidixic acid", "30.0", "MG_PER_ML"),)

CIPROFLOXACIN_SIGNATURE: tuple[Component, ...] = (("Ciprofloxacin", "5.0", "MG_PER_ML"),)
...
FINAL_SOLUTION_SIGNATURE: tuple[SolutionSignature, ...] = (
    ("Nalidixic acid solution (30 mg/ml)", "1.0", "ML_PER_L", NALIDIXIC_SIGNATURE),
    ("Ciprofloxacin solution (5 mg/ml)", "1.0", "ML_PER_L", CIPROFLOXACIN_SIGNATURE),
)

GROUNDINGS: dict[str, tuple[str, str]] = {
    "Agar (if needed)": ("CHEBI:2509", "agar"),
    "Ciprofloxacin": ("CHEBI:100241", "ciprofloxacin"),
...
NOTES = (
    "TOGO M2013 imports NBRC Medium 1301 LB + Nalidixic acid, Ciprofloxacin "
    "medium. NBRC 1301 lists the LB base with 1.0 ml/L Nalidixic acid "
    "solution (30 mg/ml), 1.0 ml/L Ciprofloxacin solution (5 mg/ml), "
...
PARENT_MEDIA = {
    ...
    "name": "lb_medium",
    "notes": (
        "NBRC Medium 1301 supplements LB Medium with 1.0 ml/L Nalidixic acid "
        "solution (30 mg/ml) and 1.0 ml/L Ciprofloxacin solution (5 mg/ml), "
...
VARIANT_CHILD = {
    ...
    "relationship": "SUPPLEMENTED_VARIANT",
    "id": EXPECTED_ID,
    "name": "lb_nalidixic_acid_ciprofloxacin_medium",
    "notes": PARENT_MEDIA["notes"],
}

VARIANT_MODIFICATIONS = (
    "Adds 1.0 ml/L Nalidixic acid solution (30 mg/ml) and 1.0 ml/L "
    "Ciprofloxacin solution (5 mg/ml) after separate filter sterilization."
)
...
PREPARATION_STEPS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "description": (
            "Sterilize the 30 mg/ml nalidixic acid and 5 mg/ml ciprofloxacin "
            "stocks separately by filtration."
        ),
    },
    {
        "step_number": 3,
        "action": "MIX",
        "description": (
            "Add 1.0 ml/L filter-sterilized Nalidixic acid solution "
            "(30 mg/ml) and 1.0 ml/L filter-sterilized Ciprofloxacin solution "
            "(5 mg/ml) to the LB base."
        ),
    },
)
...
INGREDIENTS: tuple[dict[str, Any], ...] = (
    _component(
        ...
        "G_PER_L",
        notes=(
            "NBRC Medium 1301 lists Bacto Tryptone (Difco) as a source-disclosed "
...
SOLUTIONS: tuple[dict[str, Any], ...] = (
    {
        "preferred_term": "Nalidixic acid solution (30 mg/ml)",
        "concentration": {"value": "1.0", "unit": "ML_PER_L"},
        "source": SOURCE,
        "notes": f"{SOURCE} lists 1 ml/L Nalidixic acid solution (30 mg/ml).",
        "term": _term(*GROUNDINGS["Nalidixic acid"]),
        "mediaingredientmech_chebi_term": _term(*GROUNDINGS["Nalidixic acid"]),
        "composition": [
            _component(
                "Nalidixic acid",
                "30.0",
                "MG_PER_ML",
                notes="NBRC Medium 1301 specifies this stock as 30 mg/ml Nalidixic acid.",
            )
        ],
        "name": "Nalidixic acid solution (30 mg/ml)",
        "preparation_notes": "Sterilize separately by filtration.",
    },
    {
        "preferred_term": "Ciprofloxacin solution (5 mg/ml)",
        "concentration": {"value": "1.0", "unit": "ML_PER_L"},
        "source": SOURCE,
        "notes": f"{SOURCE} lists 1 ml/L Ciprofloxacin solution (5 mg/ml).",
        "term": _term(*GROUNDINGS["Ciprofloxacin"]),
        "mediaingredientmech_chebi_term": _term(*GROUNDINGS["Ciprofloxacin"]),
        "composition": [
            _component(
                "Ciprofloxacin",
                "5.0",
                "MG_PER_ML",
                notes="NBRC Medium 1301 specifies this stock as 5 mg/ml Ciprofloxacin.",
            )
        ],
        "name": "Ciprofloxacin solution (5 mg/ml)",
        "preparation_notes": "Sterilize separately by filtration.",
    },
)
...
def _ensure_references(doc: dict[str, Any]) -> None:
    ...
    for reference in (TOGO_M2013, NBRC_1301):
...
def _append_event(doc: dict[str, Any]) -> None:
    event = {
        ...
        "notes": (
            ...
            "30 mg/ml nalidixic acid and 5 mg/ml ciprofloxacin stocks."
        ),
    }
