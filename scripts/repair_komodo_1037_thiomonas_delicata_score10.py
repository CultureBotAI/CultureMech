#!/usr/bin/env python3
"""Repair a KOMODO Thiomonas delicata strain variant mislinked as a duplicate."""
...
PARENT = Path("bacterial/KOMODO_1037_THIOMONAS_DELICATA_medium.yaml")
PARENT_ID = "CultureMech:003551"
PARENT_NAME = "thiomonas_delicata_medium"
PARENT_SOURCE_TERM = "komodo.medium:1037"
PARENT_PH = 6.0
...
INGREDIENT_SIGNATURE = (
    ...
    ("Na2S2O3 x 5 H2O", "5", "G_PER_L"),
    ("Yeast extract", "1", "G_PER_L"),
    ("Na-aspartate", "1", "G_PER_L"),
    ("Agar", "15", "G_PER_L"),
)

CURATOR = "repair_komodo_1037_thiomonas_delicata_score10.py"
ACTION = "RESOLVED_KOMODO_1037_THIOMONAS_DELICATA_STRAIN_VARIANT"
...
class Child:
    ...
    def modification(self) -> str:
        return (
            f"{self.source_label} preserves THIOMONAS DELICATA MEDIUM components "
    ...
    def notes(self) -> str:
        return (
            f"{self.source_label} applies THIOMONAS DELICATA MEDIUM at "
            f"pH {self.ph_value:g} for {self.dsm}."
        )


CHILD = Child(
    path=Path("bacterial/for_dsm_18570.yaml"),
    record_id="CultureMech:003550",
    name="for_dsm_18570",
    source_term="komodo.medium:1037.1",
    ph_value=6.0,
    dsm="DSM 18570",
)
...
def _parent_ref(child: Child) -> dict[str, str]:
    return {
        ...
        "id": PARENT_ID,
        "name": PARENT_NAME,
        "notes": (f"KOMODO Medium 1037 is the pH {PARENT_PH:g} " "THIOMONAS DELICATA MEDIUM base."),
    }
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_thiomonas_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM, PARENT_PH)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked a THIOMONAS DELICATA MEDIUM strain-specific child",
            "source": CHILD.source_label,
            "notes": (
                "Changed DSM 18570 from SOURCE_DUPLICATE to a "
                "STRAIN_SPECIFIC_VARIANT child of KOMODO Medium 1037."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any], child: Child = CHILD) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a THIOMONAS DELICATA MEDIUM strain-specific variant",
            "source": child.source_label,
