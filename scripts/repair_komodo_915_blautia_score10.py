#!/usr/bin/env python3
"""Repair KOMODO 915 Blautia exact and strain child links."""
...
PARENT = Path("bacterial/blautia_medium.yaml")
PARENT_ID = "CultureMech:002083"
PARENT_NAME = "blautia_medium"
PARENT_SOURCE_TERM = "mediadive.medium:915"
...
SIGNATURE = (
    ...
    ("(NH4)2SO4", "0.299401", "G_PER_L"),
    ("NH4Cl", "0.998004", "G_PER_L"),
    ("NaCl", "1.6087820000000002", "G_PER_L"),
    ("MgSO4 x 7 H2O", "3.11976", "G_PER_L"),
    ("CaCl2 x 2 H2O", "0.1798403", "G_PER_L"),
    ("Yeast extract", "1.99601", "G_PER_L"),
    ("Sodium resazurin", "0.000499002", "G_PER_L"),
    ("NaHCO3", "7.48503", "G_PER_L"),
    ("D-Glucose", "4.99002", "G_PER_L"),
    ("L-Cysteine HCl x H2O", "0.449102", "G_PER_L"),
    ("Na2S x 9 H2O", "0.449102", "G_PER_L"),
...
CHILDREN = (
    Child(
        Path("bacterial/medium_for_carbon_dioxide_reducers.yaml"),
        "CultureMech:006792",
        "komodo.medium:915",
        "KOMODO Medium 915",
        SOURCE_DUPLICATE_RELATIONSHIP,
        "KOMODO Medium 915 has the same 35-ingredient signature as DSMZ Medium 915.",
    ),
    Child(
        Path("bacterial/for_dsm_14465_and_dsm_14466.yaml"),
        "CultureMech:006790",
        "komodo.medium:915.1",
        "KOMODO Medium 915.1",
        STRAIN_RELATIONSHIP,
        "KOMODO Medium 915.1 applies Blautia medium to DSM 14465 and DSM 14466.",
    ),
    Child(
        Path("bacterial/for_dsm_14469.yaml"),
        "CultureMech:006791",
        "komodo.medium:915.2",
        "KOMODO Medium 915.2",
        STRAIN_RELATIONSHIP,
        "KOMODO Medium 915.2 applies Blautia medium to DSM 14469.",
    ),
)
CHILD_BY_PATH = {child.path: child for child in CHILDREN}
EXPECTED_CHILD_COUNT = 3

CURATOR = "repair_komodo_915_blautia_score10.py"
ACTION = "RESOLVED_KOMODO_915_BLAUTIA_TOPOLOGY"
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Re-rooted KOMODO Medium 915 Blautia exact topology",
            "source": "DSMZ Medium 915; KOMODO Medium 915, 915.1, 915.2",
            "notes": (
                "Moved exact KOMODO Medium 915 and its DSM-specific wrappers "
                "under DSMZ Medium 915."
            ),
        },
    )
    return repaired
...
def repair_child(relative_path: Path, doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": f"Linked under DSMZ Medium 915 as {child.relationship}",
