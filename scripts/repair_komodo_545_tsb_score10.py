#!/usr/bin/env python3
"""Repair a KOMODO TSB strain variant mislinked as a duplicate."""
...
PARENT = Path("bacterial/KOMODO_545_TRYPTONE_SOYA_BROTH_TSB.yaml")
PARENT_ID = "CultureMech:005984"
PARENT_NAME = "tryptone_soya_broth_tsb"
PARENT_SOURCE_TERM = "komodo.medium:545"
PARENT_PH = 7.3

INGREDIENT_SIGNATURE = (
    ("Casein peptone", "17", "G_PER_L"),
    ("Soy peptone", "3", "G_PER_L"),
    ("D(+)-Glucose", "2.5", "G_PER_L"),
    ("NaCl", "5", "G_PER_L"),
    ("K2HPO4", "2.5", "G_PER_L"),
    ("Sodium resazurin", "0.0005", "G_PER_L"),
    ("L-Cysteine HCl x H2O", "0.3", "G_PER_L"),
    ("Na-thioglycolate", "0.3", "G_PER_L"),
)

CURATOR = "repair_komodo_545_tsb_score10.py"
ACTION = "RESOLVED_KOMODO_545_TSB_STRAIN_VARIANT"
...
class Child:
    ...
    def modification(self) -> str:
        return (
            f"{self.source_label} preserves TRYPTONE SOYA BROTH components "
    ...
    def notes(self) -> str:
        return (
            f"{self.source_label} applies TRYPTONE SOYA BROTH at "
            f"pH {self.ph_value:g} for {self.dsm}."
        )


CHILD = Child(
    path=Path("bacterial/for_dsm_22112.yaml"),
    record_id="CultureMech:005979",
    name="for_dsm_22112",
    source_term="komodo.medium:545.1",
    ph_value=7.3,
    dsm="DSM 22112",
)
...
def _parent_ref(child: Child) -> dict[str, str]:
    return {
        ...
        "id": PARENT_ID,
        "name": PARENT_NAME,
        "notes": f"KOMODO Medium 545 is the pH {PARENT_PH:g} TSB base.",
    }
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    _require_tsb_record(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM, PARENT_PH)
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked a TSB strain-specific child",
            "source": CHILD.source_label,
            "notes": (
                "Changed DSM 22112 from SOURCE_DUPLICATE to a "
                "STRAIN_SPECIFIC_VARIANT child of KOMODO Medium 545."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any], child: Child = CHILD) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked as a TSB strain-specific variant",
            "source": child.source_label,
