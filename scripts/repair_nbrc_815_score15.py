#!/usr/bin/env python3
"""Repair the curated NBRC Medium 814 Trypticase Soy Yeast Extract record."""
...
CURATOR = "repair_nbrc_815_score15.py"
ACTION = "RESOLVED_NBRC_815_SCORE15"
TIMESTAMP = "2026-09-11T00:00:00-07:00"

TARGET_PATH = "bacterial/NBRC_815.yaml"
TARGET_ID = "CultureMech:007495"
REQUIRED_ACTION = "Removed corrupt rows, kept curated composition"

SOURCE = "NBRC Medium 814"
NBRC_URL = "https://www.nite.go.jp/nbrc/catalogue/NBRCMediumDetailServlet?NO=814"
TITLE = "Trypticase Soy Yeast Extract Medium"
PH_RANGE = {"min": 7.0, "max": 7.2}

Component = tuple[str, str, str]

CURATED_INGREDIENT_SIGNATURE: tuple[Component, ...] = (
...
GROUNDINGS: dict[str, tuple[str, str]] = {
    ...
    "Dipotassium phosphate": ("CHEBI:131527", "dipotassium hydrogen phosphate"),
    "Glucose": ("CHEBI:17234", "glucose"),
    "Sodium chloride": ("CHEBI:26710", "sodium chloride"),
}

NOTES = (
    "Source: NBRC Medium 814 | Link: "
    "https://www.nite.go.jp/nbrc/catalogue/NBRCMediumDetailServlet?NO=814\n\n"
    "NBRC lists Trypticase Soy Yeast Extract Medium as Trypticase Soy Broth "
    "(Difco), yeast extract, optional agar, and distilled water at pH 7.0 - "
    "7.2. The ingredient list keeps the curated Tryptic Soy Broth / Tryptic "
    "Soy Agar constituent expansion because NBRC lists only the commercial "
    "premix."
)
...
def _ensure_target(doc: dict[str, Any]) -> None:
    ...
    if doc.get("name") not in {"815", TITLE}:
        raise ValueError(f"{TARGET_PATH}: NBRC title/name drifted")

    ingredient_signature = _signature(doc.get("ingredients"), "ingredients")
    if ingredient_signature != CURATED_INGREDIENT_SIGNATURE:
        raise ValueError(
            f"{TARGET_PATH}: ingredient signature drifted from "
            f"{CURATED_INGREDIENT_SIGNATURE!r} to {ingredient_signature!r}"
        )
...
def _ensure_flags(doc: dict[str, Any]) -> None:
    ...
    for row in doc.get("ingredients") or []:
        ...
        if not isinstance(term, dict) or not term.get("id"):
            raise ValueError(f"{TARGET_PATH}: not all curated rows are grounded")
...
def _ensure_event(doc: dict[str, Any]) -> None:
    event = {
        ...
        "curator": CURATOR,
        "action": ACTION,
        "changes": "Added NBRC Medium 814 identity to curated TSA expansion",
        "source": NBRC_URL,
        "notes": (
            "Added the NBRC Medium 814 source term and official title, "
            "structured its pH range, preserved the existing curated Tryptic "
            "Soy Agar constituent expansion, and added the official NBRC "
            "reference."
        ),
    }
...
def repair_document(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _put_after(
        ...
        "media_term",
        {
            ...
            "term": _term("nbrc.medium:814", SOURCE),
        },
        "physical_state",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"{action} {changed_count} NBRC 815 records")
    return 0
