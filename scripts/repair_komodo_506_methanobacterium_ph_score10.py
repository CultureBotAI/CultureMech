#!/usr/bin/env python3
"""Repair KOMODO 506.1 Methanobacterium espanolae pH child link."""
...
PARENT = Path("archaea/KOMODO_506_METHANOBACTERIUM_ESPANOLAE_MEDIUM.yaml")
PARENT_ID = "CultureMech:005699"
PARENT_NAME = "methanobacterium_espanolae_medium"
PARENT_SOURCE_TERM = "komodo.medium:506"
PARENT_PH_RANGE = {"min": 5.5, "max": 6.0}

CHILD = Path("bacterial/for_dsm_19849.yaml")
CHILD_ID = "CultureMech:005698"
CHILD_SOURCE_TERM = "komodo.medium:506.1"
CHILD_PH = 7.0
RELATIONSHIP = "PH_VARIANT"
CHILD_NOTES = (
    "KOMODO Medium 506.1 applies METHANOBACTERIUM ESPANOLAE medium at " "pH 7.0 to DSM 19849."
)

SOURCE_DUPLICATE_PARENT = {
    "path": "data/normalized_yaml/archaea/methanobacterium_espanolae_medium.yaml",
    "relationship": "SOURCE_DUPLICATE",
    "id": "CultureMech:001638",
    "name": "methanobacterium_espanolae_medium",
    "notes": (
        "KOMODO record states DSMZ Medium 506 provenance and exactly matches "
        "the same-category DSMZ parent signature."
    ),
}

INGREDIENT_SIGNATURE = (
    ("Na-acetate x 3 H2O", "2.4728", "G_PER_L"),
    ("(NH4)2SO4", "0.445104", "G_PER_L"),
    ("K2HPO4", "0.286845", "G_PER_L"),
    ("KH2PO4", "0.178042", "G_PER_L"),
    ("MgSO4 x 7 H2O", "3.118694", "G_PER_L"),
    ("CaCl2 x 2 H2O", "0.15934720000000002", "G_PER_L"),
    ("NaCl", "1.049456", "G_PER_L"),
    ("Sodium resazurin", "0.00049456", "G_PER_L"),
    ("Na2CO3", "0.49456", "G_PER_L"),
...
CURATOR = "repair_komodo_506_methanobacterium_ph_score10.py"
ACTION = "RESOLVED_KOMODO_506_METHANOBACTERIUM_PH_TOPOLOGY"
...
def _parent_ref() -> dict[str, str]:
    return {
        ...
        "id": PARENT_ID,
        "name": PARENT_NAME,
        "notes": "KOMODO Medium 506 is the METHANOBACTERIUM ESPANOLAE base for KOMODO Medium 506.1.",
    }
...
def _require_parent(doc: dict[str, Any]) -> None:
    _require_signature(doc, PARENT, PARENT_ID, PARENT_SOURCE_TERM)
    if doc.get("ph_range") != PARENT_PH_RANGE:
        raise ValueError(f"{PARENT}: expected pH range {PARENT_PH_RANGE!r}")


def _require_child(doc: dict[str, Any]) -> None:
    _require_signature(doc, CHILD, CHILD_ID, CHILD_SOURCE_TERM)
    if doc.get("ph_value") != CHILD_PH:
        raise ValueError(f"{CHILD}: expected pH {CHILD_PH!r}")
...
def repair_parent(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _put_after(repaired, "parent_media", SOURCE_DUPLICATE_PARENT, "variant_children")
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Reclassified KOMODO Medium 506.1 as a pH variant",
            "source": "KOMODO Medium 506 and 506.1",
            "notes": (
                "Linked KOMODO Medium 506.1 under the KOMODO Medium 506 "
                "METHANOBACTERIUM ESPANOLAE base while preserving the DSMZ "
                "506 duplicate parent relationship."
            ),
        },
    )
    return repaired
...
def repair_child(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    _upsert_event(
        repaired,
        {
            ...
            "curator": CURATOR,
            "action": ACTION,
            "changes": "Linked under KOMODO Medium 506",
            "source": "KOMODO Medium 506.1",
            "notes": CHILD_NOTES,
        },
    )
    return repaired
