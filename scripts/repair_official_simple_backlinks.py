#!/usr/bin/env python3
"""Add reciprocal links for repaired official-simple variants."""
...
CURATOR = "repair_official_simple_backlinks.py"
ACTION = "ADDED_OFFICIAL_SIMPLE_BACKLINKS"
...
GYP_SODIUM_ACETATE = Parent(
    Path("bacterial/gyp_sodium_acetate_mineral_salts_broth.yaml"),
    "CultureMech:003035",
    "gyp_sodium_acetate_mineral_salts_broth",
    "mediadive.medium:J68",
    "JCM Medium 68-69 and TOGO Medium M61",
)
OATMEAL_ISP3 = Parent(
    Path("bacterial/TOGO_M42_Oatmeal_Agar_ISP-3.yaml"),
    "CultureMech:009815",
    "oatmeal_agar_isp_3",
    "TOGO:M42",
)
ISP4 = Parent(
    Path("bacterial/TOGO_M50_Inorganic_Salts-Starch_Agar_ISP-4.yaml"),
    "CultureMech:009900",
    "inorganic_salts_starch_agar_isp_4",
    "TOGO:M50",
)

CHILDREN = (
    Child(
        Path("bacterial/jcm_medium_no_69.yaml"),
        "CultureMech:003045",
        "jcm_medium_no_69",
        "mediadive.medium:J69",
        GYP_SODIUM_ACETATE,
        SALINITY_VARIANT,
        "Supplements 1.0 L GYP-sodium acetate-mineral salts broth with 50.0 g/L NaCl.",
    ),
    Child(
        Path("bacterial/togo_medium_m61.yaml"),
        "CultureMech:010019",
        "togo_medium_m61",
        "TOGO:M61",
        GYP_SODIUM_ACETATE,
    ...
    Child(
        Path("fungal/oatmeal_agar_isp_3_with_0_1_yeast_extract.yaml"),
        "CultureMech:010531",
        "oatmeal_agar_isp_3_with_0_1_yeast_extract",
        "mediadive.medium:J51",
        OATMEAL_ISP3,
        SUPPLEMENTED_VARIANT,
        "Supplements 1.0 L Oatmeal agar (ISP-3) with 1.0 g/L Yeast extract (BD-Difco).",
    ),
    Child(
        Path("fungal/inorganic_salts_starch_agar_isp_4_with_0_05_yeast_extract.yaml"),
        "CultureMech:010510",
        "inorganic_salts_starch_agar_isp_4_with_0_05_yeast_extract",
        "mediadive.medium:J217",
        ISP4,
        SUPPLEMENTED_VARIANT,
        "Supplements 1.0 L Inorganic salts-starch agar (ISP-4) with 0.5 g/L Yeast extract.",
    ),
)
PARENTS = (GYP_SODIUM_ACETATE, OATMEAL_ISP3, ISP4)
CHILDREN_BY_PARENT: dict[Path, tuple[Child, ...]] = {
    parent.path: tuple(child for child in CHILDREN if child.parent == parent) for parent in PARENTS
}
...
def _upsert_event(doc: dict[str, Any], parent: Parent) -> None:
    event = {
        ...
        "source": parent.event_source or parent.source_term,
        "notes": "Mirrored curated official-simple parent_media links on the parent record.",
    }
