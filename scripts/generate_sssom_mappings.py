...
_warned_adapter_prefixes: set[str] = set()
...
def canonical_label(curie: str, fallback: str = "") -> str:
    ...
    recipe-provided label so non-ontology objects are unaffected. Adapter
    failures warn once per prefix; fallback output still needs the separate
    ``validate-products`` correspondence check. Failed lookups are memoized
    for this process so repeated ingredients do not repeatedly query an
    unavailable adapter. A fresh generation retries them.
    """
    ...
    if src:
        try:
            ...
            if prefix not in _warned_adapter_prefixes:
                _warned_adapter_prefixes.add(prefix)
                _logger.warning(
                    "Canonical label lookup failed for %s using %s: %s: %s. "
                    "Using recipe labels when %s lookups fail; further failures "
                    "for this prefix will not be logged. Resolve the adapter "
                    "problem, regenerate SSSOM, and run validate-products.",
                    curie,
                    src,
                    type(error).__name__,
                    error,
            ...
            # Keep the original per-CURIE memoization even on failure: a corpus
            # can repeat an ingredient thousands of times while an adapter is
            # unavailable. The warning directs users to a fresh generation.
            lbl = None
                ...
                if term and term.get("id"):
                    ...
                    if include_unmapped:
                        ...
                        if subject_id not in unmapped_candidates:
                            unmapped_candidates[subject_id] = {"label": preferred_term, "count": 0}
...
def generate_sssom_metadata(include_unmapped: bool = False) -> str:
    ...
    if include_unmapped:
        description += (
            ". Includes unmapped ingredients (confidence=0.0) as candidates for future curation."
        )
...
def main():
    parser = argparse.ArgumentParser(description="Generate SSSOM mapping file from normalized YAML")
    parser.add_argument(
        "--normalized-dir",
        type=Path,
    ...
    parser.add_argument(
        "--confidence-threshold", type=float, default=0.0, help="Minimum confidence score (0.0-1.0)"
    )
    parser.add_argument(
        "--include-unmapped",
        action="store_true",
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate SSSOM format after generation"
    )
    ...
    if len(unmapped) > 0:
        ...
        print("  These can be prioritized for manual curation or OLS discovery")
