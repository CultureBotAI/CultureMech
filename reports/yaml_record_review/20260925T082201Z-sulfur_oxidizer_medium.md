# YAML Record Review: Sulfur-oxidizer Medium

- Repository: `/Users/marcin/Documents/VIMSS/ontology/KG-Hub/KG-Microbe/Mechs/CultureMech`
- Record: `data/merge_yaml/merged/sulfur_oxidizer_medium.yaml` (`CultureMech:008328`)
- Started UTC: `2026-09-25T08:22:01Z`
- Finished UTC: `2026-09-25T08:22:01Z`
...
| Field | Value |
| --- | --- |
| Generated YAML | `data/merge_yaml/merged/sulfur_oxidizer_medium.yaml` |
| Normalized source | `data/normalized_yaml/bacterial/sulfur_oxidizer_medium.yaml` |
| CultureMech ID | `CultureMech:008328` |
| Media term | `TOGO:M1763` |
| Original source | TOGO Medium M1763, NBRC Medium 976, Sulfur-oxidizer Medium |
| Merge fingerprint | `7480749721754f73b6f5f06dbc945f6855f77112120f51d7ec277874d1c70c17` |
| Merged from | `sulfur_oxidizer_medium` |
...
The generated record has the expected `CultureMech:008328` identifier, `TOGO:M1763` source term, NBRC 976 provenance, and merge fingerprint. Exact gitignore-independent searches with `--no-ignore --hidden` for `CultureMech:008328`, `TOGO:M1763`, the merge fingerprint, and `sulfur_oxidizer_medium.yaml` found this normalized source and generated merge as the only direct recipe records, with the expected normalized indexes also referencing the same ID and source term.

Several simple-salt groundings are plausible, but the stock hierarchy is not. KP buffer, Trace elements solution, and Vitamin solution remain listed as separate `solutions` while their internal stock ingredients were also flattened into top-level final-medium ingredients at stock concentrations. `CoCl2 x 6 H2O` and `NiCl2 x 6 H2O` are grounded to generic cobalt dichloride and nickel dichloride terms, respectively, despite hydrate-specific source strings.

## Evidence

NBRC 976 and TOGO M1763 list the completed main solution as 10 ml KP buffer, 0.75 g MgCl2 x 6 H2O, 0.15 g CaCl2 x 2 H2O, 0.54 g NH4Cl, 0.8 g Na2S2O3, 2 ml Trace elements solution, 2 ml Vitamin solution, 0.5 g Na2CO3, and 1 L distilled water, with the pH unadjusted.

The source preparation says to mix ingredients except KP buffer and vitamin solution, dispense under N2/CO2/O2 at 60/20/20, separately autoclave KP buffer under N2, filter-sterilize the vitamin solution, aseptically and anaerobically add KP buffer and vitamin solution before inoculation, and pressurize inoculated vessels to 150 kPa with the same gas mixture.

The NBRC 976 stock recipes define KP buffer as 119 g KH2PO4 plus 21 g K2HPO4 per 1 L stock; Trace elements solution as NTA, FeCl3 x 6 H2O, MnCl2 x 4 H2O, CoCl2 x 6 H2O, CaCl2 x 2 H2O, ZnCl2, CuCl2 x 2 H2O, H3BO3, Na2MoO4 x 2 H2O, NaCl, NiCl2 x 6 H2O, Na2SeO4, Na2WO4, and KAl(SO4)2 x 12 H2O per 1 L stock; and Vitamin solution as milligram-scale biotin, folic acid, pyridoxine-HCl, thiamine-HCl, riboflavin, nicotinic acid, Ca-pantothenate, p-aminobenzoic acid, and Vitamin B12 per 1 L stock.

The generated YAML stores KP buffer as `10 G_PER_L`, Trace elements solution as `2 G_PER_L`, Vitamin solution as `2 G_PER_L`, flattens all stock ingredients as top-level `G_PER_L` rows, and carries the gas names as variable top-level ingredients. Merge generation then summed four scoped distilled-water rows to `4.0 G_PER_L`, even though the normalized source had already repaired the duplicate-water value to `1.0`.
...
- KP buffer, Trace elements solution, and Vitamin solution are represented with `G_PER_L` concentrations instead of the source's 10 ml/L, 2 ml/L, and 2 ml/L additions.
...
- Repair `data/normalized_yaml/bacterial/sulfur_oxidizer_medium.yaml` or the TOGO nested-stock import, then regenerate `data/merge_yaml/merged/sulfur_oxidizer_medium.yaml`.
- Keep KP buffer, Trace elements solution, and Vitamin solution as nested 1 L stocks added to the final medium at 10 ml/L, 2 ml/L, and 2 ml/L.
...
- Add the source preparation text for separate KP-buffer autoclaving, vitamin filtration, anaerobic addition, and 150 kPa pressurization.
...
- Re-run exact gitignore-independent searches for `CultureMech:008328`, `TOGO:M1763`, `7480749721754f73b6f5f06dbc945f6855f77112120f51d7ec277874d1c70c17`, and `sulfur_oxidizer_medium.yaml`.
- Recompare the regenerated M1763 recipe against both TOGO and NBRC to make sure the three nested stock recipes are scoped under the correct main-solution additions.
...
The generated YAML should not be hand-edited. The review findings target the TOGO import, normalized source, and merge generation for nested stocks.
