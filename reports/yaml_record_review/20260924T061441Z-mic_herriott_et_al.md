# YAML Record Review: Mic; Herriott et al

- Repository: CultureMech
- Record: `data/merge_yaml/merged/mic_herriott_et_al.yaml`
- Started UTC: 2026-09-24T06:14:41Z
- Finished UTC: 2026-09-24T06:14:41Z
- Verdict: needs curation

## Target

- Reviewed merged record `data/merge_yaml/merged/mic_herriott_et_al.yaml`.
- Editable normalized source: `data/normalized_yaml/bacterial/mic_herriott_et_al.yaml`.
- MediaDB medium: `MEDIADB:281`, Mic; Herriott et al.
...
- The MediaDB identity is coherent: MediaDB medium 281 is `Mic; herriott et al`, has 30 compounds, links source 94, and reports one growth-data record for *Haemophilus influenzae* Rd.
...
- The MediaDB tab-delimited export lists the same 30 compound amounts as the generated record, including `L-Arginine` 3.36738 mM, `Uracil` 0.89217 mM, `L-Glutamate` 8.83572 mM, sodium chloride 99.2471 mM, sodium L-lactate 7.13903 mM, and hemin 0.01534 mM.
- The MediaDB HTML page lists the same 30 compounds and links the formulation to `Herriott et al, 1969`.
...
- Ingredient coverage is complete relative to MediaDB medium 281 after the normalized `NAD+` repair.
...
- The generated file is stale relative to `data/normalized_yaml/bacterial/mic_herriott_et_al.yaml` and still publishes the former empty first ingredient instead of `NAD+`.
...
- Regenerate `data/merge_yaml/merged/mic_herriott_et_al.yaml` from `data/normalized_yaml/bacterial/mic_herriott_et_al.yaml`.
...
- Re-run focused open-schema, strict, reference, and term validators for `data/merge_yaml/merged/mic_herriott_et_al.yaml`.
- Compare the regenerated ingredient list against `https://mediadb.systemsbiology.net/defined_media/media_text/281/` and verify all 30 row labels are non-empty.
