...
- Record: `data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__a7fbb591.yaml`
- Started UTC: 2026-09-22T08:41:11Z
- Finished UTC: 2026-09-22T08:43:40Z
...
|---|---|
| Class | `MediaRecipe` |
| Generated record | `data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__a7fbb591.yaml` |
| Stable ID | `CultureMech:009104` |
| Label | `chopped_meat_medium_with_carbohydrates` |
| Source term | `TOGO:M2534` |
| Source document | `DSMZ_Medium110.pdf` |
| Maintained owner | `data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml` |
| Merge status | Generated from one source recipe, `TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates` |
...
`data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml`
...
| Check | Result |
|---|---|
| Open LinkML schema | Pass; `linkml-validate -s src/culturemech/schema/culturemech.yaml --target-class MediaRecipe data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__a7fbb591.yaml` exited 0. |
| Strict schema | Pass; `scripts/validate_strict.py data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__a7fbb591.yaml --out /private/tmp/chopped_meat_medium_with_carbohydrates__a7fbb591.strict.tsv --workers 1 --quiet` scanned one file and emitted zero error rows. |
| Reference validation | Pass with no semantic coverage; `linkml-reference-validator validate data data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__a7fbb591.yaml --schema src/culturemech/schema/culturemech.yaml --target-class MediaRecipe` reported zero total checks because the record has no structured evidence references. |
| Term validation | Pass; `linkml-term-validator validate-data data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__a7fbb591.yaml -s src/culturemech/schema/culturemech.yaml -t MediaRecipe --labels -c conf/oak_config.yaml` completed successfully, aside from the expected `eutils` `pkg_resources` deprecation warning. |
...
The record is a TOGO rendering of the DSMZ Medium 110 Vitamin K1 branch. TOGO
`M2534` and the inspected DSMZ Medium 110 PDF agree on the medium name, the
base Chopped Meat Medium With Carbohydrates recipe, and a post-autoclave branch
that adds 10 ml Haemin solution plus 10 ml Vitamin K1 solution to 1000 ml of
medium.

This identity is distinct from the base DSMZ 110 recipe, the Vitamin K3 branch,
and strain-specific pH or horse-serum variants. The record should remain a
variant of DSMZ 110 with only the Haemin plus Vitamin K1 additions.

## Evidence

Supported by the inspected TOGO `M2534` API payload and the DSMZ Medium 110 PDF:
...
- `Haemin solution` and `Vitamin K1 solution` are represented as empty unknown
...
The record has enough labels to identify the Haemin plus Vitamin K1 branch, but
not enough structure to reconstruct it. The final recipe, Haemin stock, and
Vitamin K1 stock are mixed into one parent ingredient list, source volumes are
stored as gram-per-liter concentrations, Haemin-stock water is summed into
parent water, and the post-autoclave supplement timing is missing.

The omission of Vitamin K3 is correct for this K1-specific branch. The omission
...
`reports` for exact `CultureMech:009104`, `TOGO:M2534`, `M2534`, the
`a7fbb5914904d96fe118b2747c4c0354923104404b871a900c4db0b3caaaa035`
fingerprint, and the `TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates` stem
found this single owner plus its generated product, indexes, and review
manifests. It did not find a second owner for exact TOGO `M2534`.

Empty `target_organisms` and growth-evidence slots are acceptable here because
DSMZ Medium 110 is a source recipe, and this TOGO record does not tie the K1
...
|---|---|---|---|
| Major | Haemin and Vitamin K1 stock internals are flattened into the parent medium. | DSMZ says to add 10 ml Haemin solution and 10 ml Vitamin K1 solution after autoclaving; the 50 mg haemin, 1 ml 1 N NaOH, 99 ml Haemin-stock water, 0.1 ml vitamin K1, and 20 ml ethanol are stock-internal materials. | `data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml` and, if systematic, the TOGO importer for subcomponents. |
| Major | Stock and preparation volumes are represented as `G_PER_L` concentrations. | DSMZ specifies 10 ml final Haemin stock, 10 ml final K1 stock, 25 ml base 1 N NaOH, and 0.5 ml Na-resazurin stock; the YAML records those volumes as gram-per-liter quantities or empty `solutions`. | `data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml`. |
| Major | The source preparation workflow, pH, and post-autoclave supplement timing are missing. | DSMZ defines the meat infusion, 1000 ml final makeup, anoxic 100% N2 workflow, pH 7.0 adjustment, Hungate-tube dispensing, 121 C for 30 min autoclaving, and only then the Haemin/K1 additions. | `data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml`. |
| Major | Nitrogen gas is modeled as a variable ingredient rather than the exact gas atmosphere. | DSMZ scopes nitrogen to cooling and dispensing under 100% N2; TOGO preserves that 100% value, but the YAML defaulted it to `VARIABLE`. | `data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml`. |
| Minor | Several retained source ingredients remain ungrounded or legacy-grounded. | Import tracking flags `NaOH 1 N`, `L-cysteine hydrochloride`, `95% ethanol`, and `Starch (soluble)` as unresolved, and the L-cysteine node still uses a legacy `mediaingredientmech_term`. | `data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml`. |

## Recommended Edits

1. In `data/normalized_yaml/bacterial/TOGO_M2534_Chopped_Meat_Medium_With_Carbohydrates.yaml`,
...
6. Re-resolve `NaOH 1 N`, `L-cysteine hydrochloride`, `95% ethanol`, and
   `Starch (soluble)` through the packaged MediaIngredientMech label index so
   each has a current exact CHEBI-backed link or an explicit unresolved note.

## Follow-up Checks

- Run focused open-schema, strict, reference, and term validation on the edited
  TOGO M2534 normalized record and any stock solution records used by it.
...
- Re-read the regenerated generated record against DSMZ Medium 110 and confirm
  that the M2534 K1 branch has no Vitamin K3 stock internals and no agar slant
  material in the base ingredient list.
- Compare the regenerated M2534 branch with the base, Vitamin K3, and other
  Vitamin K1 TOGO Medium 110 variants before deciding whether any exact
  duplicates should merge.

## Additional Notes

- `data/import_tracking/reports/merged_duplicates.tsv` already flags
  `Distilled water` as a sum of `1000.0` and `99.0`; that 99 ml component is
  Haemin-stock dilution water, not parent medium water.
- The TOGO source omits an explicit `ph` field for `M2534`, but the shared
