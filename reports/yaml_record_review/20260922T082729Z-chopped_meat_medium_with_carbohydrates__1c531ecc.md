...
- Record: data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__1c531ecc.yaml
- Started UTC: 2026-09-22T08:26:52Z
- Finished UTC: 2026-09-22T08:27:38Z
...
| --- | --- |
| Class | MediaRecipe |
| ID | CultureMech:009106 |
| Label | Chopped Meat Medium With Carbohydrates |
| Generated record | data/merge_yaml/merged/chopped_meat_medium_with_carbohydrates__1c531ecc.yaml |
| Maintained owner | data/normalized_yaml/bacterial/TOGO_M2536_Chopped_Meat_Medium_With_Carbohydrates.yaml |
| Source | TOGO Medium M2536, DSMZ Medium 110 |

The reviewed file is a generated one-source merge of TOGO M2536, which imports
...
| `scripts/validate_strict.py` on the generated record | Passed; 0 errors in `/private/tmp/chopped_meat_medium_with_carbohydrates__1c531ecc.strict.tsv` |
...
The source identity is coherent: `CultureMech:009106`, `TOGO:M2536`, the DSMZ
...
TSVs, and `data/import_tracking/reports` for the CultureMech ID, TOGO M2536
...
The inspected DSMZ Medium 110 PDF and TOGO M2536 JSON support a base
...
solution, Vitamin K1 solution, 15 g agar, and all of the stock internals as
unconditional parent ingredients.

The nested stock scopes are also flattened. Base water and Haemin-stock water
are merged into a 1099 g/L parent water ingredient; the 10 mL Haemin and
Vitamin K1 stock additions are empty `G_PER_L` solution shells; 25 mL 1 N
NaOH, 1 mL Haemin-stock NaOH, 50 mg haemin, 20 mL 95% ethanol, and 0.1 mL
Vitamin K1 are all asserted as parent `G_PER_L` ingredients.
...
agar-slant branch, Haemin stock, and Vitamin K1 stock are modeled as separate
recipe scopes with source-faithful millilitre units, pH, 100% N2 handling, and
the meat-filtrate preparation sequence.

## Findings

| Severity | Finding |
| --- | --- |
| Major | The conditional agar-slant branch and the conditional Haemin/Vitamin K branch are represented as unconditional parent-medium ingredients and stock additions. **Owner:** `data/normalized_yaml/bacterial/TOGO_M2536_Chopped_Meat_Medium_With_Carbohydrates.yaml` or the TOGO/DSMZ importer. |
| Major | Haemin solution and Vitamin K1 solution are flattened into parent ingredients while their 10 mL post-autoclave additions remain as empty `G_PER_L` solution shells. **Owner:** the maintained normalized owner or solution migration. |
| Major | Source volumes and milligram quantities are converted to parent `G_PER_L`, affecting 25 mL 1 N NaOH, 0.5 mL Na-resazurin, 50 mg haemin, 20 mL 95% ethanol, and 0.1 mL Vitamin K1. **Owner:** the TOGO/DSMZ unit-conversion importer. |
| Major | Water from the base medium and Haemin stock is merged into one 1099 g/L parent ingredient, erasing the 1000 mL base medium volume and the 99 mL Haemin-stock water quantity. **Owner:** duplicate merging plus solution migration. |
...
1. Keep DSMZ 110 base medium, agar-slant agar, Haemin solution, and Vitamin K1
...
2. Restore source volume and mass units for water, 1 N NaOH, Na-resazurin
   solution, Haemin stock, and Vitamin K1 stock.
...
Then manually compare the regenerated record against TOGO M2536 and DSMZ
