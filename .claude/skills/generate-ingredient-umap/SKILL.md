...
PaCMAP reduction of KG-Microbe KG-Microbe DeepWalk embeddings.
...
# Explicit legacy UMAP parameters
python scripts/generate_ingredient_umap.py --method umap --n-neighbors 30 --min-dist 0.05
...
1. `data/embeddings/DeepWalkSkipGramEnsmallen_degreenorm_embedding_512_v3_2026-06-26_12_55_27.tsv.gz` (local, preferred)
2. `../CommunityMech/data/embeddings/...tsv.gz` (sibling repo fallback)
...
The default reducer is PaCMAP; labels and dimensions are derived from the actual run. `KG_MICROBE_EMBEDDINGS` selects an explicit source before local/sibling discovery. A missing selected source fails. Existing published coordinates have not been regenerated by changing this skill; do not infer their reducer or source from the new default.
