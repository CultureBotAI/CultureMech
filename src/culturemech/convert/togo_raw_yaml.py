"""Convert TOGO JSON to raw YAML format.

Performs direct 1:1 conversion of TOGO media database exports to YAML
without any normalization or LinkML validation.

Input: raw/togo/*.json
Output: raw_yaml/togo/*.yaml
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert TOGO JSON to raw YAML format (no normalization)"
    )
    parser.add_argument(
        "-i", "--input",
        type=Path,
    ...
    logger.info(f"Converting TOGO JSON to raw YAML")
