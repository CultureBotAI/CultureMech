"""Convert CCAP JSON metadata to raw YAML format.

CCAP recipes are in PDF format. This converter:
...
Input: raw/ccap/ccap_media.json (and optionally raw/ccap/pdfs/*.pdf)
Output: raw_yaml/ccap/*.yaml
"""
...
class CCAPRawYAMLConverter(RawYAMLConverter):
    """Convert CCAP JSON metadata to raw YAML."""
    ...
    def convert_file(self, input_file: Path, output_dir: Path):
        """Convert CCAP JSON file to raw YAML files.

        Args:
            input_file: Path to ccap_media.json
        ...
        for i, recipe in enumerate(recipes):
            ...
            recipe_id = recipe.get('id', f'ccap_{i:04d}')
    ...
    def convert_directory(self, input_dir: Path, output_dir: Path, pattern: str = "ccap_media.json"):
        """Convert CCAP media JSON to raw YAML.

        Args:
            input_dir: Input directory containing ccap_media.json
            output_dir: Output directory for raw YAML files
            pattern: Filename pattern (default: ccap_media.json)
        """
        ...
        # Look for ccap_media.json
        input_file = input_dir / pattern

        if not input_file.exists():
            self.log(f"No {pattern} found in {input_dir}", force=True)
            self.log(f"Run 'just fetch-ccap' first to download data", force=True)
            return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert CCAP JSON to raw YAML format (no normalization)"
    )
    parser.add_argument(
        "-i", "--input",
        type=Path,
    ...
    logger.info(f"Converting CCAP JSON to raw YAML")
    logger.info(f"  Input: {args.input}")
    logger.info(f"  Output: {args.output}")
    if args.extract_pdfs:
        logger.info(f"  PDF extraction: enabled")

    converter = CCAPRawYAMLConverter(verbose=args.verbose, extract_pdfs=args.extract_pdfs)
