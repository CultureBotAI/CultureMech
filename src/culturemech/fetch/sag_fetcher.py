"""Fetch algae media recipes from SAG (Sammlung von Algenkulturen Göttingen).

SAG is one of the oldest and largest algal culture collections in the world,
housed at the University of Göttingen, Germany.

Website: https://sagdb.uni-goettingen.de/
Media recipes: https://www.uni-goettingen.de/de/186449.html
Collection: 2,300+ strains

This fetcher:
1. Fetches the media recipe list page
2. Extracts recipe names and PDF URLs
3. Downloads PDF files (optional)
...
class SAGFetcher:
    """Fetch media recipes from SAG."""

    BASE_URL = "https://sagdb.uni-goettingen.de"
    MEDIA_LIST_URL = "https://www.uni-goettingen.de/de/186449.html"
    ...
    def fetch_media_index(self) -> List[Dict[str, str]]:
        """Fetch media list page and extract recipe URLs.

        Returns:
            List of dicts with 'name', 'pdf_url', and 'id' for each recipe
        """
        logger.info(f"Fetching media index from {self.MEDIA_LIST_URL}")

        response = self.session.get(self.MEDIA_LIST_URL)
        ...
        # Find all PDF links
        # SAG uses links like http://sagdb.uni-goettingen.de/culture_media/XX Medium_Name.pdf
        for link in soup.find_all('a', href=True):
            href = link.get('href', '')
            if '.pdf' in href and 'culture_media' in href:
                ...
                if name:
                    ...
                    # e.g., "20 BG11 Medium.pdf" -> "BG11"
                    # Try to extract meaningful ID from filename
                    filename = href.split('/')[-1]
                    filename_base = filename.replace('.pdf', '')

                    # Remove leading numbers (e.g., "20 BG11 Medium" -> "BG11 Medium")
                    id_match = re.sub(r'^\d+\s+', '', filename_base)
                    # Use first meaningful part as ID (remove "Medium" suffix)
                    recipe_id = id_match.replace(' Medium', '').replace(' ', '_')
    ...
    def fetch_recipe_details(self, recipe_info: Dict[str, str]) -> Dict[str, Any]:
        ...
        For SAG, recipes are in PDF format. We store metadata and optionally
    ...
    def fetch_all(self, limit: Optional[int] = None) -> Dict[str, Any]:
        ...
        output_data = {
            'source': 'SAG',
            'source_url': self.MEDIA_LIST_URL,
            'fetched_date': datetime.now().isoformat(),
            'count': len(recipes),
        ...
        # Save to file
        output_file = self.output_dir / 'sag_media.json'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch media recipes from SAG (Sammlung von Algenkulturen Göttingen)"
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
    ...
    fetcher = SAGFetcher(args.output, rate_limit=args.rate_limit, download_pdfs=args.download_pdfs)
