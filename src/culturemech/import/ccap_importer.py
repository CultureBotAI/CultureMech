"""CCAP importer for CultureMech.

Converts CCAP (Culture Collection of Algae and Protozoa) media recipes to CultureMech LinkML YAML format.

CCAP provides ~107 algae media recipes, primarily in PDF format.
"""
...
class CCAPImporter:
    ...
    def __init__(self, raw_data_dir: Path, output_dir: Path):
        ...
        Args:
            raw_data_dir: Directory containing ccap_media.json
    ...
    def load_media_data(self) -> List[Dict]:
        """Load CCAP media JSON."""
        media_file = self.raw_data_dir / "ccap_media.json"
        if not media_file.exists():
            print(f"✗ Media file not found: {media_file}")
            print(f"  Run: just fetch-ccap")
            return []
        ...
        print(f"Found {len(recipes)} CCAP recipes")
        return recipes
    ...
    def _sanitize_filename(self, name: str) -> str:
        ...
        # Replace multiple underscores/spaces/colons with single underscore
    ...
    def _determine_salinity_type(self, recipe: Dict) -> str:
        ...
        saltwater_keywords = [
            ...
            'ocean', 'saltwater', 'asw', 'artificial seawater',
            'brackish', 'estuarine', 'walne', 'antia',
            'plymouth', 'l1 medium'
        ]
        ...
        # Check for specific CCAP saltwater media IDs
        saltwater_ids = ['asw', 'f/2', 'ant', 'pe', 'l1', 'e26', 'e27', 'e31']
        if recipe_id in saltwater_ids or any(recipe_id.startswith(sid) for sid in saltwater_ids):
            return 'saltwater'

        # Freshwater indicators
        freshwater_keywords = [
            'freshwater', 'bold', 'bg-11', 'bg11', 'bg ', 'bristol',
            'tap', 'soil', 'chu', 'jaworski', 'jm', 'k medium',
            'proteose', 'euglena', 'polytoma', 'volvox'
        ]
        if any(kw in name for kw in freshwater_keywords):
            return 'freshwater'

        # Default to freshwater for algae (most CCAP media are freshwater)
        return 'freshwater'

    def convert_recipe(self, recipe: Dict) -> Dict[str, Any]:
        """Convert CCAP recipe to CultureMech format."""
        ccap_id = recipe.get('id', '')
        name = recipe.get('name', 'Unknown Medium')

        # Extract clean name (remove parenthetical if present)
        clean_name_match = re.match(r'^([^(]+)', name)
        clean_name = clean_name_match.group(1).strip() if clean_name_match else name

        # Sanitize filename
        safe_name = self._sanitize_filename(name)
        if not safe_name or safe_name == '_':
            safe_name = ccap_id.replace('/', '_').replace(':', '_')
        ...
        cm_recipe = {
            'name': clean_name,
            'category': 'algae',
            'medium_type': 'complex',  # Most CCAP media are complex
            'physical_state': 'liquid',
        }

        # Add description
        desc_parts = []
        desc_parts.append(f"Algae culture medium from CCAP Culture Collection")
    ...
    def _extract_ingredients_from_text(self, text: str) -> List[Dict]:
        ...
        # This is a simple heuristic - would need enhancement for production
        lines = text.split('\n')
    ...
    def import_all(self, limit: Optional[int] = None):
        ...
        for idx, recipe in enumerate(recipes, 1):
            try:
                ...
                # Write YAML file
                output_file = self.algae_dir / f"CCAP_{filename}.yaml"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import CCAP media recipes into CultureMech"
    )
    parser.add_argument(
        "-i", "--input",
        type=Path,
        default="data/raw/ccap",
        help="Input directory with CCAP raw JSON files (Layer 1: data/raw/ccap/)"
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
    ...
    print("=" * 60)
    print("CCAP to CultureMech Importer")
    print("=" * 60)
    print(f"Input:  {args.input}")
    print(f"Output: {args.output}")
    print()

    importer = CCAPImporter(args.input, args.output)
