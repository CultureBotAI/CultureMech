...
# Get first 100 media
curl "https://togomedium.org/sparqlist/api/list_media?limit=100&offset=0" \
    > raw/togo/togo_media_sample.json

# Get specific medium
curl "https://togomedium.org/sparqlist/api/gmdb_medium_by_gmid?gm_id=M443" \
    > raw/togo/M443_LB_medium.json
```
...
3. **Import to KB** (`normalized_yaml/`)
...
# 6. Commit
git add normalized_yaml/ raw/togo/README.md
...
jq '[.[] | select(.ingredients | length > 0)] | length' raw/togo/togo_media.json
```
